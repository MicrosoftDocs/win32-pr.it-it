...
---
# <a name="swbemservicesexputasync-method"></a>SWbemServicesEx. PutAsync, metodo

Il metodo **PutAsync** dell'oggetto [**SWbemServicesEx**](swbemservicesex.md) Salva in modo asincrono un oggetto in uno spazio dei nomi. Quando ha esito positivo, questo metodo invia un evento [**OnCompleted**](swbemsink-oncompleted.md) all'oggetto [**SWbemSink**](swbemsink.md) specificato come parametro di input.
...
Obbligatorio. Nuovo oggetto da inserire nello spazio dei nomi. Potrebbe trattarsi di un oggetto appena creato o di un oggetto modificato.
...
Consente di aggiornare le classi anche quando sono presenti classi figlio e la modifica non provoca conflitti con le classi figlio. Utilizzare questo flag quando si aggiunge una nuova proprietà a una classe di base non citata in precedenza in nessuna delle classi figlio. Se la classe dispone di istanze, l'aggiornamento avrà esito negativo.
...
Questo flag forza gli aggiornamenti delle classi quando esistono classi figlio in conflitto. Questo flag, ad esempio, impone un aggiornamento quando viene definito un qualificatore di classe in una classe figlio e la classe di base tenta di aggiungere lo stesso qualificatore in conflitto con quello esistente. In modalità Force questo conflitto viene risolto eliminando il qualificatore in conflitto nella classe figlio. Se la classe dispone di istanze, l'aggiornamento avrà esito negativo.
...
Dopo il completamento del metodo **PutAsync** , l'oggetto [Err](/previous-versions//sbf5ze0e(v=vs.85)) può contenere uno dei codici di errore elencati di seguito.
...
L'utente corrente non dispone dell'autorizzazione per l'aggiornamento di un'istanza della classe specificata.
...
È stato specificato un valore null per una proprietà che non può essere null. Un esempio di tale proprietà è quello contrassegnato da un qualificatore di **chiave**, **indicizzato** o **Not \_ null** .
...
Questa chiamata restituisce immediatamente un risultato e i risultati e lo stato vengono restituiti al chiamante tramite gli eventi recapitati al sink specificato in *objWbemSink*. Per gestire ogni oggetto all'arrivo, creare un *objWbemSink*. Subroutine dell'evento [**OnObjectReady**](swbemsink-onobjectready.md) . Qualsiasi elaborazione eseguita dopo l'arrivo di tutti gli oggetti viene eseguita in una subroutine per *objWbemSink*. Evento [**OnCompleted**](swbemsink-oncompleted.md) .
