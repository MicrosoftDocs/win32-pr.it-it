---
description: È possibile accedere ai dispositivi SNMP leggendo o scrivendo nell'istanza della classe corrispondente in WMI SMIR (SNMP Module Information Repository).
...
È possibile accedere ai dispositivi SNMP leggendo o scrivendo nell'istanza della classe corrispondente in WMI SMIR (SNMP Module Information Repository). Quando si lavora con più dispositivi dello stesso tipo, per efficienza, caricare i MIB per un tipo di dispositivo SNMP in SMIR.
...
-   Scrivere in un dispositivo SNMP.
    ...
    Nell'esempio seguente viene eseguita un'operazione Set in una classe di dispositivo.
...
Le classi correlate sono quelle supportate da un determinato dispositivo SNMP quando si verifica l'enumerazione. La correlazione influisce sul set di classi restituite da SMIR. L'enumerazione non correlata restituisce tutte le classi presenti all'interno di SMIR, indipendentemente dal fatto che il dispositivo le supporti. Per altre informazioni sull'uso delle tecniche di correlazione WMI, vedere [Correlazione di classi SNMP WMI.](wmi-snmp-class-correlation.md)
