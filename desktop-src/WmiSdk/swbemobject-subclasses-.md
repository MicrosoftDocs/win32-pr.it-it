...
---
# <a name="swbemobjectsubclasses_-method"></a>SWbemObject. subclasss ( \_ metodo)

Il metodo delle **sottoclassi \_** dell'oggetto [**SWbemObject**](swbemobject.md) restituisce un oggetto [**SWbemObjectSet**](swbemobjectset.md) . Questo oggetto è una raccolta di sottoclassi dell'oggetto corrente, che deve essere una classe. Gli elementi nella raccolta restituita possono essere ottenuti utilizzando metodi di raccolta standard. Per ulteriori informazioni, vedere [accesso a una raccolta](accessing-a-collection.md).
...
Integer che determina il modo in cui la chiamata esegue l'enumerazione. Questo parametro può accettare i valori seguenti.
...
Fa in modo che WMI restituisca i dati di modifica della classe insieme alla definizione della classe base.
...
Dopo il completamento del metodo delle **sottoclassi \_** , l'oggetto **Err** può contenere uno dei codici di errore elencati di seguito.
...
Non è un errore per la raccolta restituita avere zero elementi se non sono presenti sottoclassi dell'oggetto corrente. Il metodo delle **sottoclassi \_** funziona solo per gli oggetti classe.
