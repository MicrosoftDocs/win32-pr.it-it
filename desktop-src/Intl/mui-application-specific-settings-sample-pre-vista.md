---
description: 'Esempio di impostazioni di MUI: Application-Specific (precedente a Windows Vista)'
ms.assetid: 932aa981-ddd9-4a5b-9003-7dafd98e3ae4
title: 'Esempio di impostazioni di MUI: Application-Specific (precedente a Windows Vista)'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103753530"
---
# <a name="mui-application-specific-settings-sample-pre-windows-vista"></a><span data-ttu-id="6b390-103">Esempio di impostazioni di MUI: Application-Specific (precedente a Windows Vista)</span><span class="sxs-lookup"><span data-stu-id="6b390-103">MUI: Application-Specific Settings Sample (Pre-Windows Vista)</span></span>

<span data-ttu-id="6b390-104">L'applicazione di esempio descritta in questo argomento è simile a quella illustrata in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), ad eccezione del fatto che l'applicazione verrà eseguita in Windows 2000 e versioni successive.</span><span class="sxs-lookup"><span data-stu-id="6b390-104">The sample application described in this topic is similar to the one shown in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), except that this application will run on Windows 2000 and later.</span></span> <span data-ttu-id="6b390-105">La differenza principale rispetto all'altro esempio è che il fallback del linguaggio deve essere personalizzato per l'esecuzione su più sistemi operativi.</span><span class="sxs-lookup"><span data-stu-id="6b390-105">The main difference from the other sample is that language fallback has to be customized to run on multiple operating systems.</span></span>

<span data-ttu-id="6b390-106">Questa applicazione analizza prima di tutto un elenco di linguaggi delimitati in un file di testo e lo converte in un elenco di lingue multistringhe per definire le preferenze di lingua specifiche dell'applicazione.</span><span class="sxs-lookup"><span data-stu-id="6b390-106">This application first parses a delimited language list in a text file and converts it to a multi-string language list to define the application-specific language preferences.</span></span> <span data-ttu-id="6b390-107">I delimitatori supportati nell'esempio sono ",", ";", ":" e "".</span><span class="sxs-lookup"><span data-stu-id="6b390-107">The delimiters supported in the sample are ",", ";", ":", and " ".</span></span> <span data-ttu-id="6b390-108">Dopo l'analisi dell'elenco, il codice trova e carica le risorse nella lingua identificata.</span><span class="sxs-lookup"><span data-stu-id="6b390-108">After parsing the list, the code finds and loads the resources in the identified language.</span></span> <span data-ttu-id="6b390-109">Diversamente dall'altro esempio di impostazioni specifiche dell'applicazione, questo codice carica e rilascia i file di risorse usando le chiamate alle funzioni MUI [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) e [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).</span><span class="sxs-lookup"><span data-stu-id="6b390-109">In contrast to the other application-specific settings sample, this code loads and releases resource files using calls to the MUI functions [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) and [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).</span></span>
