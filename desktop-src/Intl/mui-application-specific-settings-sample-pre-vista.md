...
---
# <a name="mui-application-specific-settings-sample-pre-windows-vista"></a>Esempio di impostazioni di MUI: Application-Specific (precedente a Windows Vista)

L'applicazione di esempio descritta in questo argomento è simile a quella illustrata in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), ad eccezione del fatto che l'applicazione verrà eseguita in Windows 2000 e versioni successive. La differenza principale rispetto all'altro esempio è che il fallback del linguaggio deve essere personalizzato per l'esecuzione su più sistemi operativi.

Questa applicazione analizza prima di tutto un elenco di linguaggi delimitati in un file di testo e lo converte in un elenco di lingue multistringhe per definire le preferenze di lingua specifiche dell'applicazione. I delimitatori supportati nell'esempio sono ",", ";", ":" e "". Dopo l'analisi dell'elenco, il codice trova e carica le risorse nella lingua identificata. Diversamente dall'altro esempio di impostazioni specifiche dell'applicazione, questo codice carica e rilascia i file di risorse usando le chiamate alle funzioni MUI [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) e [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).
