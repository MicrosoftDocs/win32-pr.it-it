...
Quando viene creata una catena di scambio con un effetto swap di D3DSWAPEFFECT FLIP o D3DSWAPEFFECT COPY, il runtime garantisce che un'operazione \_ \_ [**IDirect3DDevice9::P resent**](/windows/desktop/api) non influirà sul contenuto di alcun buffer nascosto. Sfortunatamente, soddisfare questa garanzia può comportare un notevole sovraccarico di elaborazione o memoria video, soprattutto quando si implementa la semantica di capovolgimento per una catena di scambio con finestra o la semantica di copia per una catena di scambio a schermo intero. Un'applicazione può usare l'effetto di scambio DISCARD D3DSWAPEFFECT per evitare questi sovraccarici e consentire al driver di visualizzazione di selezionare la tecnica di presentazione più efficiente per la \_ catena di scambio. Questo è anche l'unico effetto di scambio che può essere usato quando si specifica un valore diverso da D3DMULTISAMPLE NONE per il \_ membro MultiSampleType di [**D3DPRESENT \_ PARAMETERS.**](d3dpresent-parameters.md)

Analogamente a una catena di scambio che usa D3DSWAPEFFECT FLIP, una catena di scambio che usa D3DSWAPEFFECT DISCARD può includere più buffer nascosto, a cui è possibile accedere tramite \_ \_ [**IDirect3DDevice9::GetBackBuffer**](/windows/desktop/api) o [**IDirect3DSwapChain9::GetBackBuffer**](/windows/win32/api/d3d9helper/nf-d3d9helper-idirect3dswapchain9-getbackbuffer). La catena di scambio è meglio definita come una coda in cui 0 indicizza sempre il buffer nascosto che verrà visualizzato dalla successiva operazione Present e da cui i buffer vengono eliminati quando sono stati visualizzati.

Un'applicazione che usa questo effetto di scambio non può fare supposizioni sul contenuto di un buffer nascosto eliminato e pertanto deve aggiornare un intero buffer nascosto prima di richiamare un'operazione Present che lo visualizza. Anche se ciò non viene applicato, la versione di debug del runtime sovrascriverà il contenuto dei buffer nascosto eliminati con dati casuali per consentire agli sviluppatori di verificare che le applicazioni aggiorneranno correttamente l'intera superficie del buffer nascosto.
...
Questo effetto di scambio può essere specificato solo per una catena di scambio che comprende un singolo buffer nascosto. Indipendentemente dal fatto che la catena di scambio sia in modalità finestra o a schermo intero, il runtime garantisce la semantica implicita in un'operazione Present basata sulla copia, in modo che il contenuto del buffer nascosto sia invariato, anziché sostituirlo con il contenuto del front buffer come farebbe un'operazione Present basata su capovolgimento.
...
- D3DSWAPEFFECT OVERLAY è disponibile solo \_ in Direct3D9Ex in esecuzione in Windows 7 (o più sistema operativo corrente).
...
Indica quando un'applicazione sta adottando la modalità capovolgimento, durante il quale il frame di un'applicazione viene passato anziché copiato nel Gestione finestre desktop (DWM) per la composizione quando l'applicazione viene visualizzata in modalità finestra. La modalità capovolgimento consente a un'applicazione di usare in modo più efficiente la larghezza di banda della memoria, nonché di consentire a un'applicazione di sfruttare le statistiche presenti a schermo intero. La modalità capovolgimento non influisce sul comportamento a schermo intero.

> [!Note]  
> Se si crea una catena di scambio con D3DSWAPEFFECT FLIPEX, non è possibile eseguire l'override del membro \_ **hDeviceWindow** della struttura [**PARAMETERS di D3DPRESENT \_**](d3dpresent-parameters.md) quando si presenta un nuovo frame per la visualizzazione. Ciò significa che è necessario passare **NULL** al parametro *hDestWindowOverride* di [**IDirect3DDevice9Ex::P resentEx**](/windows/desktop/api/d3d9/nf-d3d9-idirect3ddevice9ex-presentex) per indicare al runtime di usare il membro **hDeviceWindow** di **D3DPRESENT \_ PARAMETERS** per la presentazione.

Differenze tra Direct3D 9 e Direct3D 9Ex:

- D3DSWAPEFFECT FLIPEX è disponibile solo \_ in Direct3D9Ex in esecuzione in Windows 7 (o più sistemi operativi correnti).
...
Lo stato del buffer nascosto dopo una chiamata a Present è ben definito da ognuno di questi effetti di scambio e se il dispositivo Direct3D è stato creato con una catena di scambio a schermo intero o una catena di scambio finestra non ha alcun effetto su questo stato. In particolare, l'effetto di scambio FLIP D3DSWAPEFFECT funziona allo stesso modo, sia con finestra che a schermo intero, e il runtime Direct3D garantisce questo comportamento creando \_ buffer aggiuntivi. Di conseguenza, è consigliabile che le applicazioni usino D3DSWAPEFFECT DISCARD quando possibile \_ per evitare tali penalità. Questo è dovuto al fatto che questo effetto di scambio sarà sempre il più efficiente in termini di utilizzo della memoria e prestazioni.
