...
|1.1.0|DML_FEATURE_LEVEL_2_0|`0x2000`|Windows 10 versione 2004 (10.0; Build 19041) (aggiornamento Windows 10 maggio 2020). Aka "20H1".|N/D|
|1.0.0|DML_FEATURE_LEVEL_1_0|`0x1000`|Windows 10 versione 1903 (10.0; Build 18362) (Aggiornamento di Windows 10 (maggio 2019)). Aka "19H1".|N/D|
...
Per praticità, alcune funzionalità nel file di intestazione vengono dichiarate in modo condizionale in base `DirectML.h` al valore della `DML_TARGET_VERSION` macro. Impostando la `DML_TARGET_VERSION` macro su determinati valori, è possibile escludere parti di `DirectML.h` dall'applicazione.

Ciò può essere utile se si usa una copia più recente di , ma si ha come destinazione una versione inferiore del file binario DirectML, perché garantisce che qualsiasi tentativo di usare funzionalità oltre il livello di destinazione scelto non venga `DirectML.h` compilato. Questo meccanismo è simile alla `NTDDI_VERSION` macro (vedere [Macro per le dichiarazioni condizionali](../winprog/using-the-windows-headers.md#macros-for-conditional-declarations)).

Ecco i valori validi per la `DML_TARGET_VERSION` macro.

|DML_TARGET_VERSION|Effetto|
|-|-|
|`0x4000`|Tutte le funzionalità che richiedono una versione di DirectML successiva alla **1.6.0** sono escluse da `DirectML.h` .|
...
Se `DML_TARGET_VERSION` non è impostato, viene selezionato automaticamente nel modo seguente.
...
* In caso contrario, la versione di destinazione viene selezionata in base al valore della `NTDDI_VERSION` macro.
  *  `NTDDI_WIN10_CO` comporta una versione di destinazione di `0x4000` .
  *  `NTDDI_WIN10_FE` comporta una versione di destinazione di `0x3000` .
...
Si consideri un'applicazione che usa la versione 10.0.19041.0 (Windows 10, versione 2004) di Windows Software Development Kit (SDK). Dalla tabella precedente, la versione di DirectML a cui corrisponde è 1.1.0 e la corrispondente `DML_TARGET_VERSION` è `0x2000` .
...
Ad esempio, un'applicazione compilata su un SDK più recente, ma in esecuzione in una versione precedente di Windows, potrebbe (in fase di esecuzione) visualizzare un livello di funzionalità supportato inferiore, anche se viene compilato con l'SDK più recente.
