...
Si inizia creando sia una grafica che una firma radice di calcolo nel metodo **LoadAssets** . Entrambe le firme radice hanno una visualizzazione buffer costante radice (CBV) e una tabella dei descrittori di visualizzazione risorse shader (SRV). La firma radice di calcolo dispone anche di una tabella dei descrittori UAV (unordered Access View).
...
|-----------------------------------------------------------------------|-----------------------------------------------------------------------|
| [**\_Intervallo descrittore CD3DX12 \_**](cd3dx12-descriptor-range.md)        | [**\_Tipo di \_ intervallo descrittore D3D12 \_**](/windows/win32/api/d3d12/ne-d3d12-d3d12_descriptor_range_type) |
| [**\_Parametro radice \_ CD3DX12**](cd3dx12-root-parameter.md)            | [**\_Visibilità shader D3D12 \_**](/windows/win32/api/d3d12/ne-d3d12-d3d12_shader_visibility)          |
| [**\_Descrizione della \_ firma \_ radice CD3DX12**](cd3dx12-root-signature-desc.md) | [**\_Flag di \_ firma \_ radice D3D12**](/windows/win32/api/d3d12/ne-d3d12-d3d12_root_signature_flags)   |
| [**ID3DBlob**](/previous-versions/windows/desktop/legacy/ff728743(v=vs.85))                                   |                                                                       |
| [**D3D12SerializeRootSignature**](/windows/win32/api/d3d12/nf-d3d12-d3d12serializerootsignature)    | [**\_Versione della \_ firma \_ radice D3D**](/windows/win32/api/d3d12/ne-d3d12-d3d_root_signature_version)   |
...
## <a name="create-the-cbv-and-vertex-buffers"></a>Creare i buffer CBV e vertex

Per la pipeline grafica, CBV è uno **struct** che contiene due matrici usate dal geometry shader. Geometry shader acquisisce la posizione di ogni particella nel sistema e genera un quad per rappresentarlo usando queste matrici.
...
Di conseguenza, il buffer dei vertici utilizzato dal vertex shader in realtà non contiene alcun dato posizionale.
...
Per la pipeline di calcolo, CBV è uno **struct** che contiene alcune costanti usate dalla simulazione di gravità del corpo n nel compute shader.
...
Una volta inizializzati tutti i buffer, verranno avviati il rendering e il lavoro di calcolo. Il thread di calcolo cambierà lo stato dei due buffer di posizione/velocità tra SRV e UAV durante l'iterazione della simulazione e il thread di rendering deve assicurarsi che pianifica il lavoro sulla pipeline grafica che opera sul SRV. Le recinzioni vengono usate per sincronizzare l'accesso ai due buffer.
...
| [**Oggetti executecommandlist**](/windows/win32/api/d3d12/nf-d3d12-id3d12commandqueue-executecommandlists)  |            |
...
Per semplificare l'esempio, il thread di calcolo attende che la GPU completi ogni iterazione prima di pianificare altre attività di calcolo. In pratica, è probabile che le applicazioni mantengano la coda di calcolo completa per ottenere le massime prestazioni dalla GPU.
