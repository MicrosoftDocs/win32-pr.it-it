...
description: Informazioni sul valore di riferimento dello stencil nella grafica Direct3D 12. L'abilitazione dei pixel shader per l'uso del valore di riferimento dello stencil consente un controllo fine sulle operazioni di stencil.
...
Il valore di riferimento dello stencil viene in genere specificato dal [**metodo ID3D12GraphicsCommandList::OMSetStencilRef.**](/windows/desktop/api/d3d12/nf-d3d12-id3d12graphicscommandlist-omsetstencilref) Questo metodo imposta il valore di riferimento dello stencil su una granularità per disegno. Tuttavia, questo valore può essere sovrascritto dal pixel shader.

Questa funzionalità D3D12 (e D3D11.3) consente agli sviluppatori di leggere e usare il valore di riferimento dello stencil (*SV \_ StencilRef*) restituito da un pixel shader, consentendo una granularità per pixel o per campione.
