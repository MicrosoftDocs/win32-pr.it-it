...
Si consideri un **DML_MAX_POOLING2_OPERATOR_DESC** 2x2 senza spaziatura interna né dilazioni e uno stride di 1, che esegue le operazioni seguenti.
...
Nel caso di un tie (in cui più di un elemento in una finestra ha lo stesso valore massimo), viene scelto l'elemento con l'indice logico più basso.
...
Tensore della funzionalità di input. Si tratta in genere dello stesso tensore fornito *dall'inputTensor* DML_MAX_POOLING2_OPERATOR_DESC [nel](./ns-directml-dml_max_pooling2_operator_desc.md) passaggio in avanti.

`InputGradientTensor`

Tipo: **const [DML_TENSOR_DESC](/windows/win32/api/directml/ns-directml-dml_tensor_desc) \***

Tensore sfumatura in ingresso. Questa operazione viene in genere ottenuta dall'output della backpropagazione di un livello precedente. In genere questo tensore ha le stesse dimensioni *dell'output* dell'oggetto [DML_MAX_POOLING2_OPERATOR_DESC](./ns-directml-dml_max_pooling2_operator_desc.md) nel passaggio in avanti.

`OutputGradientTensor`

Tipo: **const [DML_TENSOR_DESC](/windows/win32/api/directml/ns-directml-dml_tensor_desc) \***

Tensore di output contenente le sfumature backpropagate. In genere, questo tensore ha le stesse dimensioni *dell'input* dell'oggetto [DML_MAX_POOLING2_OPERATOR_DESC](./ns-directml-dml_max_pooling2_operator_desc.md) nel passaggio in avanti.
