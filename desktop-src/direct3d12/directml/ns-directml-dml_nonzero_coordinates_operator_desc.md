...
Questo operatore produce una matrice MxN di valori, in cui ogni riga M contiene una coordinata N-dimensionale di un valore diverso da zero dall'input. Quando si usano input **FLOAT32** o **FLOAT16,** sia il valore 0 negativo che quello positivo (0,0f e -0,0f) vengono considerati come zero ai fini di questo operatore.

L'operatore richiede che *outputCoordinatesTensor* abbia dimensioni sufficienti per supportare uno scenario nel peggiore dei casi in cui ogni elemento dell'input è diverso da zero. Questo operatore restituisce il conteggio degli elementi diversi da zero tramite *OutputCountTensor*, che i chiamanti possono controllare per determinare il numero di coordinate scritte in *OutputCoordinatesTensor.*
...
Tensore di output che contiene il conteggio degli elementi diversi da zero nel tensore di input. Questo tensore deve essere &mdash; scalare, ad esempio le dimensioni di questo tensore devono essere tutte 1. Il tipo di questo tensore deve essere **UINT32.**
...
Questo tensore  deve avere dimensioni pari a (se DimensionCount è 4) o (se DimensionCount è 5), dove M è il numero totale di elementi in InputTensor e N è maggiore o uguale alla classificazione effettiva di `{1,1,M,N}`  `{1,1,1,M,N}`  *InputTensor*, fino a *DimensionCount*  dell'input. 
...
Anche se questo tensore richiede che la dimensione M sia uguale al numero di elementi nel tensore di input, questo operatore scriverà solo un massimo di elementi OutputCount in questo tensore. OutputCount viene restituito tramite *outputCountTensor scalare.*
