...
Tensore facoltativo contenente i dati ATensor zero point. Le dimensioni previste di sono se è necessaria la quantizzazione per `AZeroPointTensor` `{ 1, 1, 1, 1 }` tensore o se è necessaria la `{ 1, 1, M, 1 }` quantizzazione per riga. Questi valori di punto zero vengono usati per dequantizzare i *valori di ATensor.*
...
Tensore facoltativo contenente i dati del punto zero *di BTensor.* Le dimensioni previste di sono `BZeroPointTensor` se è necessaria la `{ 1, 1, 1, 1 }` quantizzazione per tensore o se è necessaria `{ 1, 1, 1, N }` la quantizzazione per colonna. Questi valori di punto zero vengono usati per dequantizzare i valori BTensor.
