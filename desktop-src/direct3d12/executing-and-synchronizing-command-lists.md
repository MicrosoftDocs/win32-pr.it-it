...
-   [Sincronizzazione delle risorse a cui si accede dalle code di comandi](#synchronizing-resources-accessed-by-command-queues)
...
-   [**CALCOLO DEL TIPO DI ELENCO DI COMANDI D3D12 \_ \_ \_ \_**](/windows/desktop/api/d3d12/ne-d3d12-d3d12_command_list_type)
-   [**COPIA DEL TIPO DI \_ \_ ELENCO COMANDI \_ D3D12 \_**](/windows/desktop/api/d3d12/ne-d3d12-d3d12_command_list_type)

In generale, le code e gli elenchi di comandi DIRECT accettano qualsiasi comando, le code COMPUTE e gli elenchi di comandi accettano comandi correlati al calcolo e alla copia e le code e gli elenchi di comandi COPY accettano solo comandi di copia.

## <a name="executing-command-lists"></a>Esecuzione di elenchi di comandi

Dopo aver registrato un elenco di comandi e recuperato la coda di comandi predefinita o averne creato uno nuovo, è possibile eseguire gli elenchi di comandi chiamando [**ID3D12CommandQueue::ExecuteCommandLists**](/windows/desktop/api/d3d12/nf-d3d12-id3d12commandqueue-executecommandlists).
...
-   Il limite della coda di comandi indica che un'esecuzione precedente dell'elenco di comandi non è stata ancora completata. I recinti della coda di comandi sono descritti in dettaglio di seguito.
...
-   Gli stati prima e dopo le barriere di transizione delle risorse non corrispondono correttamente. Per altre informazioni, vedere [Uso delle barriere alle risorse per sincronizzare gli stati delle risorse.](using-resource-barriers-to-synchronize-resource-states-in-direct3d-12.md)
...
1. Una risorsa non può essere scritta in da più code di comandi contemporaneamente. Quando una risorsa è passata a uno stato scrivibile in una coda, viene considerata di proprietà esclusiva di tale coda e deve passare a uno stato di lettura o COMMON (vedere [**D3D12_RESOURCE_STATES**](/windows/desktop/api/d3d12/ne-d3d12-d3d12_resource_states)) prima che possa essere accessibile da un'altra coda.
...
| Metodo                                                              | Descrizione                                                                                              |
|---------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| [**CopyTileMappings**](/windows/desktop/api/d3d12/nf-d3d12-id3d12commandqueue-copytilemappings)     | Copia i mapping da una risorsa in riquadri di origine a una risorsa affiancata di destinazione.<br/>                 |
