...
---
# <a name="directml-helper-functions"></a>Funzioni helper DirectML

## <a name="dmlcalcbuffertensorsize"></a>DMLCalcBufferTensorSize

Questa funzione helper calcola il numero minimo di byte necessari per archiviare un tensore del buffer con il tipo, le dimensioni e gli stride specificati. La formula può essere espressa come indicato di seguito.
...
In altre parole, la dimensione minima di un tensore è l'indice dell'elemento One-Past-the-end, moltiplicato per le dimensioni dell'elemento (ad esempio, 2 byte per un tensore **FLOAT16** ). Inoltre, DirectML richiede che tutti i buffer associati abbiano una dimensione totale con allineamento **DWORD** e, di conseguenza, la dimensione minima implicita in byte deve essere arrotondata al limite di 4 byte più vicino.
...
Questa funzione helper calcola gli stride per i tensori 4D con layout NCHW o NHWC e la trasmissione facoltativa.
