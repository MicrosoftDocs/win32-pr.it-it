...
Handle per le credenziali del server. Il server chiama la [**funzione AcquireCredentialsHandle (Negotiate)**](acquirecredentialshandle--negotiate.md) con il \_ flag SECPKG CRED INBOUND o \_ SECPKG CRED BOTH impostato per recuperare \_ questo \_ handle.
...
La **funzione AcceptSecurityContext (Negotiate)** è la controparte del server alla [**funzione InitializeSecurityContext (Negotiate).**](initializesecuritycontext--negotiate.md)
...
3.  Il client riceve il token e lo passa a [**InitializeSecurityContext (Negotiate)**](initializesecuritycontext--negotiate.md). Se **InitializeSecurityContext (Negotiate)** restituisce SEC \_ E \_ OK, l'autenticazione reciproca è stata completata e può iniziare una sessione protetta. Se **InitializeSecurityContext (Negotiate)** restituisce un codice di errore, la negoziazione di autenticazione reciproca termina. In caso contrario, il token di sicurezza restituito da **InitializeSecurityContext (Negotiate)** viene inviato al client e i passaggi 2 e 3 vengono ripetuti.
