...
---
# <a name="initializesecuritycontext-ntlm-function"></a>Funzione InitializeSecurityContext (NTLM)

La funzione **InitializeSecurityContext (NTLM)** avvia il lato client, il contesto di [*sicurezza*](../secgloss/s-gly.md) in uscita da un handle di credenziale. La funzione viene utilizzata per creare un [*contesto di sicurezza*](../secgloss/s-gly.md) tra l'applicazione client e un peer remoto. **InitializeSecurityContext (NTLM)** restituisce un token che il client deve passare al peer remoto, a sua volta inviato dal peer all'implementazione della sicurezza locale tramite la chiamata [**AcceptSecurityContext (NTLM)**](acceptsecuritycontext--ntlm.md) . Il token generato deve essere considerato opaco da tutti i chiamanti.

In genere, la funzione **InitializeSecurityContext (NTLM)** viene chiamata in un ciclo fino a quando non viene stabilito un [*contesto di sicurezza*](../secgloss/s-gly.md) sufficiente.
...
Handle per le credenziali restituite da [**AcquireCredentialsHandle (NTLM)**](acquirecredentialshandle--ntlm.md). Questo handle viene utilizzato per compilare il [*contesto di sicurezza*](../secgloss/s-gly.md). La funzione **InitializeSecurityContext (NTLM)** richiede almeno credenziali in uscita.
...
Puntatore a una struttura [CtxtHandle](sspi-handles.md) . Alla prima chiamata a **InitializeSecurityContext (NTLM)**, questo puntatore è **null**. Alla seconda chiamata, questo parametro è un puntatore all'handle per il contesto parzialmente formato restituito nel parametro *phNewContext* dalla prima chiamata.
...
Usare un nome di destinazione completo poiché i nomi brevi non sono supportati tra foreste.
...
Puntatore a una struttura [CtxtHandle](sspi-handles.md) . Alla prima chiamata a **InitializeSecurityContext (NTLM)**, questo puntatore riceve il nuovo handle di contesto. Alla seconda chiamata, *phNewContext* può corrispondere all'handle specificato nel parametro *phContext* .
...
|----------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <dl> <dt>**SEC \_ E \_ OK**</dt> </dl>                      | Il [*contesto di sicurezza*](../secgloss/s-gly.md) è stato inizializzato correttamente. Non è necessario eseguire un'altra chiamata a [**InitializeSecurityContext (NTLM)**](initializesecuritycontext--ntlm.md) . Se la funzione restituisce un token di output, ovvero se il token SECBUFFER \_ in *pOutput* è di lunghezza diversa da zero, il token deve essere inviato al server.<br/> |
...
| <dl> <dt>**SEC \_ \_ continuazione \_ necessaria**</dt> </dl>        | Il client deve inviare il token di output al server e attendere un token restituito. Il token restituito viene quindi passato in un'altra chiamata a [**InitializeSecurityContext (NTLM)**](initializesecuritycontext--ntlm.md). Il token di output può essere vuoto.<br/>                                                                                                                                                       |
...
La funzione **InitializeSecurityContext (NTLM)** viene utilizzata da un client per inizializzare un contesto in uscita.
...
3.  Il client invia il token restituito nel buffer *pOutput* al server di destinazione. Il server passa quindi il token come argomento di input in una chiamata alla funzione [**AcceptSecurityContext (NTLM)**](acceptsecuritycontext--ntlm.md) .
4.  [**AcceptSecurityContext (NTLM)**](acceptsecuritycontext--ntlm.md) può restituire un token, che il server invia al client per una seconda chiamata a **InitializeSecurityContext (NTLM)** se la prima chiamata restituita al secondo, che continua a essere \_ \_ \_ necessaria.
...
3.  Al momento della ricezione della risposta del server, il client chiama di nuovo **InitializeSecurityContext (NTLM)** , con *phContext* impostato sull'handle restituito dall'ultima chiamata a. Il token ricevuto dal server viene fornito nel parametro *Pinput* .
...
L'azione eseguita dal client dipende dal codice restituito da questa funzione. Se il codice restituito è SEC \_ E \_ OK, non sarà prevista una seconda chiamata **InitializeSecurityContext (NTLM)** e non sarà prevista alcuna risposta dal server. Se il codice restituito è SEC \_ \_ , continuerà a essere \_ necessario, il client aspetta un token in risposta dal server e lo passa in una seconda chiamata a **InitializeSecurityContext (NTLM)**. Il \_ \_ \_ codice restituito necessario per il completamento del secondo indica che il client deve completare la compilazione del messaggio e chiamare la funzione [**CompleteAuthToken**](/windows/win32/api/sspi/nf-sspi-completeauthtoken) . Il \_ \_ \_ codice per il completamento e la \_ continuazione del codice incorpora entrambe le azioni.

Se **InitializeSecurityContext (NTLM)** restituisce Success sulla prima o solo chiamata, il chiamante deve chiamare la funzione [**DeleteSecurityContext**](/windows/win32/api/sspi/nf-sspi-deletesecuritycontext) sull'handle restituito, anche se la chiamata ha esito negativo in una fase successiva dello scambio di autenticazione.

Il client può chiamare nuovamente **InitializeSecurityContext (NTLM)** dopo che è stato completato correttamente. Indica al pacchetto di [*sicurezza*](../secgloss/s-gly.md) che si desidera una riautenticazione.
