---
description: L'autenticazione iniziale avviene quando il server riceve una risposta di richiesta da un client.
...
L'autenticazione iniziale avviene quando il server riceve una risposta di richiesta da un client. L'autenticazione di una risposta di richiesta richiede in genere almeno due server:

-   Il server di origine: riceve la richiesta dal client e riceve una richiesta e quindi riceve la risposta di richiesta dal client che deve essere autenticato.
...
-   L'identità del server di origine viene verificata rispetto al server codificato nel nonce della richiesta.
-   Viene controllato il timestamp codificato nel nonce. Se il nonce è scaduto e le informazioni sul nome utente/password sono valide, il server di origine termina l'autenticazione emettendo una nuova richiesta digest con la direttiva non obsoleta impostata su "true". Ciò indica che solo il nonce era "obsoleto" e il client può rispondere alla nuova richiesta usando la password usata nella risposta precedente. Se il client riceve una nuova richiesta dopo l'invio della risposta di verifica per la richiesta non valida, il client deve generare una nuova risposta di richiesta.
-   Se viene applicato il rilevamento della riproduzione, la direttiva nc (conteggio nonce) viene verificata rispetto al database di sessione nonce gestito dal server.
-   Il server di autenticazione viene identificato e inviato le informazioni di autorizzazione del client.
-   Il server di autenticazione controlla l'identità del server codificato nel server nonce rispetto all'identità del server di origine.
-   Il server di autenticazione, che è un controller di dominio, recupera la password dell'utente.
-   Usando le informazioni di autorizzazione, la password e l'identificazione del server di origine, il server di autenticazione calcola il valore che il client deve aver fornito nella direttiva di risposta della richiesta di verifica. Il server di autenticazione confronta il valore calcolato con la risposta del client per determinare l'esito positivo o negativo dell'autenticazione.
...
La chiave di sessione digest restituita dal server di autenticazione viene memorizzata nella cache dal server di origine per l'uso nell'autenticazione delle richieste future. Per altre informazioni, vedere [Autenticazione delle richieste successive tramite Microsoft Digest](authenticating-subsequent-requests-using-microsoft-digest.md).
