---
description: Avvia il contesto di sicurezza in uscita [*sul*](../secgloss/s-gly.md) lato client da un handle di credenziali usando la delega [*vincolata*](../secgloss/s-gly.md)Negotiate.
...
Handle per le credenziali restituite da [**AcquireCredentialsHandle (Negotiate).**](acquirecredentialshandle--negotiate.md) Questo handle viene utilizzato per compilare il [*contesto di sicurezza*](../secgloss/s-gly.md). La **funzione InitializeSecurityContext (Negotiate)** richiede almeno credenziali OUTBOUND.
...
Puntatore a una [struttura CtxtHandle.](sspi-handles.md) Alla prima chiamata a **InitializeSecurityContext (Negotiate)** questo puntatore è **NULL.** Nella seconda chiamata questo parametro è un puntatore all'handle al contesto parzialmente formato restituito nel parametro *phNewContext* dalla prima chiamata.
...
Le applicazioni devono fornire un nome SPN valido per mitigare gli attacchi di riproduzione.
...
Puntatore a una [struttura CtxtHandle.](sspi-handles.md) Alla prima chiamata a **InitializeSecurityContext (Negotiate),** questo puntatore riceve il nuovo handle di contesto. Nella seconda chiamata, *phNewContext* può essere uguale all'handle specificato nel *parametro phContext.*
...
|----------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <dl> <dt>**SEC \_ E \_ OK**</dt> </dl>                      | Il [*contesto di sicurezza*](../secgloss/s-gly.md) è stato inizializzato correttamente. Non è necessaria [**un'altra chiamata InitializeSecurityContext (Negotiate).**](initializesecuritycontext--negotiate.md) Se la funzione restituisce un token di output, ad esempio se IL TOKEN SECBUFFER \_ in *pOutput* è di lunghezza diversa da zero, tale token deve essere inviato al server.<br/> |
...
La **funzione InitializeSecurityContext (Negotiate)** viene usata da un client per inizializzare un contesto in uscita.
...
4.  [**AcceptSecurityContext (Negotiate)**](acceptsecuritycontext--negotiate.md) può restituire un token che il server invia al client per una seconda chiamata a **InitializeSecurityContext (Negotiate)** se la prima chiamata ha restituito SEC \_ I CONTINUE \_ \_ NEEDED.
...
3.  Al momento della ricezione della risposta del server, il client chiama nuovamente **InitializeSecurityContext (Negotiate),** con *phContext* impostato sull'handle restituito dall'ultima chiamata. Il token ricevuto dal server viene fornito nel *parametro pInput.*
...
L'azione eseguita dal client dipende dal codice restituito da questa funzione. Se il codice restituito è SEC E OK, non sarà presente alcuna seconda chiamata \_ \_ a **InitializeSecurityContext (Negotiate)** e non è prevista alcuna risposta dal server. Se il codice restituito è SEC I CONTINUE NEEDED, il client prevede un token in risposta dal server e lo passa in una seconda chiamata \_ \_ a \_ **InitializeSecurityContext (Negotiate).** Il codice restituito SEC I COMPLETE NEEDED indica che il client deve completare la compilazione del \_ messaggio e chiamare la funzione \_ \_ [**CompleteAuthToken.**](/windows/win32/api/sspi/nf-sspi-completeauthtoken) Il codice SEC \_ I COMPLETE AND CONTINUE incorpora \_ \_ \_ entrambe queste azioni.

Se **InitializeSecurityContext (Negotiate)** restituisce l'esito positivo alla prima (o solo) chiamata, il chiamante deve chiamare la funzione [**DeleteSecurityContext**](/windows/win32/api/sspi/nf-sspi-deletesecuritycontext) sull'handle restituito, anche se la chiamata ha esito negativo in una fase successiva dello scambio di autenticazione.

Il client può chiamare **nuovamente InitializeSecurityContext (Negotiate)** dopo il completamento. Ciò indica al pacchetto [*di sicurezza che*](../secgloss/s-gly.md) è necessario eseguire una riautenticazione.
