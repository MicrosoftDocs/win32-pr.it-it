...
title: Metodo ProtectKeyWithTPMAndStartupKey della Win32_EncryptableVolume classe
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119667131"
---
# <a name="protectkeywithtpmandstartupkey-method-of-the-win32_encryptablevolume-class"></a>Metodo ProtectKeyWithTPMAndStartupKey della classe \_ EncryptableVolume Win32

Il metodo **ProtectKeyWithTPMAndStartupKey** della classe [**Win32 \_ EncryptableVolume**](win32-encryptablevolume.md) protegge la chiave di crittografia del volume usando l'hardware di sicurezza del Trusted Platform Module (TPM) nel computer, se disponibile, migliorato da una chiave esterna che deve essere presentata al computer all'avvio.

Sia la convalida da parte del TPM che l'input di un dispositivo di memoria USB che contiene la chiave esterna sono necessari per accedere alla chiave di crittografia del volume e sbloccare il contenuto del volume. Usare il [**metodo SaveExternalKeyToFile**](saveexternalkeytofile-win32-encryptablevolume.md) per salvare questa chiave esterna in un file in un dispositivo di memoria USB per l'utilizzo come chiave di avvio.
...
Identificatore di stringa assegnato dall'utente per questa protezione con chiave. Se questo parametro non viene specificato, viene usato un valore vuoto.
...
-   Radice principale di attendibilità della misurazione (CRTM)
-   BIOS
-   Estensioni della piattaforma (PCR 0)
-   Option ROM Code (PCR 2)
-   Codice MBR (Master Boot Record) (PCR 4)
-   Tabella delle partizioni MBR (Master Boot Record) (PCR 5)
-   Settore di avvio NTFS (PCR 8)
-   BLOCCO DI AVVIO NTFS (PCR 9)
-   Boot Manager (PCR 10)
-   Controllo di accesso BitLocker (PCR 11)

Per la sicurezza del computer, è consigliabile usare il profilo predefinito. Per una protezione aggiuntiva dalle modifiche della configurazione di avvio anticipato, usare un profilo di PCR 0, 1, 2, 3, 4, 5, 8, 9, 10, 11. Per impostazione Extensible Firmware Interface computer basati su UEFI (Unified Extensible Firmware Interface) non usano PCR 5.
...
Al massimo può esistere una protezione con chiave di tipo "TPM e chiave di avvio" per un volume in qualsiasi momento. Se si vuole modificare il nome visualizzato o il profilo di convalida della piattaforma usato da una protezione con chiave "TPM plus Startup Key" esistente, è necessario prima rimuovere la protezione della chiave esistente e quindi chiamare **ProtectKeyWithTPMAndStartupKey** per crearne una nuova. È necessario specificare altre protezione delle chiavi per sbloccare il volume negli scenari di ripristino in cui non è possibile ottenere l'accesso alla chiave di crittografia del volume. ad esempio quando il TPM non è in grado di eseguire correttamente la convalida rispetto al profilo di convalida della piattaforma o quando la memoria USB che contiene la chiave esterna viene persa.
...
Sebbene sia possibile avere una protezione con chiave di tipo "TPM" e un'altra di tipo "TPM e chiave di avvio", la presenza del tipo di protezione della chiave "TPM" nega gli effetti di altre protezione delle chiavi basate su TPM.
