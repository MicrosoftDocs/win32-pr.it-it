...
---
# <a name="configuring-an-installable-driver"></a><span data-ttu-id="75cf5-107">Configurazione di un driver installabile</span><span class="sxs-lookup"><span data-stu-id="75cf5-107">Configuring an Installable Driver</span></span>

<span data-ttu-id="75cf5-108">Per indirizzare un driver installabile per eseguire attività utili, è necessario aprire il driver usando la funzione [OpenDriver](/windows/win32/api/mmiscapi/nf-mmiscapi-opendriver) e inviare i messaggi tramite la funzione [SendDriverMessage](/windows/win32/api/mmiscapi/nf-mmiscapi-senddrivermessage) .</span><span class="sxs-lookup"><span data-stu-id="75cf5-108">To direct an installable driver to carry out useful tasks, you must open the driver by using the [OpenDriver](/windows/win32/api/mmiscapi/nf-mmiscapi-opendriver) function and send it messages by using the [SendDriverMessage](/windows/win32/api/mmiscapi/nf-mmiscapi-senddrivermessage) function.</span></span> <span data-ttu-id="75cf5-109">Nell'esempio seguente viene illustrato come indirizzare il driver per visualizzare la relativa finestra di dialogo di configurazione.</span><span class="sxs-lookup"><span data-stu-id="75cf5-109">The following example shows how to direct the driver to display its configuration dialog box.</span></span>
