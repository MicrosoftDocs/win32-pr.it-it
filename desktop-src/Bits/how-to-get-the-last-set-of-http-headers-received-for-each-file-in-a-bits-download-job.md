...
---
# <a name="how-to-get-the-last-set-of-http-headers-received-for-each-file-in-a-bits-download-job"></a><span data-ttu-id="1d506-104">Come ottenere l'ultimo set di intestazioni HTTP ricevute per ogni file in un processo di download BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-104">How to get the last set of HTTP headers received for each file in a BITS download job</span></span>

<span data-ttu-id="1d506-105">Questo esempio illustra come usare il nuovo metodo [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) dell'interfaccia [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) per ottenere le ultime intestazioni HTTP del set ricevute per ogni file in un processo di download servizio trasferimento intelligente in background (BITS).</span><span class="sxs-lookup"><span data-stu-id="1d506-105">This sample shows how to use the new [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) interface's [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) method to obtain the last set HTTP headers received for each file in a Background Intelligent Transfer Service (BITS) download job.</span></span> <span data-ttu-id="1d506-106">Le informazioni contenute nell'intestazione HTTP possono essere utilizzate, ad esempio, per determinare il tipo di file o la data dell'Ultima modifica nel server.</span><span class="sxs-lookup"><span data-stu-id="1d506-106">The information in the HTTP header could be used, for example, to determine the file type or when it last changed on the server.</span></span> <span data-ttu-id="1d506-107">Prima di Windows 8 e Windows Server 2012, BITS non forniva un mezzo tramite il quale l'applicazione poteva recuperare e controllare le intestazioni di risposta HTTP di un download completato.</span><span class="sxs-lookup"><span data-stu-id="1d506-107">Prior to Windows 8 and Windows Server 2012, BITS did not provide a means by which the application could retrieve and inspect the HTTP response headers of a completed download.</span></span> <span data-ttu-id="1d506-108">Questo esempio illustra come usare l'API BITS per creare un processo BITS con più URL da scaricare, elencare gli URL in un processo e recuperare le intestazioni di risposta HTTP per ogni URL.</span><span class="sxs-lookup"><span data-stu-id="1d506-108">This sample shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>

## <a name="what-you-need-to-know"></a><span data-ttu-id="1d506-109">Informazioni importanti</span><span class="sxs-lookup"><span data-stu-id="1d506-109">What you need to know</span></span>

### <a name="technologies"></a><span data-ttu-id="1d506-110">Tecnologie</span><span class="sxs-lookup"><span data-stu-id="1d506-110">Technologies</span></span>

### <a name="prerequisites"></a><span data-ttu-id="1d506-111">Prerequisiti</span><span class="sxs-lookup"><span data-stu-id="1d506-111">Prerequisites</span></span>

-   <span data-ttu-id="1d506-112">Microsoft Visual Studio</span><span class="sxs-lookup"><span data-stu-id="1d506-112">Microsoft Visual Studio</span></span>

## <a name="instructions"></a><span data-ttu-id="1d506-113">Istruzioni</span><span class="sxs-lookup"><span data-stu-id="1d506-113">Instructions</span></span>

### <a name="step-1-include-the-required-bits-header-files"></a><span data-ttu-id="1d506-114">Passaggio 1: includere i file di intestazione BITS necessari</span><span class="sxs-lookup"><span data-stu-id="1d506-114">Step 1: Include the required BITS header files</span></span>

<span data-ttu-id="1d506-115">Inserire le direttive header seguenti all'inizio del file di origine.</span><span class="sxs-lookup"><span data-stu-id="1d506-115">Insert the following header directives at the top of the source file.</span></span>
...
### <a name="step-2-initialize-com-and-instantiate-a-bits-background-copy-manager-object-interface"></a><span data-ttu-id="1d506-116">Passaggio 2: inizializzare COM e creare un'istanza di un'interfaccia dell'oggetto gestione copia in background BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-116">Step 2: Initialize COM and instantiate a BITS Background Copy Manager object interface</span></span>

<span data-ttu-id="1d506-117">Prima di creare un'istanza dell'interfaccia [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) (utilizzata per creare un processo BITS), è necessario inizializzare com e impostare il modello di threading COM desiderato.</span><span class="sxs-lookup"><span data-stu-id="1d506-117">Before instantiating the [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) interface (used to create a BITS job), you must initialize COM and set the desired COM threading model.</span></span>
...
### <a name="step-3-create-the-bits-job"></a><span data-ttu-id="1d506-118">Passaggio 3: creare il processo BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-118">Step 3: Create the BITS job</span></span>

<span data-ttu-id="1d506-119">Solo l'utente che crea il processo o un utente con privilegi di amministratore può aggiungere file al processo e modificare le proprietà del processo.</span><span class="sxs-lookup"><span data-stu-id="1d506-119">Only the user who creates the job or a user with administrator privileges can add files to the job and change the job's properties.</span></span>
...
### <a name="step-4-add-the-files-to-the-bits-job"></a><span data-ttu-id="1d506-120">Passaggio 4: aggiungere i file al processo BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-120">Step 4: Add the files to the BITS job</span></span>

<span data-ttu-id="1d506-121">Nell'esempio seguente vengono scaricati i documenti disponibili pubblicamente dall'area download Microsoft.</span><span class="sxs-lookup"><span data-stu-id="1d506-121">The following example downloads publicly available documents from the Microsoft Download Center.</span></span> <span data-ttu-id="1d506-122">Sarà necessario modificare questi valori per l'ambiente specifico.</span><span class="sxs-lookup"><span data-stu-id="1d506-122">You'll need to change these values for your specific environment.</span></span>
...
### <a name="step-5-start-the-bits-job"></a><span data-ttu-id="1d506-123">Passaggio 5: avviare il processo BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-123">Step 5: Start the BITS job</span></span>

<span data-ttu-id="1d506-124">Dopo aver configurato il processo BITS, chiamare la funzione [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) dell'interfaccia [**Metodo ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) per avviare o continuare il download.</span><span class="sxs-lookup"><span data-stu-id="1d506-124">After setting up the BITS job, call the [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) interface's [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) function to start or continue the download.</span></span>
...
### <a name="step-6-monitor-and-display-the-bits-jobs-progress"></a><span data-ttu-id="1d506-125">Passaggio 6: monitorare e visualizzare lo stato del processo BITS</span><span class="sxs-lookup"><span data-stu-id="1d506-125">Step 6: Monitor and display the BITS job's progress</span></span>

<span data-ttu-id="1d506-126">La `MonitorJobProgress` funzione helper accetta un oggetto [**Metodo ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) come unico parametro e esegue il polling del processo per uno stato ogni 500 millisecondi.</span><span class="sxs-lookup"><span data-stu-id="1d506-126">The `MonitorJobProgress` helper function takes an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object as its only parameter and polls the job for a status every 500 milliseconds.</span></span> <span data-ttu-id="1d506-127">Questa funzione non viene restituita finché il processo non è stato completato o non è stato annullato.</span><span class="sxs-lookup"><span data-stu-id="1d506-127">This function does not return until the job has completed or been canceled.</span></span>
...
### <a name="step-7-display-the-downloaded-file-headers"></a><span data-ttu-id="1d506-128">Passaggio 7: visualizzare le intestazioni dei file scaricati</span><span class="sxs-lookup"><span data-stu-id="1d506-128">Step 7: Display the downloaded file headers</span></span>

<span data-ttu-id="1d506-129">La `DisplayFileHeaders` funzione helper enumera i processi definiti per un oggetto [**Metodo ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) .</span><span class="sxs-lookup"><span data-stu-id="1d506-129">The `DisplayFileHeaders` helper function enumerates the jobs defined for an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object.</span></span>
...
## <a name="example"></a><span data-ttu-id="1d506-130">Esempio</span><span class="sxs-lookup"><span data-stu-id="1d506-130">Example</span></span>

<span data-ttu-id="1d506-131">L'esempio di codice seguente è un'applicazione console completamente funzionante che Mostra come usare l'API BITS per creare un processo BITS con più URL da scaricare, elencare gli URL in un processo e recuperare le intestazioni di risposta HTTP per ogni URL.</span><span class="sxs-lookup"><span data-stu-id="1d506-131">The following code example is a fully working console application that shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>
