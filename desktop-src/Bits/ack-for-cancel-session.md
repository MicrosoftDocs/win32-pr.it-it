...
---
# <a name="ack-for-cancel-session"></a>ACK per Cancel-Session

Usare **ACK per il pacchetto Annulla sessione** per confermare la richiesta di [**annullamento della sessione**](cancel-session.md) del client. Il server invia il riconoscimento dopo il rilascio di tutte le risorse associate alla sessione di caricamento.
...
GUID di stringa che identifica la sessione per il client. Sostituire {GUID} con l'identificatore di sessione inviato dal client nel pacchetto di richiesta di [**annullamento della sessione**](cancel-session.md) . Se non si riconosce l'identificatore di sessione, impostare l'intestazione BITS-Error-Code su BG \_ E la \_ sessione \_ non \_ trovata.
