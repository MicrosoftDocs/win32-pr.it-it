---
description: La classe CIM \_ USBDevice rappresenta le caratteristiche di gestione di un dispositivo USB.
ms.assetid: 0ff39701-826a-434b-a628-0af586600a80
ms.tgt_platform: multiple
title: CIM_USBDevice classe (provider WMI CIMWin32)
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119918801"
---
# <a name="cim_usbdevice-class-cimwin32-wmi-providers"></a>CIM_USBDevice classe (provider WMI CIMWin32)

La **classe CIM \_ USBDevice** rappresenta le caratteristiche di gestione di un dispositivo USB.
...
| Metodo                                                               | Descrizione                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbdevice.md) | Restituisce il descrittore del dispositivo USB. Non implementato da WMI.<br/>                                                                    |
| [**Reset**](reset-method-in-class-cim-usbdevice.md)                 | Richiede una reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbdevice.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
Impostazioni usb alternative per ogni interfaccia nella configurazione attualmente selezionata (indicata dalla **proprietà CurrentConfigValue).** Questa matrice include una voce per ogni interfaccia nella configurazione. Se la **proprietà CurrentConfigValue** ha un valore pari a 0 (zero), che indica che il dispositivo non è configurato, la matrice non è definita. Per altre informazioni sull'analisi di questa stringa dell'ottetto, vedere la specifica USB.
...
Configurazione attualmente selezionata per il dispositivo. Se il valore è 0 (zero), il dispositivo non è configurato.
...
Versione USB più recente supportata dal dispositivo USB. Questa proprietà è espressa come bcd (binary-coded decimal) in cui un separatore decimale è implicito tra la seconda e la terza cifra. Ad esempio, un valore 0x201 indica che è supportata la versione 2.01.

</dd> </dl>

## <a name="remarks"></a>Commenti

La **classe \_ CIM USBDevice** è derivata da [**CIM \_ LogicalDevice.**](cim-logicaldevice.md)

WMI non implementa questa classe. Per le classi WMI che **implementano CIM \_ USBDevice,** vedere [Classi Win32.](win32-provider.md)
