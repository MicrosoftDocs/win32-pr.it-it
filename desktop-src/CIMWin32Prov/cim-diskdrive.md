...
La classe **CIM \_ DiskDrive** presenta questi tipi di membri:
...
La classe **CIM \_ DiskDrive** presenta questi metodi.



| Metodo                                                                | Descrizione                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-cdromdrive.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
...
La classe **CIM \_ DiskDrive** dispone di queste proprietà.
...
Il dispositivo è noto in modalità di risparmio energia, ma lo stato esatto in questa modalità è sconosciuto.
...
Il dispositivo è in uno stato di avviso e anche in modalità di risparmio energia.
...
Funzionalità del dispositivo di accesso multimediale. Questa proprietà viene ereditata da [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).
...
Distingue un dispositivo che può accedere a entrambi i lati del supporto a doppio lato da un dispositivo che legge solo un singolo lato e richiede che il file multimediale venga riattivato.
...
Indica che non è necessario che il supporto venga espulso in modo esplicito dal dispositivo prima dell'accesso da parte di un elemento di selezione.
...
Matrice di stringhe in formato libero che fornisce spiegazioni dettagliate per le funzionalità di accesso ai dispositivi indicate nell'array delle **funzionalità** . Questa proprietà viene ereditata da [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).
...
Nome della classe (o sottoclasse) utilizzata per la creazione di un'istanza di. Se utilizzata con altre proprietà chiave della classe, questa proprietà consente di identificare in modo univoco tutte le istanze della classe e le relative sottoclassi.
...
Se **true**, l'errore riportato nella proprietà **LastErrorCode** è deselezionato.
...
Stringa in formato libero che descrive il tipo di rilevamento e correzione degli errori supportato dal dispositivo.
...
Dimensioni massime, in kilobyte, dei supporti supportati dal dispositivo. I kilobyte vengono interpretati come il numero di byte moltiplicato per 1000 (non per il numero di byte moltiplicato per 1024).
...
Se **true**, il dispositivo di accesso multimediale deve essere pulito. Se la pulizia manuale o automatica è possibile, è indicata nella proprietà Array delle **funzionalità** .
...
Quando il dispositivo di accesso multimediale supporta più supporti singoli, questa proprietà definisce il numero massimo che può essere supportato o inserito.
...
Il metodo [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) può essere richiamato con il parametro *PowerState* impostato su 5 ("ciclo di alimentazione") e il parametro *ora* impostato su una data e un'ora specifiche, o su un intervallo, per l'accensione.
...
WMI non implementa questa classe. Vedere [le classi Win32](win32-provider.md) per le classi derivate da **CIM \_ DiskDrive**.
