---
description: La classe CIM \_ DiskDrive rappresenta un'unità disco fisica, come illustrato dal sistema operativo.
ms.assetid: 3a63506e-455e-4108-b0c7-03b2af249d61
ms.tgt_platform: multiple
title: CIM_DiskDrive classe (provider WMI CIMWin32)
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
La **classe CIM \_ DiskDrive** ha questi tipi di membri:
...
La **classe CIM \_ DiskDrive** dispone di questi metodi.
...
La **classe CIM \_ DiskDrive** ha queste proprietà.
...
Esecuzione/alimentazione completa.
...
Il dispositivo è noto per essere in modalità risparmio energia, ma lo stato esatto in questa modalità è sconosciuto.
...
Il dispositivo è in uno stato di avviso e anche in modalità risparmio energia.
...
Funzionalità del dispositivo di accesso ai supporti. Questa proprietà viene ereditata da [**CIM \_ MediaAccessDevice.**](cim-mediaaccessdevice.md)
...
Matrice di stringhe in formato libero che forniscono spiegazioni dettagliate per l'accesso alle funzionalità del dispositivo indicate nella matrice **Capabilities.** Questa proprietà viene ereditata da [**CIM \_ MediaAccessDevice.**](cim-mediaaccessdevice.md)
...
Se **TRUE,** l'errore segnalato nella **proprietà LastErrorCode** viene cancellato.
...
Stringa in formato libero che descrive il tipo di rilevamento e correzione degli errori supportati dal dispositivo.
...
Dimensioni massime, in kilobyte, dei supporti supportati dal dispositivo. I kilobyte vengono interpretati come il numero di byte moltiplicati per 1000 (non il numero di byte moltiplicati per 1024).
...
Quando il dispositivo di accesso ai supporti supporta più supporti singoli, questa proprietà definisce il numero massimo che può essere supportato o inserito.
...
Il [**metodo SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) è supportato.
...
Il [**metodo SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) può essere richiamato con il *parametro PowerState* impostato su 5 ("Ciclo di alimentazione").
...
Il [**metodo SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) può essere richiamato con il parametro *PowerState* impostato su 5 ("Ciclo di alimentazione") e il parametro *Time* impostato su una data e un'ora o un intervallo specifico per l'accensione.
...
La **classe CIM \_ DiskDrive** è derivata da [**CIM \_ MediaAccessDevice.**](cim-mediaaccessdevice.md)

WMI non implementa questa classe. Vedere [Classi Win32 per](win32-provider.md) le classi derivate da **CIM \_ DiskDrive**.
