---
description: Posiziona il servizio, rappresentato dall'oggetto Servizio Win32, \_ nello stato arrestato.
ms.assetid: cc2c71f7-12e6-4ba4-bfb4-f23845d798b5
ms.tgt_platform: multiple
title: Metodo StopService della Win32_Service classe (Sdoias.h)
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119751881"
---
# <a name="stopservice-method-of-the-win32_service-class-sdoiash"></a>Metodo StopService della Win32_Service classe (Sdoias.h)

Il metodo della [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **StopService** posiziona il servizio, rappresentato dall'oggetto [**\_ Servizio Win32,**](win32-service.md) nello stato arrestato.
...
Dopo aver determinato quali servizi possono essere arrestati o sospesi, è possibile usare i **metodi StopService** e [**PauseService**](pauseservice-method-in-class-win32-systemdriver.md) per arrestare e sospendere i servizi. La decisione di arrestare un servizio anziché sospenderlo o viceversa dipende da diversi fattori, tra cui:
...
Se si tenta di arrestare un servizio in cui sono in esecuzione servizi dipendenti, il **metodo StopService** ha esito negativo e viene restituito il valore 3. I servizi dipendenti devono essere arrestati per primi.

Se si arresta un servizio, controllare immediatamente il [**servizio Win32 \_**](win32-service.md).**Proprietà** State, poiché il valore può comunque mostrare il servizio come in esecuzione.

## <a name="examples"></a>Esempio

[Set-RemoteService](https://Gallery.TechNet.Microsoft.Com/79595ce9-bfc3-463e-9e84-d9e0b78590c1) Esempio di PowerShell Imposta lo stato del servizio per i computer remoti.

[L'esempio VBScript Arresta un](https://Gallery.TechNet.Microsoft.Com/ae07e623-2cbd-4983-b991-aa4d1e6e2732) servizio e i relativi dipendenti arresta un servizio e tutti i servizi dipendenti.
...
L'esempio di codice Perl seguente illustra come arrestare un servizio.
...
L'esempio di codice VBScript seguente mostra che non è possibile arrestare il servizio NetDDE fino a quando i servizi dipendenti non sono stati arrestati. Per eseguire lo script, verificare che il servizio NetDDE e i relativi servizi dipendenti siano in esecuzione usando lo snap-in MMC Services.msc o il **comando Net Start.**

La [**classe \_ DependentService Win32**](win32-dependentservice.md) consente di individuare le dipendenze del servizio tramite una query [Associators Of.](/windows/desktop/WmiSdk/associators-of-statement)
