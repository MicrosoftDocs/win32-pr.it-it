---
description: Il metodo statico della classe WMI SetNumForwardPackets viene usato per impostare il numero di intestazioni di pacchetti IP allocate per la coda di pacchetti del router. Quando tutte le intestazioni sono in uso, il router inizierà a rimuovere i pacchetti dalla coda in modo casuale.
...
ms.locfileid: "119759751"
---
# <a name="setnumforwardpackets-method-of-the-win32_networkadapterconfiguration-class"></a>Metodo SetNumForwardPackets della classe NetworkAdapterConfiguration Win32 \_

Il metodo statico della classe [WMI](/windows/desktop/WmiSdk/retrieving-a-class) **SetNumForwardPackets** viene usato per impostare il numero di intestazioni di pacchetti IP allocate per la coda di pacchetti del router. Quando tutte le intestazioni sono in uso, il router inizierà a rimuovere i pacchetti dalla coda in modo casuale.
...
*NumForwardPackets* \[ Pollici\]
</dt> <dd>

Numero di intestazioni di pacchetti IP allocate per la coda di pacchetti del router. Deve essere grande almeno quanto il valore della proprietà [**ForwardBufferMemory**](win32-networkadapterconfiguration.md) diviso per le dimensioni massime dei dati IP delle reti connesse al router. Non deve essere maggiore del valore **ForwardBufferMemory** diviso per 256, poiché ogni pacchetto richiede almeno 256 byte di memoria del buffer di inoltro. Il numero ottimale di pacchetti di inoltro per una determinata dimensione **ForwardBufferMemory** dipende dal tipo di traffico trasportato nella rete e sarà compreso tra questi due valori. Se il router è disabilitato, questo parametro viene ignorato e non vengono allocate intestazioni. Intervallo valido: 1 - 0xFFFFFFFE.
...
L'esempio VBScript Modify [the Number of Allowed Forward Packets](https://Gallery.TechNet.Microsoft.Com/4123c28e-25c4-444e-818b-67bdbcc0cd4c) (Modifica il numero di pacchetti di inoltro consentiti) configura il numero di pacchetti di inoltro allocati alla coda di pacchetti del router.
