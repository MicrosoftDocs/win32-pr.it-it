...
---
# <a name="win32_pnpentity-class"></a>Win32 \_ PnPEntity (classe)

La [classe WMI](../wmisdk/retrieving-a-class.md) **\_ PnPEntity Win32** rappresenta le proprietà di un dispositivo Plug and Play. Plug and Play entità vengono visualizzate come voci nell'Gestione dispositivi disponibile nel pannello di controllo.
...
La classe **Win32 \_ PnPEntity** presenta questi tipi di membri:
...
La classe **Win32 \_ PnPEntity** presenta questi metodi.



| Metodo                                                             | Descrizione                                                                                                                                                                                    |
|:-------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**Disabilita**](disable-win32-pnpentity.md)                         | Disabilita questo dispositivo Plug and Play.<br/>                                                                                                                                                 |
| [**Abilitare**](enable-win32-pnpentity.md)                           | Abilita questa Plug and Play dispositivo.<br/>                                                                                                                                                  |
| [**GetDeviceProperties**](getdeviceproperties-win32-pnpentity.md) | Ottiene le proprietà specificate del dispositivo Plug and Play.<br/>                                                                                                                         |
| **Reimpostazione**                                                          | Non implementato. Per implementare questo metodo, vedere il metodo [**Reset**](reset-method-in-class-cim-controller.md) in [**CIM \_ LogicalDevice**](cim-logicaldevice.md).<br/>                 |
| **SetPowerState**                                                  | Non implementato. Per implementare questo metodo, vedere il metodo [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) in [**CIM \_ LogicalDevice**](cim-logicaldevice.md).<br/> |
...
La classe **Win32 \_ PnPEntity** dispone di queste proprietà.
...
Identificatore univoco globale (GUID) di questo dispositivo Plug and Play.
...
Una stringa di identificazione definita dal fornitore che viene utilizzata dal programma di installazione per associare un dispositivo a un file INF. In genere, a un dispositivo è associato un elenco di ID hardware. Un'eccezione è rappresentata dal driver del bus 1394, che non utilizza gli ID hardware. Il primo ID hardware nell'elenco deve essere l'ID del dispositivo. Gli ID rimanenti devono essere elencati in ordine di idoneità decrescente.

Gli ID hardware vengono visualizzati in uno dei formati seguenti:

-   specifico enumeratore enumeratore- \\ Device-ID
...
-   \*ID specifico dell'enumeratore
    ...
    Un asterisco ( \* ) indica l'utilizzo da parte di più enumeratori.

-   ID specifico del dispositivo

    Formato personalizzato.

Esempi di ID hardware sono:

<dl> <dd>\\ \* PNPOF08 radice</dd> <dd>PC \\ VEN \_ 1000&DEV \_ 001&SUBSYS \_ 00000000&rev \_ 02</dd> </dl>

Per ulteriori informazioni, vedere [Windows Driver Kit](https://developer.microsoft.com/windows/hardware/download-kits-windows-hardware-development).
...
> [!WARNING]
> Questa proprietà, nonostante sia elencata nel file MOF, non esiste effettivamente nella classe. La proprietà viene descritta solo per motivi di completezza e per chiarire il file MOF.

 

Nome del tipo di questo dispositivo Plug and Play.

**Windows server 2012 R2, Windows 8.1, Windows server 2012, Windows 8, Windows server 2008 R2, Windows 7, Windows server 2008 e Windows Vista:** Questa proprietà non è presente nel file MOF.
...
Il metodo **SetPowerState** è supportato. Questo metodo è disponibile nella classe [**\_ LogicalDevice CIM**](cim-logicaldevice.md) padre e può essere implementato. Per ulteriori informazioni, vedere [progettazione di classi Managed Object Format (MOF)](../wmisdk/designing-managed-object-format--mof--classes.md).
...
Indica se il dispositivo Plug and Play è attualmente nel sistema.
...
Nome del servizio che supporta questo dispositivo Plug and Play. Per ulteriori informazioni, vedere [**Win32 \_ SystemDriverPnPEntity**](win32-systemdriverpnpentity.md).
...
La classe **Win32 \_ PnPEntity** è derivata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

## <a name="examples"></a>Esempio

Nell'esempio [Get-BrokenHardware.ps1](https://Gallery.TechNet.Microsoft.Com/dbb678f4-b95b-45c3-bc8b-2ae2d052448e) PowerShell della raccolta TechNet viene usato **per \_ PnPEntity Win32** per recuperare un elenco di hardware non funzionante con WMI.

L'esempio di codice VBScript seguente consente di connettersi a un gruppo di computer remoti nello stesso dominio creando una matrice di nomi di computer remoti e visualizzando quindi i nomi dei dispositivi Plug and Play, ovvero le istanze di Win32 \_ PnPEntity, in ogni computer.
...
[Connessione a WMI in un computer remoto](../wmisdk/connecting-to-wmi-on-a-remote-computer.md)
</dt> <dt>
