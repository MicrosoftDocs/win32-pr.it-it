...
---
# <a name="cim_system-class-cimwin32-wmi-providers"></a>Classe CIM_System (provider WMI CIMWin32)

La classe di **\_ sistema CIM** aggrega un set enumerabile di elementi di sistema gestiti. L'aggregazione funziona come un intero funzionale. All'interno di una sottoclasse particolare del sistema è presente un elenco ben definito di classi di elementi di sistema gestite le cui istanze devono essere aggregate.

L'oggetto di **\_ sistema CIM** e i relativi derivati sono oggetti di livello superiore di CIM. Forniscono l'ambito per numerosi componenti e devono avere chiavi di sistema univoche.
...
La classe di **\_ sistema CIM** è derivata da [**CIM \_ LogicalElement**](cim-logicalelement.md).

WMI non implementa questa classe. Per le classi WMI derivate dal **\_ sistema CIM**, vedere [classi Win32](win32-provider.md).
