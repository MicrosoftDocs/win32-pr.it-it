...
---
# <a name="cim_copyfileaction-class"></a>CIM \_ CopyFileAction (classe)

La classe **CIM \_ CopyFileAction** rappresenta lo spostamento o la copia di file da un sistema di computer in una nuova posizione.

Le informazioni sulla posizione per la copia vengono specificate tramite le associazioni CIM [**\_ ToDirectorySpecification**](cim-todirectoryspecification.md) e [**CIM \_ FromDirectorySpecification**](cim-fromdirectoryspecification.md) oppure le associazioni CIM [**\_ ToDirectoryAction**](cim-todirectoryaction.md) e [**CIM \_ FromDirectoryAction**](cim-fromdirectoryaction.md) . Il primo set viene usato quando l'origine o la destinazione devono esistere prima di intraprendere qualsiasi azione. Il secondo set viene usato quando l'origine o la destinazione vengono create come parte di un'azione precedente. Nel secondo caso, l'azione per creare la directory deve essere eseguita prima dell'oggetto **CIM \_ CopyFileAction** .
...
La classe **CIM \_ CopyFileAction** presenta questi tipi di membri:
...
La classe **CIM \_ CopyFileAction** presenta questi metodi.



| Metodo                                                      | Descrizione                                                                                                                                  |
|:------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------|
| [**Richiamare**](invoke-method-in-class-cim-copyfileaction.md) | Esegue una determinata azione. I dettagli del modo in cui il metodo esegue l'azione sono specifici dell'implementazione. Non implementato da WMI.<br/> |
...
La classe **CIM \_ CopyFileAction** dispone di queste proprietà.
...
Questa proprietà viene ereditata [**dall' \_ azione CIM**](cim-action.md).
...
Se **true**, il file di origine viene eliminato dopo l'operazione di copia.
...
Nome completo del file di destinazione.
...
Indica se un particolare [**oggetto \_ azione CIM**](cim-action.md) fa parte di una sequenza di azioni per la transizione dell'elemento software corrente allo stato successivo, ad esempio "Install", o per rimuovere l'elemento software corrente, ad esempio "uninstall".
...
Nome completo del file di origine.
...
La classe **CIM \_ CopyFileAction** è derivata da [**\_ fileaction CIM**](cim-fileaction.md).

WMI non implementa questa classe. Per ulteriori informazioni sulle classi derivate da **CIM \_ CopyFileAction**, vedere [Win32 Classes](win32-provider.md).
