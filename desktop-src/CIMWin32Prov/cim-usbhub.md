...
---
# <a name="cim_usbhub-class"></a>CIM \_ USBHub (classe)

La classe **CIM \_ USBHub** rappresenta le funzionalità e la gestione di un hub USB.
...
La classe **CIM \_ USBHub** presenta questi tipi di membri:
...
La classe **CIM \_ USBHub** presenta questi metodi.



| Metodo                                                            | Descrizione                                                                                                                              |
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbhub.md) | Restituisce il descrittore del dispositivo USB. Non implementato da WMI.<br/>                                                                    |
| [**Reset**](reset-method-in-class-cim-usbhub.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ USBHub** dispone di queste proprietà.
...
Questa proprietà viene ereditata da [**CIM \_ usbdevice**](cim-usbdevice.md).
...
Qualificatori: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ usbdevice**](cim-usbdevice.md).**CurrentConfigValue**")
</dt> </dl>

Impostazioni alternative USB per ogni interfaccia della configurazione attualmente selezionata (indicata dalla proprietà **CurrentConfigValue** ). Questa matrice include una voce per ogni interfaccia della configurazione. Se la proprietà **CurrentConfigValue** ha un valore pari a 0 (zero), che indica che il dispositivo non è configurato, la matrice non è definita. Per ulteriori informazioni sull'analisi di questa stringa ottetto, vedere la specifica USB.
...
Qualificatori: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ usbdevice**](cim-usbdevice.md).**CurrentAlternateSettings**")
</dt> </dl>

Configurazione attualmente selezionata per il dispositivo. Se questo valore è 0 (zero), il dispositivo non è configurato.
...
Se **true**, Power viene attivato contemporaneamente a tutte le porte nell'hub. Se **false**, Power viene attivato singolarmente per ogni porta.
...
Numero di porte downstream nell'hub, incluse quelle incorporate nel silicio dell'hub.
...
Versione USB più recente supportata dal dispositivo USB. Questa proprietà viene espressa come un separatore decimale (BCD) con codifica binaria in cui un separatore decimale è implicito tra la seconda e la terza cifra. Ad esempio, il valore 0x201 indica che la versione 2,01 è supportata.
...
La classe **CIM \_ USBHub** è derivata da [**CIM \_ usbdevice**](cim-usbdevice.md).

WMI non implementa questa classe. Per le classi WMI derivate da **CIM \_ USBHub**, vedere [Win32 Classes](win32-provider.md).
