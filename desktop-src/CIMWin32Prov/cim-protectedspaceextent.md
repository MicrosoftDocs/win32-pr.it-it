...
---
# <a name="cim_protectedspaceextent-class"></a>CIM \_ ProtectedSpaceExtent (classe)

La classe **CIM \_ ProtectedSpaceExtent** rappresenta indirizzi di blocco logico indirizzabili, che vengono trattati come un singolo extent di archiviazione, ma si trovano in un singolo extent fisico. Gli extent dello spazio protetto escludono tutti i blocchi logici mappati come dati di controllo e contengono informazioni di mapping della profondità di striping dei dati utente. Una possibilità alternativa, se si usa la configurazione automatica, consiste nell'creare un'istanza o estendere la classe [**CIM \_ AggregatePSExtent**](cim-aggregatepsextent.md) .
...
La classe **CIM \_ ProtectedSpaceExtent** presenta questi tipi di membri:
...
La classe **CIM \_ ProtectedSpaceExtent** presenta questi metodi.



| Metodo                                                                          | Descrizione                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-protectedspaceextent.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ ProtectedSpaceExtent** dispone di queste proprietà.
...
Dimensioni del blocco, in byte, dei blocchi che formano l'extent di archiviazione. Se la dimensione del blocco di variabili è, è necessario specificare la dimensione massima del blocco in byte. Se le dimensioni del blocco sono sconosciute o se un concetto di blocco non è valido, ad esempio per extent di aggregazione, memoria o dischi logici, immettere 1 (uno).
...
Numero totale di blocchi consecutivi, ciascuno dei quali blocca la dimensione del valore contenuto nella proprietà **blockSize** , che costituisce l'extent di archiviazione. Le dimensioni totali dell'extent di archiviazione possono essere calcolate moltiplicando il valore della proprietà **blockSize** in base al valore di questa proprietà. Se il valore di **blockSize** è 1 (uno), questa proprietà corrisponde alla dimensione totale dell'extent di archiviazione.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Extent dello spazio protetto DMTF \| 001,6 "), [**unità**](/windows/desktop/WmiSdk/standard-qualifiers) (" byte ")
</dt> </dl>

Per una classe **CIM \_ ProtectedSpaceExtent** allocata a una classe CIM del set di [**\_ volumi**](cim-volumeset.md) , questa proprietà è il numero di byte di dati utente posizionati nell'extent dello spazio protetto prima di procedere al successivo extent dello spazio protetto nel set di volumi. In caso contrario, questo extent dello spazio protetto viene considerato non allocato e questa proprietà deve essere impostata su 0.
...
La classe **CIM \_ ProtectedSpaceExtent** è derivata da [**CIM \_ StorageExtent**](cim-storageextent.md).
