...
---
# <a name="cim_battery-class"></a>\_Classe CIM Battery

La classe **CIM \_ batteria** rappresenta le funzionalità e la gestione del dispositivo logico della batteria. Questa classe si applica alle batterie nei sistemi portatili e altre batterie interne ed esterne.
...
La classe **CIM \_ Battery** presenta questi tipi di membri:
...
La classe **CIM \_ Battery** presenta questi metodi.



| Metodo                                                             | Descrizione                                                                                                                                |
|:-------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-battery.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                                 |
| [**SetPowerState**](setpowerstate-method-in-class-cim-battery.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando il dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ Battery** dispone di queste proprietà.
...
Descrizione dello stato di carica della batteria. Il valore 10 non è valido nello schema CIM, che non rappresenta la batteria installata in Desktop Management Interface (DMI). In questo caso, non è necessario creare un'istanza dell'oggetto.
...
<span id="Lead_Acid"></span><span id="lead_acid"></span><span id="LEAD_ACID"></span>**Acido di piombo** (3)
...
<span id="Nickel_Cadmium"></span><span id="nickel_cadmium"></span><span id="NICKEL_CADMIUM"></span>**Nichel cadmio** (4)
...
<span id="Nickel_Metal_Hydride"></span><span id="nickel_metal_hydride"></span><span id="NICKEL_METAL_HYDRIDE"></span>**Idruro Nickel Metal** (5)
...
<span id="Zinc_air"></span><span id="zinc_air"></span><span id="ZINC_AIR"></span>**Zinco aria** (7)
...
<span id="Lithium_Polymer"></span><span id="lithium_polymer"></span><span id="LITHIUM_POLYMER"></span>**Polimero Lithium** (8)
...
Capacità progettata della batteria in milliwatt ore. Se questa proprietà non è supportata, immettere 0.
...
Percentuale stimata del costo totale rimanente.
...
Tempo stimato, in minuti, fino a quando non viene esaurito il costo della batteria sotto le condizioni di carico attuali se l'alimentazione dell'utilità è disattivata, viene persa e rimane spenta o se un portatile è disconnesso da una fonte di alimentazione.
...
Durata prevista della batteria, in minuti, presupponendo che la batteria venga addebitata completamente. Questa proprietà rappresenta la durata totale prevista della batteria, non la durata rimanente corrente, indicata dalla proprietà **EstimatedRunTime** .
...
La capacità di addebito completo della batteria in milliwatt ore. Confrontare questo valore con la proprietà **DesignCapacity** per determinare quando la batteria deve essere sostituita. La durata della batteria è in genere quando la proprietà **FullChargeCapacity** è inferiore al 80% della proprietà **DesignCapacity** . Se questa proprietà non è supportata, immettere 0.
...
Tempo massimo, in minuti, per l'addebito completo della batteria. Questa proprietà rappresenta il tempo per ricaricare una batteria completamente esaurita, non il tempo di ricarica rimanente corrente, indicato nella proprietà **TimeToFullCharge** .
...
Indica le funzionalità specifiche relative al risparmio di energia del dispositivo logico.
...
Le capacità relative al risparmio di energia sono sconosciute.
...
Il metodo **SetPowerState** può essere richiamato con il parametro *PowerState* impostato su 5 ("ciclo di alimentazione") e il parametro *ora* impostato su una data e un'ora specifiche, o su un intervallo, per l'accensione.
...
Smart Battery data-specifica il numero di versione supportato da questa batteria. Se la batteria non supporta questa funzione, il valore deve essere lasciato vuoto.
...
Tempo trascorso, in secondi, dal momento in cui l'UPS del sistema informatico ha comportato l'ultima alimentazione a batteria o la quantità di tempo trascorsa dall'ultimo riavvio del sistema o dell'UPS, a seconda del valore minore. Se la batteria è "online", viene restituito il valore 0.
...
Tempo rimanente, espresso in minuti, per addebitare la batteria in modo completo al tasso di ricarica corrente e usare.

</dd> </dl>

## <a name="remarks"></a>Commenti

La classe **CIM \_ Battery** è derivata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

WMI non implementa questa classe. Per ulteriori informazioni sulle classi derivate dalla **\_ batteria CIM**, vedere la pagina relativa alle [classi Win32](win32-provider.md).
