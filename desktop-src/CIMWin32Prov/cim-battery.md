---
description: La classe CiM \_ Battery rappresenta le funzionalità e la gestione del dispositivo logico a batteria. Questa classe si applica alle batterie nei sistemi portatili e ad altre batterie interne ed esterne.
ms.assetid: af127b7a-021b-4cd8-af1b-176aff760858
ms.tgt_platform: multiple
title: CIM_Battery classe
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119080905"
---
# <a name="cim_battery-class"></a>Classe \_ CiM Battery

La **classe CiM \_ Battery** rappresenta le funzionalità e la gestione del dispositivo logico a batteria. Questa classe si applica alle batterie nei sistemi portatili e ad altre batterie interne ed esterne.
...
La **classe \_ CiM Battery** ha questi tipi di membri:
...
La **classe \_ CiM Battery** include questi metodi.



| Metodo                                                             | Descrizione                                                                                                                                |
|:-------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-battery.md)                 | Richiede una reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                                 |
...
La **classe \_ CiM Battery** ha queste proprietà.
...
Descrizione dello stato di carica della batteria. Il valore 10 non è valido nello schema CIM, che rappresenta nessuna batteria installata in Desktop Management Interface (DMI). In questo caso, non è necessario creare un'istanza dell'oggetto.
...
<span id="Charging_and_High"></span><span id="charging_and_high"></span><span id="CHARGING_AND_HIGH"></span>**Addebito e alta** (7)
...
<span id="Charging_and_Low"></span><span id="charging_and_low"></span><span id="CHARGING_AND_LOW"></span>**Addebito e bassa** (8)
...
<span id="Charging_and_Critical"></span><span id="charging_and_critical"></span><span id="CHARGING_AND_CRITICAL"></span>**Addebito e criticità** (9)


</dt> <dd>

Addebito e criticità.
...
<span id="Nickel_Cadmium"></span><span id="nickel_cadmium"></span><span id="NICKEL_CADMIUM"></span>**Nickel Cadmium** (4)
...
<span id="Nickel_Metal_Hydride"></span><span id="nickel_metal_hydride"></span><span id="NICKEL_METAL_HYDRIDE"></span>**Nickel Metal Hydride** (5)


</dt> <dd>

Iridetto di nichel metal.

</dd> <dt>

<span id="Lithium-ion"></span><span id="lithium-ion"></span><span id="LITHIUM-ION"></span>

<span id="Lithium-ion"></span><span id="lithium-ion"></span><span id="LITHIUM-ION"></span>**Ioni di litio** (6)
...
<span id="Zinc_air"></span><span id="zinc_air"></span><span id="ZINC_AIR"></span>**Aria di zinco** (7)
...
<span id="Lithium_Polymer"></span><span id="lithium_polymer"></span><span id="LITHIUM_POLYMER"></span>**Lithium Polymer** (8)
...
**Questo dispositivo non può essere avviato.** (10)
...
**Windows verificare le risorse del dispositivo.** (13)
...
**Questo dispositivo non funziona correttamente perché è probabile che si sia verificato un problema di rienumere.** (15)
...
**Windows identificare tutte le risorse utilizzate dal dispositivo.** (16)
...
**Questo dispositivo non è presente, non funziona correttamente o non dispone di tutti i relativi driver installati.** (24)
...
**Questo dispositivo usa una risorsa richiesta di interruzione (IRQ) in uso da un altro dispositivo.** (30)
...
**Questo dispositivo non funziona correttamente perché non Windows caricare i driver necessari per questo dispositivo.** (31)
...
Tempo stimato, in minuti, fino a quando la carica della batteria non viene esaurita in condizioni di carico attuali se l'alimentazione dell'utilità è spenta, viene persa e rimane spenta o se un portatile è disconnesso da una fonte di alimentazione.
...
Capacità di carica completa della batteria in milliwatt-hours. Confrontare questo valore con la **proprietà DesignCapacity** per determinare quando la batteria richiede la sostituzione. La durata di una batteria si verifica in genere quando la **proprietà FullChargeCapacity** scende al di sotto dell'80% della **proprietà DesignCapacity.** Se questa proprietà non è supportata, immettere 0.
...
Tempo massimo, in minuti, per caricare completamente la batteria. Questa proprietà rappresenta il tempo necessario per caricare una batteria completamente esaurita, non il tempo di ricarica rimanente corrente, indicato nella **proprietà TimeToFullCharge.**
...
Numero di versione della specifica dei dati della batteria intelligente supportata da questa batteria. Se la batteria non supporta questa funzione, il valore deve essere lasciato vuoto.
...
Tempo trascorso, in secondi, dall'ultimo passaggio del gruppo di sicurezza del computer all'alimentazione a batteria o dalla quantità di tempo trascorsa dall'ultimo riavvio del sistema o del gruppo di sicurezza, a seconda del valore minore. Se la batteria è "online", viene restituito il valore 0.
...
Tempo rimanente, in minuti, per caricare completamente la batteria alla tariffa di ricarica e all'uso correnti.
...
WMI non implementa questa classe. Per altre informazioni sulle classi derivate dalla **batteria CIM, \_** vedere [Classi Win32.](win32-provider.md)
