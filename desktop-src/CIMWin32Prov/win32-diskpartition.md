...
---
# <a name="win32_diskpartition-class"></a>Win32 \_ DiskPartition (classe)

La [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ DiskPartition Win32** rappresenta le funzionalità e la capacità di gestione di un'area partizionata di un disco fisico in un sistema di computer che esegue Windows. Esempio: disco \# 0, partizione \# 1.
...
La classe **Win32 \_ DiskPartition** presenta questi tipi di membri:
...
La classe **Win32 \_ DiskPartition** presenta questi metodi.



| Metodo                                                     | Descrizione                                                                                                   |
|:-----------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------|
| [**Reset**](win32-diskpartition-reset.md)                 | Richiede la reimpostazione del dispositivo logico.<br/>                                                            |
| [**SetPowerState**](win32-diskpartition-setpowerstate.md) | Imposta lo stato di alimentazione desiderato per un dispositivo logico e quando deve essere inserito un dispositivo in tale stato.<br/> |
...
La classe **Win32 \_ DiskPartition** dispone di queste proprietà.
...
Disponibilità e stato aggiuntive del dispositivo, oltre a quello specificato nella proprietà Availability. La proprietà **Availability** indica lo stato primario e la disponibilità del dispositivo. In alcuni casi, non sarà sufficiente indicare lo stato completo del dispositivo. In questi casi, è possibile usare la proprietà **AdditionalAvailability** per fornire ulteriori informazioni. Ad esempio, la **disponibilità** primaria di un dispositivo potrebbe essere offline (valore = 8), ma potrebbe anche essere in uno stato di basso consumo (valore **AdditonalAvailability** = 14) oppure il dispositivo potrebbe eseguire la diagnostica (valore **AdditionalAvailability** = 5, in test) ".
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("Win32API \| file functions \| ReadFile")
</dt> </dl>

Partition è la partizione attiva. Il sistema operativo utilizza la partizione attiva quando si esegue l'avvio da un disco rigido.
...
Matrice di stringhe in formato libero che fornisce spiegazioni e dettagli dietro le voci della matrice OtherIdentifyingInfo. Si noti che ogni voce di questa matrice è correlata alla voce in OtherIdentifyingInfo che si trova nello stesso indice.
...
Qualificatori: **depricated**
</dt> </dl>

Tempo massimo in millisecondi durante il quale un dispositivo può essere eseguito in uno stato inattivo. Lo stato di un dispositivo viene definito nelle proprietà Availability e AdditionalAvailability, in cui inattivo viene trasmesso dal valore 21. Ciò che si verifica alla fine del limite di tempo è specifico del dispositivo. Il dispositivo può riattivare, può essere offline o eseguire altre azioni. Il valore 0 indica che un dispositivo può rimanere inattivo per un periodo illimitato.

> [!Note]
>
> "La proprietà MaxQuiesceTime è stata deprecata. Quando si valuta l'uso di mettere in stato, è stato determinato che questa singola proprietà non è adatta per la descrizione del momento in cui un dispositivo uscirà automaticamente da uno stato di riposo. In realtà, lo scenario più probabile per un dispositivo di uscire da uno stato di riposo è stato determinato in base al numero di richieste in attesa in coda anziché a un tempo massimo. Questa operazione verrà nuovamente valutata e riposizionata in un secondo momento. \\n
...
Matrice che acquisisce dati aggiuntivi, oltre alle informazioni di DeviceID, che possono essere usati per identificare un LogicalDevice. Un esempio è quello di contenere il nome descrittivo dell'utente del sistema operativo per il dispositivo in questa proprietà. La lunghezza massima è 256.
...
Indica le funzionalità specifiche relative al risparmio di energia del dispositivo logico. I valori della matrice, 0 = "Unknown", 1 = "not supported" e 2 = "disabled" sono autoesplicativi. Il valore 3 = "Enabled" indica che le funzionalità di risparmio energia sono attualmente abilitate, ma il set di funzionalità esatto è sconosciuto oppure le informazioni non sono disponibili. "Le modalità di risparmio energia immesse automaticamente" (4) descrivono che un dispositivo può modificare lo stato di alimentazione in base all'utilizzo o ad altri criteri. "Lo stato di alimentazione impostabile" (5) indica che il metodo SetPowerState è supportato. "Power Cycling supported" (6) indica che il metodo SetPowerState può essere richiamato con la variabile di input PowerState impostata su 5 ("ciclo di alimentazione"). "Tempo di accensione supportato" (7) indica che il metodo SetPowerState può essere richiamato con la variabile di input PowerState impostata su 5 ("ciclo di alimentazione") e il parametro ora impostato su una data e un'ora specifiche, o un intervallo, per l'accensione.
...
Il numero di ore consecutive in cui il dispositivo è stato alimentato, dall'ultimo ciclo di alimentazione.
...
Se **true**, si tratta della partizione primaria.
...
Descrizione del supporto e del relativo utilizzo.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("Win32API \| device input and output Structure \| [**\_ Information**](/windows/desktop/api/winioctl/ns-winioctl-partition_information) \| RewritePartition")
</dt> </dl>

Se **true**, le informazioni sulla partizione sono state modificate. Quando si modifica una partizione (con [**il \_ \_ layout dell' \_ unità \_ del set di dischi IOCTL**](/windows/desktop/api/winioctl/ni-winioctl-ioctl_disk_set_drive_layout)), il sistema utilizza questa proprietà per determinare quali partizioni sono state modificate ed è necessario riscrivere le relative informazioni. Se **true**, la partizione deve essere riscritta.
...
Il numero totale di ore in cui il dispositivo è stato alimentato.
...
<dl> <dd>Inutilizzati</dd> <dd>"FAT a 12 bit"</dd> <dd>"XENIX tipo 1"</dd> <dd>"XENIX Type 2"</dd> <dd>"FAT a 16 bit"</dd> <dd>"Partizione estesa"</dd> <dd>"MS-DOS V4 Huge"</dd> <dd>"File system installabile"</dd> <dd>"Piattaforma di riferimento PowerPC"</dd> <dd>UNIX</dd> <dd>NTFS</dd> <dd>"Win95 w/Extended int 13"</dd> <dd>"Extended w/Extended int 13"</dd> <dd>"Gestione dischi logici"</dd> <dd>Sconosciuto</dd> </dl>
...
**File system installabile** ("file system installabile")
...
**Piattaforma** di riferimento PowerPC ("piattaforma di riferimento PowerPC")
...
**Win95 w/int esteso 13** ("Win95 w/Extended int 13")
...
La classe **Win32 \_ DiskPartition** è derivata da [**CIM \_ DiskPartition**](cim-diskpartition.md).

Una partizione è una divisione strutturale di un'unità disco fisica. Sebbene un'unità possa contenere una singola partizione, i volumi più grandi vengono spesso divisi in più partizioni. Questo è il motivo per cui è possibile che siano presenti unità C, D ed e anche se il computer dispone di un solo disco rigido fisico.

Windows supporta i tipi di partizione seguenti:

-   Partizione primaria. Si tratta dell'unico tipo di partizione in cui è possibile installare un sistema operativo. Ogni unità può avere un massimo di quattro partizioni primarie, ognuna delle quali assegna una lettera di unità diversa.
-   Partizione estesa. Partizione aggiuntiva che può essere suddivisa in più unità logiche, ciascuna delle quali assegna una lettera di unità univoca. Un'unità può avere una sola partizione estesa. è tuttavia possibile dividere questa partizione in più unità logiche. Questo consente a un disco di avere più di quattro partizioni primarie consentite.
-   Partizione di sistema. Qualsiasi partizione primaria contenente un sistema operativo.

Le partizioni possono indicare il modo in cui viene effettivamente utilizzata un'unità disco fisica. Esaminando le partizioni fisiche su un disco, è possibile determinare i tipi di elementi seguenti:

-   Modalità di suddivisione del disco in unità logiche.
-   Se sul disco è disponibile spazio non partizionato. Questa operazione può essere determinata sottraendo le dimensioni di tutte le partizioni in un disco a partire dalle dimensioni del disco.
-   Se è possibile avviare il computer da tale disco (ovvero, il disco contiene una partizione di avvio).

Tutte queste domande possono essere risolte usando la classe **Win32 \_ DiskPartition** .

## <a name="examples"></a>Esempio

L'esempio di codice PowerShell seguente controlla l'allineamento dei dischi in un computer: se l'offset è frazionario, il disco non è allineato correttamente.
