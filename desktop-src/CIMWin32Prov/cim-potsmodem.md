...
---
# <a name="cim_potsmodem-class"></a>CIM \_ POTSModem (classe)

La classe **CIM \_ POTSModem** rappresenta un dispositivo che converte i dati binari in modulazioni Wave per la trasmissione basata su audio connettendosi alla rete Plain Old Phone System (POTS).
...
La classe **CIM \_ POTSModem** presenta questi tipi di membri:
...
La classe **CIM \_ POTSModem** presenta questi metodi.



| Metodo                                                               | Descrizione                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-potsmodem.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-potsmodem.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ POTSModem** dispone di queste proprietà.
...
Impostazione della risposta automatica o della chiamata per il modem corrente.
...
Paesi o aree geografiche in cui il modem può funzionare.
...
Paese o area geografica del modem attualmente programmato. Quando sono supportati più paesi o aree geografiche, questa proprietà definisce quello attualmente selezionato per l'utilizzo.
...
Password attualmente definite per il modem. Questa matrice può essere lasciata vuota per motivi di sicurezza.
...
Tipo di connessione utilizzato.
...
Limite di tempo (in secondi) per la disconnessione automatica della linea telefonica se non vengono scambiati dati. Il valore 0 (zero) indica che questa funzionalità è presente ma non abilitata.
...
Velocità di comunicazione massima impostabile per l'accesso al sistema telefonico.
...
Velocità di comunicazione massima impostabile per la porta COM per un modem esterno. Immettere 0 (zero) se non è applicabile.
...
Numero massimo di password definibili nel modem. Se questa funzionalità non è supportata, immettere 0 (zero).
...
Numero di anelli prima che il modem risponda a una chiamata in ingresso.
...
Livello del volume dei toni acustici dal modem. Ad esempio, è possibile segnalare un volume elevato, medio o basso.
...
Se **true**, il modem supporta il call-back.
...
Se **true**, sincrona e asincrona, la comunicazione è supportata.
...
La classe **CIM \_ POTSModem** è derivata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

WMI non implementa questa classe. Per le classi WMI derivate da **CIM \_ POTSModem**, vedere [Win32 Classes](win32-provider.md).
