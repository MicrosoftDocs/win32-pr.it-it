...
---
# <a name="cim_volumeset-class"></a>\_Classe di volumi CIM

La classe **CIM \_ volumet** rappresenta un intervallo contiguo di blocchi logici presentati all'ambiente operativo per la lettura e la scrittura di dati utente. I set di volumi non devono sovrapporsi tra loro e sono basati su uno o più extent fisici, extent dello spazio protetto o extent aggregati (tutti dello stesso tipo). Per le associazioni basate su è necessario creare un'istanza o sottoclassare secondo le esigenze.
...
La classe **CIM del \_ volumet** dispone di questi tipi di membri:
...
La classe **CIM del \_ volumet** presenta questi metodi.



| Metodo                                                               | Descrizione                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-volumeset.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
...
La classe **CIM del \_ volumet** dispone di queste proprietà.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Set di volumi DMTF \| 001,5 ")
</dt> </dl>

Numero di extent dello spazio protetto a cui eseguire lo striping come set collettivo. In SCC, questo valore viene definito come il numero di strip da contare prima di continuare a eseguire il mapping al successivo set contiguo di extent, oltre lo stripe corrente.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Set di volumi DMTF \| 001,4 ")
</dt> </dl>

Numero di extent dello spazio protetto contigui conteggiati prima di eseguire il loop fino al primo extent dello spazio protetto della striscia corrente. Si tratta del numero di extent che formano il stripe di dati utente.
...
La classe **CIM del \_ volume** è derivata da [**CIM \_ StorageExtent**](cim-storageextent.md).
