...
---
# <a name="cim_diskpartition-class"></a>CIM \_ DiskPartition (classe)

La classe **CIM \_ DiskPartition** rappresenta un intervallo contiguo di blocchi logici identificabili dal sistema operativo mediante i campi tipo e sottotipo della partizione. Le partizioni disco devono essere realizzate direttamente da supporti fisici (indicati dall'associazione [**CIM \_ RealizesDiskPartition**](cim-realizesdiskpartition.md) ) o basate su volumi di archiviazione.
...
La classe **CIM \_ DiskPartition** presenta questi tipi di membri:
...
La classe **CIM \_ DiskPartition** presenta questi metodi.



| Metodo                                                                   | Descrizione                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-diskpartition.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-diskpartition.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ DiskPartition** dispone di queste proprietà.
...
Specifica se i supporti sono leggibili, scrivibili o entrambi, ad esempio. Questa proprietà viene ereditata da [**CIM \_ StorageExtent**](cim-storageextent.md).
...
Dimensioni, in byte, dei blocchi che formano l'extent di archiviazione. Se si tratta di una dimensione di blocco variabile, è necessario specificare la dimensione massima del blocco, in byte. Se le dimensioni del blocco sono sconosciute o se un concetto di blocco non è valido, ad esempio per extent di aggregazione, memoria o dischi logici, immettere il valore 1.
...
Se **true**, la partizione del disco viene etichettata come avviabile. Ciò non significa che un sistema operativo sia caricato nella partizione.
...
Stringa in formato libero che fornisce ulteriori informazioni sull'errore registrato nella proprietà **LastErrorCode** e sulle azioni correttive da eseguire.
...
Se **true**, la partizione del disco viene etichettata come partizione primaria per un sistema di computer.
...
Descrive i supporti e il relativo utilizzo.
...
La classe **CIM \_ DiskPartition** è derivata da [**CIM \_ StorageExtent**](cim-storageextent.md).

WMI non implementa questa classe. Per le classi derivate da **CIM \_ DiskPartition**, vedere [Win32 Classes](win32-provider.md).
