...
---
# <a name="cim_networkadapter-class"></a>CIM \_ NetworkAdapter (classe)

La classe **CIM \_ NetworkAdapter** è una classe astratta che definisce i concetti relativi all'hardware di rete generale, ad esempio l'indirizzo permanente o la velocità dell'operazione. Le informazioni vengono trasmesse mediante l'associazione [**CIM \_ DeviceSAPImplementation**](cim-devicesapimplementation.md) .

Le schede di rete e gli endpoint correlati rappresentano la possibilità di connettività tra i peer. Il potenziale di connettività è significativamente diverso da quello del Master, del subordinato e del controller o delle relazioni controllate dal [**\_ controller CIM**](cim-controller.md). Occasionalmente, tuttavia, un singolo dispositivo funge sia da scheda di rete che da controller, ad esempio quando una scheda Fibre Channel funziona come controller SCSI del sistema del computer. In tal caso, gli aspetti del dispositivo sono orientati alla rete e altri sono orientati ai controller. È necessario creare un'istanza delle classi controller e adapter e creare anche una relazione di identità del dispositivo per collegare i diversi aspetti del dispositivo.
...
La classe **CIM \_ NetworkAdapter** presenta questi tipi di membri:
...
La classe **CIM \_ NetworkAdapter** presenta questi metodi.



| Metodo                                                                    | Descrizione                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-networkadapter.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
...
La classe **CIM \_ NetworkAdapter** dispone di queste proprietà.
...
Se **true**, la scheda di rete è in grado di determinare automaticamente la velocità o altre caratteristiche di comunicazione del supporto di rete collegato.
...
Velocità massima, in bit al secondo (BPS), per la scheda di rete.
...
Elenco di indirizzi di rete per un adapter.
...
Indirizzo di rete hardcoded in un adapter. L'indirizzo hardcoded può essere modificato con un aggiornamento del firmware o una configurazione software. Se modificato, il campo deve essere aggiornato quando viene apportata la modifica. Questa proprietà deve essere lasciata vuota se non esiste alcun indirizzo hardcoded per la scheda di rete.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB. ifSpeed "," MIF. \|Scheda di rete DMTF 802 porta \| 001,5 "), [**unità**](/windows/desktop/WmiSdk/standard-qualifiers) (" bit al secondo ")
</dt> </dl>

Stima della larghezza di banda corrente, in BPS. Per gli endpoint che variano a seconda della larghezza di banda o per quelli in cui non è possibile effettuare una stima accurata, questa proprietà deve contenere la larghezza di banda nominale.
...
La classe **CIM \_ NetworkAdapter** è derivata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

WMI non implementa questa classe. Per le classi derivate da **CIM \_ NetworkAdapter**, vedere [Win32 Classes](win32-provider.md).
