---
description: La classe CiM NetworkAdapter è una classe astratta che definisce i concetti generali relativi all'hardware di rete, ad esempio l'indirizzo permanente \_ o la velocità di funzionamento. Le informazioni vengono trasmesse tramite l'associazione \_ CIM DeviceSAPImplementation.
ms.assetid: dd44e05a-1124-42c2-aa69-340c06da35a2
ms.tgt_platform: multiple
title: CIM_NetworkAdapter classe
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119021029"
---
# <a name="cim_networkadapter-class"></a>Classe \_ NetworkAdapter CIM

La **classe \_ CiM NetworkAdapter** è una classe astratta che definisce i concetti generali relativi all'hardware di rete, ad esempio l'indirizzo permanente o la velocità di funzionamento. Le informazioni vengono trasmesse tramite [**l'associazione \_ CIM DeviceSAPImplementation.**](cim-devicesapimplementation.md)

Le schede di rete e gli endpoint correlati rappresentano il potenziale di connettività tra peer. La possibilità di connettività è significativamente diversa dalle relazioni master o subordinata e controller o controllata da del [**\_ controller CIM.**](cim-controller.md) In alcuni casi, tuttavia, un singolo dispositivo funge sia da scheda di rete che da controller, ad esempio quando una scheda Fiber Channel funziona come controller SCSI di un computer. In tal caso, gli aspetti del dispositivo sono orientati alla rete e altri sono orientati al controller. È necessario creare un'istanza delle classi controller e adapter e creare anche una relazione di identità del dispositivo per collegare i diversi aspetti del dispositivo.
...
La **classe \_ CiM NetworkAdapter** ha questi tipi di membri:
...
La **classe \_ NetworkAdapter CIM** dispone di questi metodi.



| Metodo                                                                    | Descrizione                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-networkadapter.md)                 | Richiede una reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La **classe \_ CiM NetworkAdapter** dispone di queste proprietà.
...
Se **TRUE,** la scheda di rete può determinare automaticamente la velocità o altre caratteristiche di comunicazione del supporto di rete collegato.
...
Velocità massima, in bit al secondo, per la scheda di rete.
...
Elenco di indirizzi di rete per una scheda.
...
Indirizzo di rete hard-coded in una scheda. L'indirizzo hard-coded può essere modificato con un aggiornamento del firmware o una configurazione software. Se viene modificato, il campo deve essere aggiornato quando viene apportata la modifica. Questa proprietà deve essere lasciata vuota se non esiste alcun indirizzo hard coded per la scheda di rete.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB.ifSpeed", "MIF. DmTF \| Network Adapter 802 Port \| 001.5"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("bits per second")
</dt> </dl>

Stima della larghezza di banda corrente, in BPS. Per gli endpoint che variano in larghezza di banda o per quelli in cui non è possibile eseguire stime accurate, questa proprietà deve contenere la larghezza di banda nominale.
...
La **classe \_ CiM NetworkAdapter** è derivata da [**CIM \_ LogicalDevice.**](cim-logicaldevice.md)

WMI non implementa questa classe. Per le classi derivate da **CiM \_ NetworkAdapter,** vedere [Classi Win32.](win32-provider.md)
