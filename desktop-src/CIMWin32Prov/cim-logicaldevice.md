...
---
# <a name="cim_logicaldevice-class-cimwin32-wmi-providers"></a>Classe CIM_LogicalDevice (provider WMI CIMWin32)

La classe **CIM \_ LogicalDevice** rappresenta un'entità hardware che può essere o meno realizzata nell'hardware fisico.
...
| Metodo                                                                   | Descrizione                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-logicaldevice.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-logicaldevice.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
Il metodo **SetPowerState** è supportato. Questo metodo è disponibile nella classe **\_ LogicalDevice CIM** padre e può essere implementato. Per ulteriori informazioni, vedere [progettazione di classi Managed Object Format (MOF)](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes).
...
Le caratteristiche del dispositivo logico che gestiscono l'operazione o la configurazione sono contenute in o sono associate all'oggetto **\_ LogicalDevice CIM** . Le proprietà operative della stampante, ad esempio, sono formati di carta supportati o errori rilevati. Le proprietà di configurazione del dispositivo del sensore, ad esempio, sono impostazioni di soglia. Diverse configurazioni possono esistere per un dispositivo logico e sono contenute negli oggetti [**delle \_ Impostazioni CIM**](cim-setting.md) , che sono associati al dispositivo logico.

La classe **CIM \_ LogicalDevice** è derivata da [**CIM \_ LogicalElement**](cim-logicalelement.md).

WMI non implementa questa classe. Per le classi derivate da **CIM \_ LogicalDevice**, vedere [Win32 Classes](win32-provider.md).
