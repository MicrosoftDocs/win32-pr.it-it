...
---
# <a name="win32_memorydevice-class"></a>Win32 \_ MemoryDevice (classe)

La [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ MemoryDevice Win32** rappresenta le proprietà di un dispositivo di memoria di sistema del computer e degli indirizzi mappati associati.
...
La classe **Win32 \_ MemoryDevice** presenta questi tipi di membri:
...
La classe **Win32 \_ MemoryDevice** presenta questi metodi.



| Metodo            | Descrizione                                                                                                                                                                                      |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Reimpostazione**         | Non implementato. Per implementare questo metodo, vedere il metodo [**Reset**](reset-method-in-class-cim-controller.md) in [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md).<br/>                 |
| **SetPowerState** | Non implementato. Per implementare questo metodo, vedere il metodo [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) in [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md).<br/> |
...
La classe **Win32 \_ MemoryDevice** dispone di queste proprietà.
...
Accesso ai supporti disponibile.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| Type 18 \| 32-bit memory Error Information Error \| Syndrome"), [**Max**](/windows/desktop/WmiSdk/standard-qualifiers) (64)
</dt> </dl>

Matrice di informazioni aggiuntive sull'errore. Un esempio è la sindrome di controllo degli errori e correzione (ECC) o la restituzione dei bit di controllo se viene usata una metodologia di errore basata su un controllo di ridondanza ciclico (CRC). Nel secondo caso, se viene riconosciuto un errore a bit singolo e l'algoritmo CRC è noto, è possibile determinare il bit esatto che ha avuto esito negativo. In questo campo sono inclusi questo tipo di dati (sindrome ECC, bit check, dati di bit di parità o altre informazioni fornite dal fornitore). Questa proprietà viene utilizzata solo quando la proprietà **errorInfo** è diversa da 3.

Questa proprietà viene ereditata da [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md).
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| tipo 18 \| 32-bit errore informazioni errore di memoria \| ")
</dt> </dl>

Se **true**, l'errore più recente è correggibile. Questa proprietà non viene utilizzata se **errorInfo** è impostato su 3.
...
Identificatore univoco del dispositivo di memoria.

Questa proprietà viene ereditata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).

Esempio: "dispositivo di memoria 1"
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS- \| tipo 19 \| indirizzi con mapping del dispositivo di memoria che \| terminano l'indirizzo")
</dt> </dl>

Indirizzo finale a cui fa riferimento un'applicazione o un sistema operativo. Questo indirizzo di memoria è mappato da un controller di memoria per questo oggetto memoria.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| di tipo \| 32 18-operazione di errore informazioni errore memoria bit \| ")
</dt> </dl>

Tipo di operazione di accesso alla memoria che ha causato l'ultimo errore. Questa proprietà è valida solo se **errorInfo** non è impostato su 3.
...
Indirizzo dell'ultimo errore di memoria. Questa proprietà viene utilizzata solo se **errorInfo** non è impostato su 3.
...
Qualificatori: [**arrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ("indexed"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS"), [**Max**](/windows/desktop/WmiSdk/standard-qualifiers) (64)
</dt> </dl>

Matrice di dati acquisiti dall'ultimo accesso alla memoria con un errore. I dati occupano i primi *n* ottetti della matrice necessari per conservare il numero di bit specificato dalla proprietà **ErrorTransferSize** . Se **ErrorTransferSize** è 0 (zero), questa proprietà non viene utilizzata.
...
Ordinamento dei dati archiviati nella proprietà **ErrorData** . Questa proprietà viene utilizzata solo quando **ErrorTransferSize** è 0 (zero).
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| tipo 18 \| errore granularità")
</dt> </dl>

Livello in cui è possibile risolvere l'errore.
...
**Livello partizione di memoria** (4)
...
Qualificatori: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ Memory**](cim-memory.md).**OtherErrorDescription**"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (" SMBIOS \| Type 18 \| 32-bit Error Information Error Information \| Type ")
</dt> </dl>

Tipo di errore che si è verificato più di recente. I valori 12-14, che indicano se un errore è correggibile, non vengono utilizzati con questa proprietà, ma queste informazioni si trovano nella proprietà **CorrectableError** .
...
**Correzione di un errore a bit singolo** (12)
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| tipo 16 memoria \| fisica matrice \| errore di memoria")
</dt> </dl>

Tipi di controllo degli errori utilizzati dall'hardware di memoria.
...
**Ecc a bit singolo** ("ecc a bit singolo")
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| tipo 18 \| 32-bit informazioni errore di memoria errore \| "), [**unità**](/windows/desktop/WmiSdk/standard-qualifiers) ("byte")
</dt> </dl>

Quantità di dati effettivamente determinata per causare l'errore. Questa proprietà non viene utilizzata quando la proprietà **errorInfo** è impostata su 3.
...
Ora in cui si è verificato l'ultimo errore di memoria. Questa proprietà è valida solo se **errorInfo** non è impostato su 3.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS"), [**unità**](/windows/desktop/WmiSdk/standard-qualifiers) ("bits")
</dt> </dl>

Dimensione dei dati (contenente l'ultimo errore) trasferiti. Questa proprietà è impostata su 0 (zero) se non è presente alcun errore.
...
Qualificatori: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ Memory**](cim-memory.md).**ErrorInfo**"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (" SMBIOS ")
</dt> </dl>

Ulteriori informazioni quando la proprietà **errorInfo** è impostata su 1.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("SMBIOS \| Type 19 \| Memory Device mapping \| Address Starting Address")
</dt> </dl>

Indirizzo iniziale a cui fa riferimento un'applicazione o il sistema operativo. Questo indirizzo di memoria è mappato da un controller di memoria per questo oggetto memoria.
...
Se **true**, le informazioni sull'indirizzo nella proprietà **ErrorAddress** sono un indirizzo a livello di sistema. Se **false**, si tratta di un indirizzo fisico. Questa proprietà viene utilizzata solo se **errorInfo** non è impostato su 3.
...
La classe **Win32 \_ MemoryDevice** è derivata da [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md).

## <a name="examples"></a>Esempio

L'esempio [List Memory Devices](https://Gallery.TechNet.Microsoft.Com/ddc9c2ab-3f88-44fb-935f-98da3bcf5858) Perl restituisce gli indirizzi iniziale e finale per tutti i dispositivi di memoria installati in un computer.
