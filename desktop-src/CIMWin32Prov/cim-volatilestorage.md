...
---
# <a name="cim_volatilestorage-class"></a>CIM \_ VolatileStorage (classe)

La classe **CIM \_ VolatileStorage** rappresenta le funzionalità e la gestione dell'archiviazione volatile.
...
La classe **CIM \_ VolatileStorage** presenta questi tipi di membri:
...
La classe **CIM \_ VolatileStorage** presenta questi metodi.



| Metodo                                                                     | Descrizione                                                                                                                              |
|:---------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-volatilestorage.md)                 | Richiede la reimpostazione del dispositivo logico. Non implementato da WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volatilestorage.md) | Definisce lo stato di alimentazione desiderato per un dispositivo logico e quando un dispositivo deve essere inserito in tale stato. Non implementato da WMI.<br/> |
...
La classe **CIM \_ VolatileStorage** dispone di queste proprietà.
...
Proprietà di lettura/scrittura del supporto.
...
Il dispositivo è noto come in modalità di risparmio energia, ma lo stato esatto è sconosciuto.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. DMTF \| informazioni sulla memoria risorse di sistema \| 001,5 ")
</dt> </dl>

Se **true**, la memoria può essere memorizzata nella cache.
...
Qualificatori: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. DMTF \| informazioni sulla memoria risorse di sistema \| 001,6 ")
</dt> </dl>

Tipo di cache compatibile con la memoria. Se la proprietà **memorizzabile nella cache** è impostata su **false**, questa proprietà non ha significato e deve essere impostata su 4 ("non applicabile").
...
Indirizzo finale, a cui viene fatto riferimento da un'applicazione o da un sistema operativo e mappato da un controller di memoria, per l'oggetto memoria. L'indirizzo finale è specificato in kilobyte.
...
Ordinamento dei dati archiviati nella matrice **ErrorData** . Se **ErrorTransferSize** è 0, questa proprietà non ha alcun significato.
...
Tipo di errore che si è verificato più di recente. I valori 12-14 non sono definiti nello schema CIM, perché la semantica del tipo di errore e se è stato correggibile o meno è mista in DMI. La proprietà **CorrectableError** indica se è correggibile.
...
Stringa in formato libero che descrive il tipo di rilevamento e correzione degli errori supportato dall'extent di archiviazione.
...
Dimensioni del trasferimento dei dati in bit che hanno causato l'ultimo errore. 0 indica che non è presente alcun errore. Se la proprietà **errorInfo** è uguale a 3, "OK", questa proprietà deve essere impostata su 0.
...
Stringa in formato libero che fornisce informazioni aggiuntive se la proprietà **ErrorType** è impostata su 1 ("other"). Se un valore diverso da 1 (uno), questa stringa non ha significato.
...
Identificatore del dispositivo Windows Plug and Play del dispositivo logico. Questa proprietà viene ereditata da [**CIM \_ LogicalDevice**](cim-logicaldevice.md).
...
Indirizzo iniziale, a cui viene fatto riferimento da un'applicazione o da un sistema operativo e mappato da un controller di memoria, per l'oggetto memoria. L'indirizzo iniziale è specificato in kilobyte.
...
Se **true**, le informazioni sull'indirizzo nella proprietà **ErrorAddress** sono un indirizzo a livello di sistema; Se **false**, si tratta di un indirizzo fisico. Se la proprietà **errorInfo** è uguale a 3 ("OK"), questa proprietà non ha alcun significato.
...
La classe **CIM \_ VolatileStorage** è derivata dalla [**\_ memoria CIM**](cim-memory.md).
