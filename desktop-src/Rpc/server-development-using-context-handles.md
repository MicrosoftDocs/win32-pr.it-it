...
Si supponga, ad esempio, che un client usi un handle di contesto per richiedere una serie di aggiornamenti a un record in un database. Il client chiama una procedura remota nel server e la passa a una chiave di ricerca. Il programma server cerca la chiave di ricerca nel database e ottiene il numero di record intero del record corrispondente. Il server può quindi puntare un puntatore a void in una posizione di memoria contenente il numero di record. Quando viene restituito, la procedura remota deve restituire il puntatore come handle di contesto tramite il relativo valore restituito o il relativo elenco di parametri. Il client deve passare il puntatore al server ogni volta che chiama le procedure remote per aggiornare il record. Durante ognuna di queste operazioni di aggiornamento, il server esegue il cast del puntatore void come puntatore a un integer.
...
La funzione RemoteClose chiude un file nel server. Si noti che l'applicazione server deve assegnare **NULL** all'handle di contesto come parte della funzione close. Ciò comunica al server stub e alla libreria di runtime RPC che l'handle di contesto è stato eliminato. In caso contrario, la connessione verrà mantenuta aperta e alla fine si verificherà un run-down del contesto.
...
> [!Note]  
> Sebbene sia previsto che il client passi un handle di contesto valido a una chiamata con attributi direzionali out, RPC non rifiuta gli handle di contesto NULL per questa combinazione \[ \] di attributi direzionali.  **L'handle** di contesto NULL viene passato al server come **puntatore NULL.** Il codice server per le chiamate che contengono un handle di contesto in e out deve essere scritto per evitare una violazione di accesso quando viene ricevuto \[ \] un **puntatore NULL.**
