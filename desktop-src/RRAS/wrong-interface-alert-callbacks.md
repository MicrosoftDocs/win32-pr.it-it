...
---
# <a name="wrong-interface-alert-callbacks"></a>Callback di avviso di interfaccia errato

Dopo che il server d'avanzamento del kernel riceve i dati multicast da un'origine specifica sull'interfaccia non corretta, invia una notifica al gestore del gruppo multicast. Il gestore del gruppo multicast richiama quindi il [**PMGM \_ errato \_ se \_**](/windows/desktop/api/Mgm/nc-mgm-pmgm_wrong_if_callback) callback callback al protocollo di routing che possiede l'interfaccia su cui sono stati ricevuti i dati in modo errato.
