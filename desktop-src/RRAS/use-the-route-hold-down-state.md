---
title: USA lo stato di Hold-Down Route
description: Nell'esempio di codice seguente viene illustrato come contrassegnare una destinazione per lo stato di attesa e come creare un'enumerazione di destinazione che includa le route nello stato di attesa.
...
ms.locfileid: "106298535"
---
# <a name="use-the-route-hold-down-state"></a><span data-ttu-id="b9301-103">USA lo stato di Hold-Down Route</span><span class="sxs-lookup"><span data-stu-id="b9301-103">Use the Route Hold-Down State</span></span>

<span data-ttu-id="b9301-104">Nell'esempio di codice seguente viene illustrato come contrassegnare una destinazione per lo stato di attesa e come creare un'enumerazione di destinazione che includa le route nello stato di attesa.</span><span class="sxs-lookup"><span data-stu-id="b9301-104">The following sample code shows how to mark a destination for the hold-down state, and how to create a destination enumeration that includes routes that are in the hold-down state.</span></span>
