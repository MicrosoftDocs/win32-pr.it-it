---
description: Specifica dell'file system exFat.
...
<tr class="odd">
<td>Facoltativo</td>
<td>Questo termine descrive un campo o una struttura che un'implementazione può o meno supportare. Se un'implementazione supporta un campo o una struttura facoltativa specifica, deve modificare e interpretare il campo o la struttura come descritto in questa specifica.</td>
</tr>
<tr class="even">
<td>Non definito</td>
...
<tr class="odd">
<td>Riservato</td>
<td><p>Questo termine descrive il contenuto del campo o della struttura, ovvero le implementazioni seguenti:</p>
<ol type="1">
...
TexFAT richiede l'uso dei due FAT e delle bitmap di allocazione per l'uso nelle transazioni. Definisce anche diverse strutture aggiuntive, tra cui descrittori di spaziatura interna e descrittori di sicurezza.
...
<td>9</td>
<td>1</td>
<td>Questa area secondaria è obbligatoria e <a href="#33-main-and-backup-oem-parameters-sub-regions">la sezione 3.3</a> ne definisce il contenuto.</td>
</tr>
<tr class="odd">
<td>Main Reserved</td>
<td>10</td>
...
<td>11</td>
<td>1</td>
<td>Questa area secondaria è obbligatoria e <a href="#34-main-and-backup-boot-checksum-sub-regions">la sezione 3.4</a> ne definisce il contenuto.</td>
</tr>
<tr class="odd">
...
<td>12</td>
<td>1</td>
<td>Questa area secondaria è obbligatoria e <a href="#31-main-and-backup-boot-sector-sub-regions">la sezione 3.1</a> ne definisce il contenuto.</td>
</tr>
<tr class="odd">
<td>Eseguire il backup dei settori di avvio esteso</td>
<td>13</td>
<td>8</td>
<td>Questa area secondaria è obbligatoria e <a href="#32-main-and-backup-extended-boot-sectors-sub-regions">la sezione 3.2</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
...
<p>Nota: i settori principale e di avvio del backup contengono entrambi il campo FatOffset.</p></td>
</tr>
<tr class="odd">
<td>Primo FAT</td>
<td>FatOffset</td>
<td>FatLength</td>
<td><p>Questa area secondaria è obbligatoria e <a href="#41-first-and-second-fat-sub-regions">la sezione 4.1</a> ne definisce il contenuto.</p>
<p>Nota: i settori principale e di avvio del backup contengono entrambi i campi FatOffset e FatLength.</p></td>
</tr>
<tr class="even">
<td>Secondo FAT</td>
...
<p>Nota: i settori principale e di avvio del backup contengono entrambi i campi FatOffset, FatLength e NumberOfFats. Il campo NumberOfFats può contenere solo i valori 1 e 2.</p></td>
</tr>
<tr class="odd">
...
<td><p>Questa area secondaria è obbligatoria e il relativo contenuto, se presente, non è definito.</p>
<p>Nota: i settori di avvio principale e di avvio del backup contengono entrambi i campi FatOffset, FatLength, NumberOfFats e ClusterHeapOffset. I valori validi del campo NumberOfFats sono 1 e 2.</p></td>
</tr>
<tr class="odd">
<td>Cluster Heap</td>
<td>ClusterHeapOffset</td>
<td>ClusterCount * 2<sup>SectorsPerClusterShift</sup></td>
<td><p>Questa area secondaria è obbligatoria e <a href="#51-cluster-heap-sub-region">la sezione 5.1</a> ne definisce il contenuto.</p>
<p>Nota: i settori di avvio principale e di avvio del backup contengono entrambi i campi ClusterHeapOffset, ClusterCount e SectorsPerClusterShift.</p></td>
</tr>
<tr class="even">
...
<td><p>Questa area secondaria è obbligatoria e il relativo contenuto, se presente, non è definito.</p>
<p>Nota: i settori di avvio principale e di avvio del backup contengono entrambi i campi ClusterHeapOffset, ClusterCount, SectorsPerClusterShift e VolumeLength.</p></td>
</tr>
</tbody>
</table>
...
L'area avvio principale fornisce tutte le istruzioni necessarie per l'avvio, le informazioni di identificazione e i parametri file system per consentire a un'implementazione di eseguire le operazioni seguenti:

1. Avvio di un sistema di computer da un volume exFAT.

2. Identificare il file system nel volume come exFAT.

3. Individuare la posizione delle strutture file system exFAT.

L'area Avvio backup è un backup dell'area avvio principale. Consente il ripristino del volume exFAT nel caso in cui l'area di avvio principale sia in uno stato incoerente. Ad eccezione delle circostanze poco frequenti, ad esempio l'aggiornamento delle istruzioni di avvio, le implementazioni non devono modificare il contenuto dell'area di avvio del backup.

### <a name="31-main-and-backup-boot-sector-sub-regions"></a>3.1 Aree secondarie Main e Backup Boot Sector

Il settore di avvio principale contiene il codice per l'avvio da un volume exFAT e i parametri exFAT fondamentali che descrivono la struttura del volume (vedere [la tabella 4).](#table-4-main-and-backup-boot-sector-structure) BIOS, MBR o altri agenti di avvio possono ispezionare questo settore e possono caricare ed eseguire le istruzioni di avvio contenute.

Il settore di avvio del backup è un backup del settore di avvio principale e ha la stessa struttura (vedere [la tabella 4).](#table-4-main-and-backup-boot-sector-structure) Il settore di avvio del backup può facilitare le operazioni di ripristino. Tuttavia, le implementazioni devono considerare il contenuto dei campi VolumeFlags e PercentInUse come non obsoleti.

Prima di usare il contenuto del settore di avvio principale o di backup, le implementazioni devono verificarne il contenuto convalidando il checksum di avvio corrispondente e verificando che tutti i relativi campi siano entro l'intervallo di valori valido.

Mentre l'operazione di formattazione iniziale iniziali inizialirà il contenuto dei settori di avvio principale e di avvio del backup, le implementazioni possono aggiornare questi settori (e aggiornare anche il rispettivo checksum di avvio) in base alle esigenze. Tuttavia, le implementazioni possono aggiornare i campi VolumeFlags o PercentInUse senza aggiornare il checksum di avvio corrispondente (il checksum esclude in modo specifico questi due campi).

<div id="table-4-main-and-backup-boot-sector-structure" />

**Tabella 4 Struttura dei settori di avvio principale e di backup**
...
- (VolumeLength - ClusterHeapOffset) / 2<sup>SectorsPerClusterShift</sup>arrotondato per esere all'intero più vicino, ovvero esattamente il numero di cluster che possono essere compresi tra l'inizio dell'heap cluster e la fine del volume
...
<td>3</td>
<td>1</td>
<td>Questo campo è obbligatorio e <a href="#31134-cleartozero-field">la sezione 3.1.13.4</a> ne definisce il contenuto.</td>
</tr>
<tr class="odd">
<td>Riservato</td>
<td>4</td>
...
Il campo MediaFailure deve descrivere se un'implementazione ha rilevato errori multimediali o meno, come indicato di seguito:
...
1. Il supporto di hosting non riesce ad accedere a qualsiasi area del volume

2. L'implementazione ha esaurito gli algoritmi di ripetizione dei tentativi di accesso, se presenti

Se, al momento del montaggio di un volume, il valore di questo campo è 1, le implementazioni che analizzano l'intero volume alla ricerca di errori dei supporti e registrano tutti gli errori come cluster "non valido" nel fat (o in caso contrario, risolvono gli errori dei supporti) possono cancellare il valore di questo campo su 0.
...
- 1, il che significa che le implementazioni devono cancellare questo campo su 0 prima di modificare file system, directory o file
...
- Al massimo 12 (dimensioni del settore di 4096 byte), ovvero le dimensioni di pagina della memoria delle CPU comuni nei personal computer

#### <a name="3115-sectorsperclustershift-field"></a>3.1.15 SettoriPerClusterShift Campo

Il campo SectorsPerClusterShift deve descrivere i settori per cluster espressi come log<sub>2</sub>(N), dove N è il numero di settori per cluster. Ad esempio, per 8 settori per cluster, il valore di questo campo è 3.
...
- 1, che indica che il volume contiene solo la prima bitmap FAT e la prima allocazione

- 2, che indica che il volume contiene la prima bitmap FAT, la seconda FAT, la prima bitmap di allocazione e la seconda bitmap di allocazione; Questo valore è valido solo per i volumi TexFAT
...
Il campo PercentInUse deve descrivere la percentuale di cluster allocati nell'heap del cluster.

L'intervallo valido di valori per questo campo deve essere:

- Tra 0 e 100 inclusi, ovvero la percentuale di cluster allocati nell'heap del cluster, arrotondata per estinzione all'intero più vicino

- FFh esatto, che indica che la percentuale di cluster allocati nell'heap del cluster non è disponibile

Le implementazioni modificano il valore di questo campo in modo da riflettere le modifiche nell'allocazione dei cluster nell'heap del cluster o lo modificano in FFh.

Le implementazioni non devono includere questo campo durante il calcolo del relativo checksum dell'area avvio principale o di avvio del backup. Quando si fa riferimento al settore di avvio del backup, le implementazioni devono considerare questo campo come non obsoleto.

#### <a name="3119-bootcode-field"></a>3.1.19 Campo BootCode

Il campo BootCode deve contenere istruzioni di avvio.
Le implementazioni possono popolare questo campo con le istruzioni della CPU necessarie per avviare un computer. Le implementazioni che non forniscono istruzioni di avvio devono inizializzare ogni byte in questo campo su F4h (l'istruzione di arresto per le CPU comuni nei personal computer) come parte del relativo funzionamento del formato.

#### <a name="3120-bootsignature-field"></a>3.1.20 Campo BootSignature

Il campo BootSignature deve indicare se lo scopo di un determinato settore è un settore di avvio o meno.
...
### <a name="32-main-and-backup-extended-boot-sectors-sub-regions"></a>3.2 Aree secondarie dei settori di avvio principale e di backup esteso

Ogni settore dei principali settori di avvio esteso ha la stessa struttura; Tuttavia, ogni settore può contenere istruzioni distinte per l'avvio (vedere la tabella 6). Gli agenti di avvio, ad esempio le istruzioni di avvio nel settore di avvio principale, le implementazioni alternative del BIOS o il firmware di un sistema incorporato, possono caricare questi settori ed eseguire le istruzioni che contengono.

Il backup dei settori di avvio esteso è un backup dei principali settori di avvio esteso e ha la stessa struttura (vedere [la tabella 6).](#table-6-extended-boot-sector-structure)

Prima di eseguire le istruzioni dei settori di avvio principale o di avvio esteso di backup, le implementazioni devono verificarne il contenuto assicurandosi che il campo ExtendedBootSignature di ogni settore contenga il valore prescritto.

Mentre l'operazione di formattazione iniziale iniziali inizialirà il contenuto dei settori di avvio principale e di avvio esteso di backup, le implementazioni possono aggiornare questi settori (e aggiornare anche il rispettivo checksum di avvio) in base alle esigenze.
...
Le implementazioni possono popolare questo campo con le istruzioni della CPU necessarie per avviare un computer. Le implementazioni che non forniscono istruzioni di avvio devono inizializzare ogni byte in questo campo su 00h come parte dell'operazione di formattazione.

#### <a name="322-extendedbootsignature-field"></a>3.2.2 Campo ExtendedBootSignature

Il campo ExtendedBootSignature deve indicare se lo scopo di un settore specificato è di essere un settore di avvio esteso o meno.
...
Le implementazioni devono verificare il contenuto di questo campo prima di dipendere da qualsiasi altro campo nel rispettivo settore di avvio esteso.

### <a name="33-main-and-backup-oem-parameters-sub-regions"></a>3.3 Aree secondarie dei parametri OEM principale e di backup

L'area secondaria Main OEM Parameters (Parametri OEM principali) contiene dieci strutture di parametri che possono contenere informazioni specifiche del produttore (vedere [la tabella 7).](#table-7-oem-parameters-structure) Ognuna delle dieci strutture di parametri deriva dal modello Parametri generici (vedere [la sezione 3.3.2).](#332-generic-parameters-template) I produttori possono derivare le proprie strutture di parametri personalizzate dal modello Parametri generici. Questa specifica definisce in sé due strutture di parametri: Parametri Null (vedere la sezione [3.3.3)](#333-null-parameters)e Parametri Flash (vedere la sezione [3.3.4).](#334-flash-parameters)
...
I produttori devono popolare i parametri OEM Main e Backup con le proprie strutture di parametri personalizzate, se presenti, e qualsiasi altra struttura di parametri. Le operazioni di formattazione successive mantengono il contenuto dei parametri OEM Main e Backup.

Le implementazioni possono aggiornare i parametri OEM principale e di backup in base alle esigenze (e devono anche aggiornare il checksum di avvio corrispondente).
...
La struttura Parametri Null deriva dal modello Parametri generici (vedere la sezione [3.3.2)](#332-generic-parameters-template)e deve descrivere un campo Parametri inutilizzati (vedere [la tabella 9).](#table-9-null-parameters-structure) Durante la creazione o l'aggiornamento della struttura Parameters OEM, le implementazioni popolano i campi parameters inutilizzati con la struttura Parametri Null. Inoltre, durante la creazione o l'aggiornamento della struttura Parameters OEM, le implementazioni devono consolidare le strutture dei parametri Null alla fine della matrice, lasciando tutte le altre strutture Parameters all'inizio della struttura Parameters OEM.
...
Tutti i valori possibili per tutti i campi Parametri Flash, ad eccezione del campo ParametersGuid, sono validi. Tuttavia, il valore 0 indica che il campo non è effettivamente significato (le implementazioni ignoreranno il campo specificato).

##### <a name="3341-parametersguid-field"></a>3.3.4.1 Campo ParametersGuid
...
Il valore valido per questo campo, in notazione GUID, è {0A0C7E46-3399-4021-90C8-FA6D389C4BA2}.

##### <a name="3342-eraseblocksize-field"></a>3.3.4.2 Campo EraseBlockSize

Il campo EraseBlockSize deve descrivere le dimensioni, in byte, del blocco di cancellazione dei supporti flash.

##### <a name="3343-pagesize-field"></a>3.3.4.3 Campo PageSize

Il campo PageSize deve descrivere le dimensioni, in byte, della pagina del supporto flash.

##### <a name="3344-sparesectors-field"></a>3.3.4.4 Campo SpareSectors

Il campo SpareSectors deve descrivere il numero di settori disponibili per i supporti flash per le operazioni di sparing interne.

##### <a name="3345-randomaccesstime-field"></a>3.3.4.5 Campo RandomAccessTime

Il campo RandomAccessTime deve descrivere il tempo medio di accesso casuale del supporto flash, in nanosecondi.

##### <a name="3346-programmingtime-field"></a>3.3.4.6 Campo ProgrammingTime

Il campo ProgrammingTime deve descrivere il tempo medio di programmazione dei supporti flash, in nanosecondi.

##### <a name="3347-readcycle-field"></a>3.3.4.7 Campo ReadCycle

Il campo ReadCycle deve descrivere il tempo medio del ciclo di lettura del supporto flash, in nanosecondi.

##### <a name="3348-writecycle-field"></a>3.3.4.8 Campo WriteCycle

Il campo WriteCycle deve descrivere il tempo medio del ciclo di scrittura, in nanosecondi.

### <a name="34-main-and-backup-boot-checksum-sub-regions"></a>3.4 Aree secondarie di checksum di avvio principale e di backup

I checksum di avvio principale e di backup contengono ognuno un modello ripetuto del checksum a quattro byte del contenuto di tutte le altre aree secondarie nelle rispettive aree di avvio. Il calcolo del checksum non includerà i campi VolumeFlags e PercentInUse nel rispettivo settore di avvio (vedere [la figura 1).](#figure-1-boot-checksum-computation) Il modello ripetuto del checksum a quattro byte riempie la rispettiva sottoarea di checksum di avvio dall'inizio alla fine dell'area secondaria.

Prima di usare il contenuto di qualsiasi altra sottoarea nell'aree principale o di avvio del backup, le implementazioni devono verificarne il contenuto convalidando il checksum di avvio corrispondente.

Mentre l'operazione di formattazione iniziale popola i checksum di avvio principale e di backup con il modello di checksum ripetuto, le implementazioni aggiorneranno questi settori quando cambia il contenuto degli altri settori nelle rispettive aree di avvio.
...
## <a name="4-file-allocation-table-region"></a>4 Area della tabella di allocazione file

L'area FAT (File Allocation Table) può contenere fino a due fat, uno nella prima sottoarea FAT e l'altro nella seconda sottoarea FAT.
Il campo NumberOfFats descrive il numero di fat contenute nell'area. I valori validi per il campo NumberOfFats sono 1 e 2. Pertanto, la prima sottoarea FAT contiene sempre un fat. Se il campo NumberOfFats è due, anche la seconda sottoarea FAT contiene un fat.
...
Le implementazioni devono considerare il fat che non è attivo come non obsoleto. L'uso del fat inattivo e il passaggio da un fat all'altro è specifico dell'implementazione.

### <a name="41-first-and-second-fat-sub-regions"></a>4.1 Prime e seconde sottoaree FAT

Un fat deve descrivere le catene di cluster nell'heap del cluster (vedere [la tabella 11).](#table-11-file-allocation-table-structure)
Una catena di cluster è una serie di cluster che fornisce spazio per la registrazione del contenuto di file, directory e file system strutture. Un fat rappresenta una catena di cluster come un elenco di indici del cluster collegati in modo indovino. Ad eccezione delle prime due voci, ogni voce in un fat rappresenta esattamente un cluster.
...
<td><p>Questo campo è obbligatorio e il relativo contenuto, se presente, non è definito.</p>
<p>Nota: i settori principale e di avvio del backup contengono entrambi i campi ClusterCount, FatLength e BytesPerSectorShift.</p></td>
</tr>
</tbody>
</table>

#### <a name="411-fatentry0-field"></a>4.1.1 FatEntry \[ 0 \] Field

Il campo FatEntry 0 deve descrivere il tipo di supporto nel primo byte (il byte di ordine più basso) e deve contenere \[ \] FFh nei tre byte rimanenti.

Il tipo di supporto (il primo byte) deve essere F8h.

#### <a name="412-fatentry1-field"></a>4.1.2 FatEntry \[ 1 \] Campo

Il campo FatEntry \[ 1 \] esiste solo a causa della precedenza cronologica e non descrive alcun elemento di interesse.

Il valore valido per questo campo è FFFFFFFFh. Le implementazioni devono inizializzare questo campo sul valore prescritto e non devono usare questo campo per alcun scopo. Le implementazioni non devono interpretare questo campo e devono conservarne il contenuto tra le operazioni che modificano i campi circostante.

#### <a name="413-fatentry2--fatentryclustercount1-fields"></a>4.1.3 FatEntry \[ 2 \] ... FatEntry \[ ClusterCount+1 \] Campi
...
- Tra 2 e ClusterCount + 1, inclusi, che punta al fatEntry successivo nella catena di cluster specificata; l'elemento FatEntry specificato non deve puntare ad alcun FatEntry che lo precede nella catena di cluster specificata
...
### <a name="51-cluster-heap-sub-region"></a>5.1 Sotto-area dell'heap del cluster

La struttura dell'heap del cluster è molto semplice (vedere [la tabella 12](#table-12-cluster-heap-structure)). ogni serie consecutiva di settori descrive un cluster, come definito dal campo SectorsPerClusterShift. È importante notare che il primo cluster dell'heap del cluster ha l'indice 2, che corrisponde direttamente all'indice di FatEntry \[ \] 2.

In un volume exFAT, una bitmap di allocazione (vedere [la sezione 7.1.5)](#715-allocation-bitmap)mantiene il record dello stato di allocazione di tutti i cluster. Si tratta di una differenza significativa rispetto ai predecessori di exFAT (FAT12, FAT16 e FAT32), in cui un fat mantiene un record dello stato di allocazione di tutti i cluster nell'heap del cluster.
...
<td>0</td>
<td>1</td>
<td>Questo campo è obbligatorio e <a href="#621-entrytype-field">la sezione 6.2.1</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
<td>CustomDefined</td>
<td>1</td>
...
La possibilità di interpretare il modello DirectoryEntry primaria generica è obbligatoria.
...
**Tabella 16 Modello DirectoryEntry primaria generica**
...
<td>0</td>
<td>1</td>
<td>Questo campo è obbligatorio e <a href="#631-entrytype-field">la sezione 6.3.1</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
...
La definizione di voci di directory primarie non dannose è correlata al numero di revisione secondario della exFAT. Il supporto per qualsiasi voce di directory primaria non valida definita da questa specifica o da qualsiasi specifica successiva è facoltativo. Una voce di directory primaria non riconosciuta esegue il rendering dell'intero set di voci di directory come non riconosciuto (oltre la definizione dei modelli di voci di directory applicabili).
...
Il campo SecondaryCount deve descrivere il numero di voci di directory secondarie che seguono immediatamente la voce di directory primaria specificata. Queste voci di directory secondarie, insieme alla voce di directory primaria specificata, costituiscono il set di voci di directory.
...
Le strutture critiche delle voci di directory primaria che derivano da questo modello possono ridefinire entrambi i campi SecondaryCount e SetChecksum.
...
Le strutture critiche delle voci di directory primaria che derivano da questo modello possono ridefinire questo campo.
...
Il campo NoFatChain indicherà se il fat attivo descrive o meno la catena di cluster dell'allocazione specificata.
...
- 1, ovvero l'allocazione associata è una serie contigua di cluster; le voci FAT corrispondenti per i cluster non sono valide e le implementazioni non le interpretano; Le implementazioni di possono usare l'equazione seguente per calcolare le dimensioni dell'allocazione associata: DataLength / (2<sup>SectorsPerClusterShift</sup> \* 2<sup>BytesPerSectorShift</sup>) arrotondato per es.

Se le strutture critiche delle voci della directory primaria che derivano da questo modello ridefinino il campo GeneralPrimaryFlags, le voci FAT corrispondenti per la catena di cluster dell'allocazione associata sono valide.
...
Le strutture critiche delle voci di directory primaria che derivano da questo modello possono ridefinire i campi FirstCluster e DataLength. Altre strutture che derivano da questo modello possono ridefinire i campi FirstCluster e DataLength solo se il campo AllocationPossible contiene il valore 0.
...
Lo scopo centrale delle voci di directory secondarie è fornire informazioni aggiuntive su un set di voci di directory. La possibilità di interpretare il modello DirectoryEntry secondario generico è obbligatoria.

La definizione di voci di directory secondarie critiche e non dannose è correlata al numero di revisione exFAT secondario. Il supporto per qualsiasi voce di directory secondaria critica o non dannosa definita da questa specifica o specifiche successive è facoltativo.

Tutte le strutture delle voci di directory secondarie derivano dal modello DirectoryEntry secondario generico (vedere la tabella [18),](#table-18-generic-secondary-directoryentry-template)che deriva dal modello DirectoryEntry generico (vedere la sezione [6.2).](#62-generic-directoryentry-template)
...
<td>0</td>
<td>1</td>
<td>Questo campo è obbligatorio e <a href="#641-entrytype-field">la sezione 6.4.1</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
...
###### <a name="64121-critical-secondary-directory-entries"></a>6.4.1.2.1 Voci della directory secondaria critica

Le voci di directory secondarie critiche contengono informazioni fondamentali per la corretta gestione del set di voci di directory contenitore.
Anche se il supporto per qualsiasi voce di directory secondaria critica specifica è facoltativo, una voce di directory critica non riconosciuta esegue il rendering dell'intero set di voci di directory come non riconosciuto (oltre la definizione dei modelli di voce di directory applicabili).

Tuttavia, se un set di voci di directory contiene almeno una voce di directory secondaria critica che un'implementazione non riconosce, l'implementazione interpreterà al massimo i modelli delle voci di directory nel set di voci di directory e non i dati associati a qualsiasi voce di directory nel set di voci di directory (Le voci di directory file sono un'eccezione, vedere la Sezione [7.4).](#74-file-directory-entry)

###### <a name="64122-benign-secondary-directory-entries"></a>6.4.1.2.2 Voci di directory secondarie non valide

Le voci di directory secondarie non dannose contengono informazioni aggiuntive che possono essere utili per la gestione del relativo set di voci di directory contenitore. Il supporto per qualsiasi voce di directory secondaria non valida specifica è facoltativo.
Le voci di directory secondarie non riconoscite non vengono renderizzate come non riconoscite per l'intero set di voci di directory.
...
**Tabella 19 Struttura di campo Generic GeneralSecondaryFlags**
...
Il campo AllocationPossible deve avere la stessa definizione del campo con lo stesso nome nel modello DirectoryEntry primaria generica (vedere [la Sezione 6.3.4.1).](#6341-allocationpossible-field)

##### <a name="6422-nofatchain-field"></a>6.4.2.2 Campo NoFatChain

Il campo NoFatChain deve avere la stessa definizione del campo con lo stesso nome nel modello DirectoryEntry primaria generica (vedere [la Sezione 6.3.4.2).](#6342-nofatchain-field)
...
Nella tabella exFAT file system, una FAT non descrive lo stato di allocazione dei cluster. invece, una bitmap di allocazione. Le bitmap di allocazione sono presenti nell'heap del cluster (vedere la Sezione [7.1.5)](#715-allocation-bitmap)e hanno voci di directory primarie critiche corrispondenti nella directory radice (vedere [la tabella 20).](#table-20-allocation-bitmap-directoryentry-structure)

Il campo NumberOfFats determina il numero di voci di directory bitmap di allocazione valide nella directory radice. Se il campo NumberOfFats contiene il valore 1, l'unico numero valido di voci di directory Allocation Bitmap è 1. Inoltre, la voce di directory Allocation Bitmap è valida solo se descrive la prima bitmap di allocazione (vedere [la Sezione 7.1.2.1).](#7121-bitmapidentifier-field) Se il campo NumberOfFats contiene il valore 2, l'unico numero valido di voci di directory Allocation Bitmap è 2.
...
**Tabella 20 Struttura DirectoryEntry bitmap di allocazione**
...
Per una voce di directory Allocation Bitmap, il valore valido per questo campo è 1.
...
Il campo FirstCluster deve essere conforme alla definizione fornita nel modello DirectoryEntry primaria generica (vedere [la sezione 6.3.5).](#635-firstcluster-field)
...
Per la voce di directory Up-case Table, il valore valido per questo campo è
2.
...
Questo campo contiene l'indice del primo cluster della catena di cluster, come descritto dalla FAT, che ospita la tabella Up-case.
...
Una tabella up-case è una serie di mapping di caratteri Unicode. Un mapping dei caratteri è costituito da un campo a 2 byte, con l'indice del campo nella tabella delle maiuscole e minuscole che rappresenta il carattere Unicode da utilizzare come maiuscole e minuscole e il campo a 2 byte che rappresenta il carattere Unicode con maiuscole e minuscole.
...
Le implementazioni che supportano solo caratteri dell'intervallo di mapping obbligatorio possono ignorare i mapping del resto della tabella up-case. Tali implementazioni devono usare solo caratteri dell'intervallo di mapping obbligatorio durante la creazione o la ridenominazione dei file (tramite la voce di directory Nome file, vedere [La sezione 7.7).](#77-file-name-directory-entry) Quando si esegue l'up-caseing dei nomi di file esistenti, tali implementazioni non devono eseguire l'up-case dei caratteri dall'intervallo di mapping non obbligatorio, ma devono lasciarli intatti nel nome file con maiuscole e minuscole risultante (si tratta di un up-casing parziale). Quando si confrontano i nomi di file, tali implementazioni devono considerare equivalenti i nomi di file che differiscono dal nome confrontato solo dai caratteri Unicode dell'intervallo di mapping non obbligatorio. Anche se questi nomi di file sono potenzialmente equivalenti, tali implementazioni non possono garantire che il nome di file con distinzione tra maiuscole e minuscole non collide con il nome in confronto.

<div id="table-24-mandatory-first-128-up-case-table-entries" />

**Tabella 24 Voci obbligatorie delle prime 128 tabelle up case obbligatorie**
...
**(Nota: le voci con mapping di maiuscole e minuscole senza identità sono in grassetto)**

Al momento della formattazione di un volume, le implementazioni possono generare una tabella di maiuscole e minuscole in un formato compresso usando la compressione del mapping di identità, poiché gran parte dello spazio dei caratteri Unicode non ha il concetto di maiuscole e minuscole (il che significa che i caratteri "minuscoli" e "maiuscoli" sono equivalenti).
...
Se un'implementazione definisce la propria tabella up-case, compressa o non compressa, tale tabella deve coprire l'intervallo di caratteri Unicode completo (da codici carattere 0000h a FFFFh inclusi).
...
| **0590h**      | 1D84h                        | 1 D85h   | 1D86h   | 1 D87h   | 1D88h   | 1 D89h   | 1D8Ah   | 1D8Bh   |
...
| **05E8h**      | 1DDCh                        | 1DDDh   | 1DDEh   | 1DDFh   | 1 DE0h   | 1 DE1h   | 1DE2h   | 1DE3h   |
| **05F0h**      | 1 DE4h                        | 1 DE5h   | 1 DE6h   | 1 DE7h   | 1 DE8h   | 1 DE9h   | 1 DEAh   | 1 DEBh   |
| **05F8h**      | 1 DECh                        | 1DEDh   | 1 DEEH   | 1 DEFh   | 1DF0h   | 1DF1h   | 1DF2h   | 1DF3h   |
...
| **0930h**      | 2124 ore                        | 2125h   | 2126h   | 2127h   | 2128 ore   | 2129h   | 212Ah   | 212Bh   |
...
### <a name="73-volume-label-directory-entry"></a>7.3 Voce directory etichetta volume

L'etichetta del volume è una stringa Unicode che consente agli utenti finali di distinguere i volumi di archiviazione. Nella directory exFAT file system, l'etichetta del volume esiste come voce di directory primaria critica nella directory radice (vedere [la tabella 26](#table-26-volume-label-directoryentry-structure)). Il numero valido di voci di directory Volume Label è compreso tra 0 e 1.
...
<td>0</td>
<td>1</td>
<td>Questo campo è obbligatorio e <a href="#731-entrytype-field">la sezione 7.3.1</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
...
Per la voce di directory Etichetta volume, il valore valido per questo campo è
3.
...
- Almeno 0, ovvero la stringa Unicode è lunga 0 caratteri (equivalente a nessuna etichetta di volume)

- Al massimo 11, ovvero la stringa Unicode ha una lunghezza di 11 caratteri

#### <a name="733-volumelabel-field"></a>7.3.3 Campo VolumeLabel

Il campo VolumeLabel deve contenere una stringa Unicode, ovvero il nome descrittivo del volume. Il campo VolumeLabel ha lo stesso set di caratteri non validi del campo FileName della voce di directory Nome file (vedere [la Sezione 7.7.3).](#773-filename-field)

### <a name="74-file-directory-entry"></a>7.4 Voce della directory file

Le voci della directory file descrivono file e directory. Si tratta di voci di directory primarie critiche e qualsiasi directory può contenere zero o più voci di directory file (vedere [la tabella 27).](#table-27-file-directoryentry) Per una voce di directory File valida, esattamente una voce di directory dell'estensione di flusso e almeno una voce di directory File Name deve seguire immediatamente la voce della directory File (vedere rispettivamente [Sezione 7.6](#76-stream-extension-directory-entry) e [Sezione 7.7).](#77-file-name-directory-entry)
...
<td>2</td>
<td>2</td>
<td>Questo campo è obbligatorio e <a href="#743-setchecksum-field">la sezione 7.4.3</a> ne definisce il contenuto.</td>
</tr>
<tr class="even">
...
In combinazione, i campi LastModifiedTimestamp e LastModifiedTime10msIncrement devono descrivere la data e l'ora locali dell'ultima modifica del contenuto di uno dei cluster associati alla voce di directory dell'estensione di flusso specificata. Il campo LastModifiedUtcOffset descrive l'offset di data e ora locali rispetto all'ora UTC. Le implementazioni devono aggiornare questi campi:
...
Il campo Minuto descrive la parte relativa ai minuti del campo Timestamp.
...
Il campo Month descrive la parte relativa al mese del campo Timestamp.
...
Il campo Anno descrive la parte relativa all'anno del campo Timestamp relativa all'anno 1980. Questo campo rappresenta l'anno 1980 con il valore 0 e l'anno 2107 con il valore 127.
...
2. La data e l'ora locali non sono note, nel qual caso il valore del campo OffsetValid deve essere 1 e le implementazioni considerino UTC come data e ora locali

3. UTC non è noto, nel qual caso il valore del campo OffsetValid deve essere 0

Se l'offset di data e ora locale dall'ora UTC non è un multiplo di intervalli di 15 minuti, le implementazioni registrano 00h nel campo OffsetFromUtc e considerano UTC come data e ora locali.
...
Le implementazioni devono impostare questo campo sul valore 0 solo quando l'ora UTC non è disponibile per il calcolo del valore del campo OffsetFromUtc. Se questo campo contiene il valore 0, le implementazioni trattano i campi Timestamp e 10msIncrement come con lo stesso offset UTC della data e dell'ora locali correnti.

### <a name="75-volume-guid-directory-entry"></a>7.5 Voce di directory GUID volume

La voce di directory VOLUME GUID contiene un GUID che consente alle implementazioni di distinguere in modo univoco e a livello di codice i volumi.
Il GUID del volume esiste come voce di directory primaria non valida nella directory radice (vedere [la tabella 32).](#table-32-volume-guid-directoryentry) Il numero valido di voci di directory GUID del volume è compreso tra 0 e 1.

<div id="table-32-volume-guid-directoryentry" />

**Tabella 32 Volume GUID DirectoryEntry**
...
Per la voce di directory VOLUME GUID, il valore valido per questo campo è
0.
...
##### <a name="7541-allocationpossible-field"></a>7.5.4.1 AllocationPossible Field

Il campo AllocationPossible deve essere conforme alla definizione fornita nel modello DirectoryEntry primaria generica (vedere [la Sezione 6.3.4.1).](#6341-allocationpossible-field)
...
Tutti i valori possibili per questo campo sono validi, ad eccezione del GUID Null, che è {00000000-0000-0000-0000-000000000000} .

### <a name="76-stream-extension-directory-entry"></a>7.6 Voce della directory dell'estensione di flusso

La voce di directory Dell'estensione di flusso è una voce di directory secondaria critica nei set di voci di directory file (vedere [la tabella 33).](#table-33-stream-extension-directoryentry) Il numero valido di voci di directory dell'estensione di flusso in un set di voci di directory file è 1.
Inoltre, questa voce di directory è valida solo se segue immediatamente la voce di directory File.
...
Le voci della directory Nome file sono voci di directory secondarie critiche nei set di voci della directory file (vedere [la tabella 34).](#table-34-file-name-directoryentry) Il numero valido di voci di directory Nome file in un set di voci della directory File è NameLength/ 15, arrotondato per es. al numero intero più vicino. Inoltre, le voci di directory Nome file sono valide solo se seguono immediatamente la voce di directory Estensione di flusso come serie consecutive. Le voci di directory Nome file vengono combinate in modo da formare il nome file per il set di voci della directory File.
...
Le implementazioni che non riconoscono il GUID di una voce di directory dell'estensione del fornitore devono considerare la voce di directory come qualsiasi altra voce di directory secondaria non riconosciuta non riconosciuta (vedere la sezione [8.2).](#82-implications-of-unrecognized-directory-entries)
...
##### <a name="7821-allocationpossible-field"></a>7.8.2.1 AllocationPossible Field
...
Per la voce di directory Vendor Allocation il valore valido per questo campo è 1.
...
##### <a name="7921-allocationpossible-field"></a>7.9.2.1 AllocationPossible Field
...
Il campo VendorGuid deve contenere un GUID che identifica in modo univoco l'allocazione fornitore specificata.

Tutti i valori possibili per questo campo sono validi, ad eccezione del GUID Null, che è {00000000-0000-0000-0000-000000000000} . Tuttavia, i fornitori devono usare uno strumento di generazione di GUID, ad esempio GuidGen.exe, per selezionare un GUID durante la definizione delle estensioni.
...
Le implementazioni devono garantire che il volume sia il più resiliente possibile agli errori di alimentazione e ad altri errori inevitabili. Quando si creano nuove voci di directory o si modificano le allocazioni del cluster, le implementazioni devono in genere seguire questo ordine di scrittura:

1. Impostare il valore del campo VolumeDirty su 1

2. Aggiornare la FAT attiva, se necessario
...
3. Aggiornare la FAT attiva, se necessario
...
Le specifiche exFAT future dello stesso numero di revisione principale, 1 e numero di revisione secondario superiore a 0, possono definire nuove voci di directory primaria, secondaria critica e secondarie non dannose. Solo le specifiche exFAT di un numero di revisione principale superiore possono definire nuove voci di directory primaria critiche. Le implementazioni di questa specifica, exFAT Revision 1.00 File System Basic Specification, devono essere in grado di montare e accedere a qualsiasi volume exFAT del numero di revisione principale 1 e a qualsiasi numero di revisione secondaria. Vengono presentati scenari in cui un'implementazione può riscontrare voci di directory che non riconosce. Di seguito vengono descritte le implicazioni di questi scenari:

1. La presenza di voci di directory primarie critiche non riconoscite nella directory radice rende il volume non valido. La presenza di qualsiasi voce di directory primaria critica, ad eccezione delle voci di directory file, in qualsiasi directory non radice rende la directory di hosting non valida.

2. Le implementazioni non devono modificare le voci di directory primarie non riconoscite non riconoscite o le allocazioni del cluster associate. Tuttavia, quando si elimina una directory e solo quando si elimina una directory, le implementazioni eliminano voci di directory primarie non riconoscite e liberano tutte le allocazioni del cluster associate, se presenti.

3. Le implementazioni non devono modificare le voci di directory secondarie critiche non riconoscite o le allocazioni del cluster associate. La presenza di una o più voci di directory secondarie critiche non riconoscite in un set di voci di directory rende non riconosciuto l'intero set di voci di directory. Quando si elimina un set di voci di directory che contiene una o più voci di directory secondarie critiche non riconoscite, le implementazioni liberano tutte le allocazioni del cluster, se presenti, associate a voci di directory secondarie critiche non riconoscite.
   Inoltre, se il set di voci di directory descrive una directory, le implementazioni possono:

   - Attraversamento nella directory

   - Enumerare le voci di directory in essa contenute

   - Eliminare voci di directory contenute

   - Spostare le voci di directory contenute in una directory diversa

   Tuttavia, le implementazioni non devono:

   - Modificare le voci di directory contenute, ad eccezione di delete, come illustrato
...
4. Le implementazioni non devono modificare le voci di directory secondarie non riconoscite non riconoscite o le allocazioni del cluster associate.
   Le implementazioni devono ignorare le voci di directory secondarie non riconoscite non riconoscite. Quando si elimina un set di voci di directory, le implementazioni liberano tutte le allocazioni di cluster, se presenti, associate a voci di directory secondarie non riconoscite.
...
Il campo SectorsPerClusterShift definisce i limiti delle dimensioni del cluster inferiore e superiore ( limite **inferiore: 1 settore; limite superiore: 25 -- Settori BytesPerSectorShift**, che restituisce 32 MB).

### <a name="93-cluster-heap-size-limits"></a>9.3 Limiti delle dimensioni dell'heap del cluster

L'heap del cluster deve contenere almeno spazio sufficiente per ospitare le strutture file system di base seguenti: la directory radice, tutte le bitmap di allocazione e la tabella up-case.

Il limite inferiore per le dimensioni dell'heap del cluster è una funzione del limite di dimensioni inferiori di ognuna delle strutture di file system di base che si trovano nell'heap del cluster. Anche considerando il cluster più piccolo possibile (512 byte), ognuna delle strutture di base file system non richiede più di un cluster. Il limite inferiore è **quindi: 2 + Cluster NumberOfFats**, che restituisce 3 o 4 cluster, a seconda del valore del campo NumberOfFats.

Il limite massimo di dimensioni heap cluster è una semplice funzione del numero massimo possibile di cluster definito dal campo ClusterCount (**limite superiore: 2 <sup>32</sup>- 11 cluster).** Indipendentemente dalle dimensioni del cluster, tale heap del cluster ha spazio sufficiente per ospitare almeno le strutture file system di base.

### <a name="94-volume-size-limits"></a>9.4 Limiti delle dimensioni del volume

Il campo VolumeLength definisce i limiti delle dimensioni del volume inferiore e superiore (limite inferiore: **2 <sup>settori 2 20/2</sup><sup>BytesPerSectorShift</sup>**, che restituisce 1 MB; **limite superiore: 2 <sup>64</sup>- 1** settori, che, date le dimensioni del settore più grandi possibili, restituisce circa 64ZB. Tuttavia, questa specifica consiglia non più di<sup>2 cluster da 24</sup>a 2 nell'heap del cluster (vedere [la sezione 3.1.9).](#319-clustercount-field) Pertanto, il limite superiore consigliato di un volume è: ClusterHeapOffset + (2<sup>24</sup>- \* 2) 2<sup>SectorsPerClusterShift</sup>. Data la dimensione massima possibile del cluster, 32 MB e presupponendo che ClusterHeapOffset sia di 96 MB (spazio sufficiente per le aree di avvio principale e di avvio del backup e solo il primo FAT), il limite superiore consigliato di un volume restituisce circa 512 TB.

### <a name="95-directory-size-limits"></a>9.5 Limiti delle dimensioni della directory

Il campo DataLength della voce di directory Stream Extension definisce i limiti delle dimensioni inferiori e superiori della directory (**limite inferiore: 0 byte; limite superiore: 256 MB).** Ciò significa che una directory può ospitare fino a 8.388.608 voci di directory (ogni voce di directory utilizza 32 byte). Dato il set di voci della directory File più piccolo possibile, tre voci di directory, una directory può ospitare fino a 2.796.202 file.

## <a name="10-appendix"></a>10 Appendice

### <a name="101-globally-unique-identifiers-guids"></a>10.1 Identificatori univoci globali (GUID)

Un GUID è l'implementazione Microsoft di un identificatore univoco universale. Un GUID è un valore a 128 bit costituito da un gruppo di 8 cifre esadecimali, seguiti da tre gruppi di 4 cifre esadecimali e da un gruppo di 12 cifre esadecimali, ad esempio {6B29FC40-CA47-1067-B31D-00D010662DA}, (vedere la tabella [38](#table-38-guid-structure)).

<div id="table-38-guid-structure" />

**Struttura GUID della tabella 38**
...
<td>9</td>
<td>1</td>
<td>Questo campo è obbligatorio e contiene il byte meno significativo del quarto gruppo del GUID (1Dh dell'esempio).</td>
</tr>
<tr class="even">
<td>Data4[2]</td>
<td>10</td>
...
<td>12</td>
<td>1</td>
<td>Questo campo è obbligatorio e contiene il terzo byte del quinto gruppo del GUID (01h dell'esempio).</td>
</tr>
<tr class="odd">
<td>Data4[5]</td>
<td>13</td>
<td>1</td>
<td>Questo campo è obbligatorio e contiene il quarto byte del quinto gruppo del GUID (06h dell'esempio).</td>
</tr>
<tr class="even">
<td>Data4[6]</td>
<td>14</td>
<td>1</td>
<td>Questo campo è obbligatorio e contiene il quinto byte del quinto gruppo del GUID (62h dell'esempio).</td>
</tr>
<tr class="odd">
<td>Data4[7]</td>
<td>15</td>
...
<p>Aggiunta delle voci della directory Vendor Extension e Vendor Allocation nelle sezioni 7.8 e 7.9</p>
...
<p>Chiarimento del campo VolumeDirty nella sezione 3.1.13.2 e ordinamento consigliato per la scrittura nella sezione 8.1</p>
...
<p>Modifica delle sezioni 1.5 per garantire l'allineamento con il documento di specifica TexFAT.</p>
...
<p>Aggiunta del requisito per i nomi di file e directory univoci alla sezione 7.7</p>
...
<p>Rimozione di microsoft confidential notice</p>
