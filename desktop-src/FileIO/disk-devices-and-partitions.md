...
title: Dispositivi e partizioni su disco
ms.topic: article
ms.date: 05/31/2018
...
---
# <a name="disk-devices-and-partitions"></a>Dispositivi e partizioni su disco

Un disco rigido è costituito da un set di dischi in pila, ognuno dei quali contiene dati archiviati elettromagneticamente in cerchi concentrici o *tracce*. Ogni piatto ha due teste, una su ogni lato del piatto, che legge o scrive i dati mentre il disco ruota. *Un'unità disco rigido* controlla il posizionamento, la lettura e la scrittura del disco rigido. Si noti che le teste di tutti i piatti sono posizionate come unità.

L'unità indirizzabile più piccola di una traccia è un *settore*. Un *cilindro* è definito come il set di tracce che vengono visualizzate nella stessa posizione su ogni piatto. Ad esempio, il diagramma seguente mostra un disco rigido con quattro dischi. Il cilindro X è costituito da otto tracce (traccia X da ogni lato di ogni piatto).
...
Il primo settore fisico in un disco di base contiene una struttura di dati nota come master boot record (MBR). MBR contiene quanto segue:

-   Un programma di avvio (dimensioni massime di 442 byte)
