---
description: "Altre informazioni su: Uso dell'API Cabinet"
ms.assetid: facbcbee-46c2-4fb5-85f9-5d08244dcf86
title: Uso dell'API Cabinet
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: ea6604d080054decaff20a36dc3357dc9206053d3190abb6e39eff60d4ae1ffa
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 08/11/2021
ms.locfileid: "118404420"
---
# <a name="using-the-cabinet-api"></a>Uso dell'API Cabinet

Gli esempi seguenti illustrano l'implementazione dell'API Cabinet:

-   [Creazione di un archivio](creating-a-cabinet.md)
-   [Estrazione di file da un archivio](extracting-files-from-a-cabinet.md)

## <a name="related-topics"></a>Argomenti correlati

<dl> <dt>

[Informazioni di riferimento sulle API cabinet](cabinet-api-reference.md)
</dt> </dl>

 

 



