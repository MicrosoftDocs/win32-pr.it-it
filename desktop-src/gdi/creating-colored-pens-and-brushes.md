...
---
# <a name="creating-colored-pens-and-brushes"></a><span data-ttu-id="0cad4-103">Creazione di penne colorate e pennelli</span><span class="sxs-lookup"><span data-stu-id="0cad4-103">Creating Colored Pens and Brushes</span></span>

<span data-ttu-id="0cad4-104">Sebbene sia possibile specificare qualsiasi colore per una penna durante la creazione, il sistema usa solo i colori disponibili nel dispositivo.</span><span class="sxs-lookup"><span data-stu-id="0cad4-104">Although you can specify any color for a pen when creating it, the system uses only colors that are available on the device.</span></span> <span data-ttu-id="0cad4-105">Ciò significa che il sistema usa il colore corrispondente più vicino quando realizza la penna per il disegno.</span><span class="sxs-lookup"><span data-stu-id="0cad4-105">This means the system uses the closest matching color when it realizes the pen for drawing.</span></span> <span data-ttu-id="0cad4-106">Quando si creano i pennelli, il sistema genera un colore dithered se si specifica un colore non supportato dal dispositivo.</span><span class="sxs-lookup"><span data-stu-id="0cad4-106">When creating brushes, the system generates a dithered color if you specify a color that the device does not support.</span></span> <span data-ttu-id="0cad4-107">In entrambi i casi, è possibile usare la macro [**RGB**](/windows/desktop/api/Wingdi/nf-wingdi-rgb) per specificare un colore durante la creazione di una penna o di un pennello.</span><span class="sxs-lookup"><span data-stu-id="0cad4-107">In either case, you can use the [**RGB**](/windows/desktop/api/Wingdi/nf-wingdi-rgb) macro to specify a color when creating a pen or brush.</span></span>
