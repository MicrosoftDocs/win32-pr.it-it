...
La modalità di mapping predefinita è MM \_ TEXT. Un'unità logica è uguale a un pixel. Il valore x positivo è a destra e il positivo y è in basso. Questa modalità esegue il mapping direttamente al sistema di coordinate del dispositivo. Il mapping da logico a fisico prevede solo un offset in x e y definito dalle origini della finestra e del viewport controllate dall'applicazione. Gli extent del viewport e della finestra sono tutti impostati su 1, creando un mapping uno-a-uno.

Le applicazioni che visualizzano forme geometriche (cerchi, quadrati, poligoni e così via) usano una delle modalità di mapping indipendenti dal dispositivo. Ad esempio, se si scrive un'applicazione per fornire funzionalità di creazione di grafici per un foglio di calcolo e si vuole garantire che il diametro di ogni grafico a torta sia di 2 pollici, usare la modalità di mapping MM LOENGLISH e chiamare le funzioni appropriate per disegnare e riempire il \_ grafico. Specificando MM \_ LOENGLISH, si garantisce che il diametro del grafico sia coerente su qualsiasi schermo o stampante. Se si usa MM TEXT anziché MM LOENGLISH, un grafico che appare circolare su uno schermo VGA appare ellittico su uno schermo EGA e appare molto piccolo su una stampante a colori a \_ \_ 300 dpi (punti per pollice).
