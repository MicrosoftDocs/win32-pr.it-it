...
---
# <a name="using-wct"></a>Uso di WCT

Il codice di esempio seguente illustra l'uso dell'API di attraversamento della catena di attesa. Enumera tutti i thread del sistema e stampa la catena di attesa per ogni thread.

Per enumerare tutti i thread del sistema, eseguire l'esempio senza parametri. Per enumerare solo i thread di un processo specificato, eseguire l'esempio e passare l'identificatore del processo come parametro. Nell'esempio vengono eseguiti i passaggi seguenti:

1. Chiama la funzione [RegisterWaitChainCOMCallback](/windows/desktop/api/Wct/nf-wct-registerwaitchaincomcallback) per registrare le funzioni di callback com.
2. Chiama la funzione [OpenThreadWaitChainSession](/windows/desktop/api/Wct/nf-wct-openthreadwaitchainsession) per creare la sessione della catena di attesa.
3. Chiama la funzione [AdjustTokenPrivileges](/windows/win32/api/securitybaseapi/nf-securitybaseapi-adjusttokenprivileges) per abilitare il \_ privilegio del nome di debug se \_ .
4. Chiama le funzioni [EnumProcesses](/windows/win32/api/psapi/nf-psapi-enumprocesses) e [CreateToolhelp32Snapshot](/windows/win32/api/tlhelp32/nf-tlhelp32-createtoolhelp32snapshot) per enumerare i thread specificati.
5. Chiama [GetThreadWaitChain](/windows/desktop/api/Wct/nf-wct-getthreadwaitchain) per recuperare una matrice di strutture [di \_ \_ informazioni sui nodi WAITCHAIN](/windows/desktop/api/Wct/ns-wct-waitchain_node_info) che contengono i nodi della catena di attesa.
6. Stampa le informazioni dalla catena di attesa.
7. Chiama la funzione [CloseThreadWaitChainSession](/windows/desktop/api/Wct/nf-wct-closethreadwaitchainsession) per pulire la catena di attesa.
