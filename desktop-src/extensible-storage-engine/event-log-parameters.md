...
In questo argomento sono contenuti i parametri utilizzati per il registro eventi.

JET_paramEventLogCache  
Questo parametro controlla le dimensioni (in byte) di una cache dei messaggi del registro eventi che contenerà i messaggi del registro eventi generati dal motore di database mentre il servizio registro eventi viene arrestato. Questi messaggi memorizzati nella cache verranno scaricati nel registro eventi quando il servizio diventa operativo. Tutti i messaggi che causano un overflow della cache verranno eliminati.
...
Questo parametro configura il livello di dettaglio dei messaggi del registro eventi generati nel registro eventi dal motore di database. Numeri più elevati comportano messaggi di log eventi più dettagliati.


| Etichetta | Valore |
|--------|-------|
| <p>Valore predefinito:</p> | <p>JET_EventLoggingLevelMax</p> | 
| <p>Digitare:</p> | <p>Intero</p> | 
| <p>Intervallo valido:</p> | <p>JET_EventLoggingDisable: JET_EventLoggingLevelMax</p> | 
...
Questo parametro può essere utilizzato per controllare il registro eventi usato dal motore di database per i messaggi del registro eventi. Per impostazione predefinita, tutti i messaggi del registro eventi verranno inviati al registro eventi dell'applicazione. Se il nome della chiave del Registro di sistema per un altro registro eventi è configurato, i messaggi del registro eventi verranno inviati in questa pagina.
...
Quando questo parametro è true, i messaggi del log eventi informativi che in genere verrebbero generati dal motore di database verranno eliminati.
