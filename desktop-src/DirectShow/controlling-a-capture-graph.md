...
|-------------|----------------------------------------|------------------------------------|
|             | Inizia                                  | Stop                               |
| MAXLONGLONG | Non avviare mai questo flusso.               | Non arrestarsi fino a quando il grafico non si arresta. |
...
Per controllare il pin di anteprima, chiamare **ControlStream** ma impostare il primo parametro su PIN \_ CATEGORY \_ PREVIEW. Questa operazione funziona esattamente come per PIN CATEGORY CAPTURE, ad eccezione del fatto che non è possibile usare le ore di riferimento per specificare l'inizio e l'arresto, perché i fotogrammi di anteprima non hanno \_ \_ timestamp. Pertanto, è necessario utilizzare **NULL** o MAXLONGLONG. Usare **NULL per** avviare il flusso di anteprima:
