...
---
# <a name="displaying-net-events"></a><span data-ttu-id="baa63-103">Visualizzazione di eventi NET</span><span class="sxs-lookup"><span data-stu-id="baa63-103">Displaying Net Events</span></span>

<span data-ttu-id="baa63-104">Nell'esempio di codice riportato di seguito viene illustrato come un'applicazione server può individuare eventi NET recenti che potrebbero esserne interessati.</span><span class="sxs-lookup"><span data-stu-id="baa63-104">The following sample code demonstrates how a server application could locate recent net events that might have affected it.</span></span>

> [!Note]  
> <span data-ttu-id="baa63-105">Le condizioni di filtro sono le stesse supportate dall'API **IsPortAllowed** di livello inferiore.</span><span class="sxs-lookup"><span data-stu-id="baa63-105">The filter conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
