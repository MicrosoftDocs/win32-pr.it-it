...
Questo tipo è disponibile solo in C++. Il relativo equivalente .NET è [System. Numerics. Vector4](/dotnet/api/system.numerics.vector4?view=netframework-4.8).
...
| `float4(float2 value, float z, float w)` | Crea una float4 con x e y copiata da un float2 più i valori z e w specificati. |
| `float4(float3 value, float w)` | Crea un float4 con x, y e z copiati da un float3 più il valore w specificato. |
| `explicit float4(float value)` | Crea un float4 con tutti i set di impostazioni com. ent impostati sul valore specificato. |
...
| `float dot(float4 const& vector1, float4 const& vector2)` | Calcola il prodotto a punti di due vettori. |
...
| `float4 max(float4 const& value1, float4 const& value2)` | Restituisce un vettore che contiene il valore massimo di ogni coppia di componenti corrispondente. |
| `float4 clamp(float4 const& value1, float4 const& min, float4 const& max)` | Limita un valore all'interno di un intervallo specificato. |
...
| `float4 transform4(float3 const& value, quaternion const& rotation)` | Trasforma un float3 in base al quaternione specificato, restituendo un float4. |
| `float4 transform4(float2 const& value, quaternion const& rotation)` | Trasforma un float2 in base al quaternione specificato, restituendo un float4. |

## <a name="methods"></a>Metodi

| Nome | Descrizione |
|-|-|
| `static float4 zero()` | Restituisce un float4 con tutti i relativi componenti impostati su zero. |
...
| `float4 operator* (float4 const& value1, float4 const& value2)` | Moltiplica i componenti di due vettori. |
| `float4 operator* (float4 const& value1, float value2)` | Moltiplica un vettore per un valore scalare. |
| `float4 operator* (float value1, float4 const& value2)` | Moltiplica un vettore per un valore scalare. |
| `float4 operator/ (float4 const& value1, float4 const& value2)` | Divide i componenti di un vettore in base ai componenti di un altro vettore. |
...
| `float4& operator+= (float4& value1, float4 const& value2)` | Sul posto vengono aggiunti due vettori. |
| `float4& operator-= (float4& value1, float4 const& value2)` | Il sul posto sottrae un vettore da un vettore. |
| `float4& operator*= (float4& value1, float4 const& value2)` | Sul posto moltiplica i componenti di due vettori. |
| `float4& operator*= (float4& value1, float value2)` | Sul posto moltiplica un vettore per un valore scalare. |
| `float4& operator/= (float4& value1, float4 const& value2)` | Sul posto divide i componenti di un vettore in base ai componenti di un altro vettore. |
...
| `operator Microsoft::?Graphics::?Canvas::?Numerics::?Vector4() const` | Converte un float4 in un oggetto **Microsoft. graphics. Canvas. Numerics. Vector4**. |
