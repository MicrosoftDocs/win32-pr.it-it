---
title: Applicazioni
description: Applicazioni
ms.assetid: 1bc835a9-89a0-4f67-81ca-d9693dd1ddbf
keywords:
- Framework servizi di testo (TSF), applicazioni
- TSF (Framework di servizi di testo), applicazioni
- Applicazioni abilitate per TSF, informazioni
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 14a2ac9baac60726601c011ca8db146d849b7e6d
ms.sourcegitcommit: 2d531328b6ed82d4ad971a45a5131b430c5866f7
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 09/16/2019
ms.locfileid: "104221708"
---
# <a name="applications"></a>Applicazioni

Gli elementi di programmazione TSF seguenti vengono usati o implementati da un' [applicazione abilitata per TSF](architecture.md).

-   [Archivi di testo](text-stores.md)
-   [Blocchi del documento](document-locks.md)
-   [Utilizzo degli attributi di visualizzazione](using-display-attributes.md)
-   [Oggetti incorporati](embedded-objects.md)
-   [Barra della lingua](language-bar-app.md)

 

 




