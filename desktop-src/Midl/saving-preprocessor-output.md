...
description: La visualizzazione dell'output del preprocessore può essere un metodo efficace per la risoluzione dei problemi, ad esempio quando si verifica una sintassi IDL, C o del preprocessore C non valida o quando i valori -D non validi nella riga di comando MIDL generano errori in MIDL.
...
La visualizzazione dell'output del preprocessore può essere un metodo efficace per la risoluzione dei problemi, ad esempio quando si verifica una sintassi IDL, C o del preprocessore C non valida o quando i valori -D non validi nella riga di comando MIDL generano errori in MIDL. Gli sviluppatori possono anche voler esaminare l'output del preprocessore per determinare quali file e definizioni erano presenti nel flusso di input del compilatore, ad esempio quando è necessario risolvere un caso difficile di conflitto di ridefinizione del tipo. O meno comunemente, alcuni sviluppatori potrebbero voler forzare opzioni private nel preprocessore con [**/cpp \_ opt**](-cpp-opt.md)o potrebbe essere necessario vedere il funzionamento delle opzioni.

Il modo più semplice e meno intrusivo per ottenere i file pre-elaborato da un'esecuzione di compilazione MIDL è usare [**l'opzione -savePP.**](-savepp.md) **L'opzione -savePP** impedisce semplicemente a MIDL di eliminare i file temporanei passati dal preprocessore a MIDL. Considerare quanto segue:
