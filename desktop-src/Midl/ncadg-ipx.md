...
description: La parola chiave \_ ncadg ipx identifica IPX come famiglia di protocolli per l'endpoint. Questa famiglia di protocolli è obsoleta e non deve essere usata nelle nuove applicazioni.
...
La **parola chiave \_ ncadg ipx** identifica IPX come famiglia di protocolli per l'endpoint. Questa famiglia di protocolli è obsoleta e non deve essere usata nelle nuove applicazioni.
...
Specifica un numero a 16 bit facoltativo che rappresenta l'indirizzo del socket. I valori possono variare da 1 a 65535. Se non viene specificato alcun valore, il servizio di mapping degli endpoint seleziona un valore *di nome di porta* valido.

</dd> </dl>

## <a name="remarks"></a>Commenti

Le restrizioni seguenti si applicano ai protocolli di datagramma, **ncadg \_ ipx** e [**ncadg \_ ip \_ udp:**](ncadg-ip-udp.md)
...
Quando si usa **il trasporto \_ ncadg ipx,** il nome del server è esattamente uguale al nome del server Windows 32 bit. Tuttavia, poiché i nomi vengono distribuiti usando protocolli Novell, devono essere conformi alle convenzioni di denominazione di Novell. Se un nome di server non è un nome Novell valido, i server non saranno in grado di creare endpoint con il **trasporto \_ ipx ncadg.** Di seguito è riportato un elenco parziale di caratteri non consentiti nei nomi dei server Novell:

" \* + . / : ; < = > ? \[ \] \\ \|

Il **trasporto \_ ipx ncadg** è supportato dalla versione di NWLink fornita con MS Client 3.0.

Le applicazioni client Windows a 16 bit che usano il trasporto **\_ ipx ncadg** richiedono l'installazione del Nwipxspx.dll file per poter essere eseguite nel sottosistema WOW. Per ottenere questo file, contattare Novell.
