...
Valore long integer che specifica l'ID lingua per il carattere. L'ID lingua (LANGID) per un carattere è un valore a 16 bit definito da Windows, costituito da un ID di lingua primario e da un ID di lingua secondaria. È possibile usare i valori seguenti per le lingue specificate. Per ulteriori informazioni, vedere la documentazione di Platform SDK.
...
Se non si imposta l'ID lingua per il carattere, il relativo ID lingua sarà l'ID della lingua di sistema corrente se è installata la DLL della lingua dell'agente corrispondente. in caso contrario, la lingua del carattere sarà inglese (Stati Uniti).

Questa proprietà determina anche la lingua del testo del fumetto di parole, i comandi nel menu a comparsa del carattere e il motore di riconoscimento vocale. Consente inoltre di determinare la lingua predefinita per l'output TTS. Per determinare se è disponibile un motore di riconoscimento vocale compatibile per la lingua del carattere, usare [**IAgentCharacterEx:: GetSRModeID**](iagentcharacterex--getsrmodeid.md) o [**IAgentCharacterEx:: GetTTSModeID**](iagentcharacterex--getttsmodeid.md).

Se si tenta di impostare l'ID lingua per un carattere e le risorse della lingua dell'agente, la tabella codici o un tipo di carattere visualizzato per l'ID lingua non è disponibile, Agent restituisce un errore e l'ID lingua del carattere rimane all'ultima impostazione. L'impostazione di questa proprietà non restituisce un errore se non sono presenti motori di riconoscimento vocale corrispondenti per la lingua.
