...
Questo esempio di codice illustra il markup e il codice necessari per eseguire la migrazione di un'applicazione Microsoft Foundation Classes (MFC) esistente per usare la barra Windows multifunzione. È stato creato prendendo l'applicazione di esempio HTMLEdit MFC esistente e modificandone il codice e le risorse per usare la barra Windows multifunzione.
...
- Windows Software Development Kit (SDK) (percorso di installazione standard): %ProgramFiles% Microsoft SDK Windows numero di versione \\ \\ Esempi \\ \[ \] \\ \\ winui \\ WindowsRibbon \\ HTMLEditRibbon
