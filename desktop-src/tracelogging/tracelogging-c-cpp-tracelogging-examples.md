---
title: Esempi di Tracelogging C/C++
description: Questo argomento contiene esempi di Tracelogging in C/C++.
...
ms.locfileid: "104395726"
---
# <a name="cc-tracelogging-examples"></a><span data-ttu-id="f8de1-103">Esempi di Tracelogging C/C++</span><span class="sxs-lookup"><span data-stu-id="f8de1-103">C/C++ Tracelogging Examples</span></span>

<span data-ttu-id="f8de1-104">Questo argomento contiene esempi di Tracelogging in C/C++.</span><span class="sxs-lookup"><span data-stu-id="f8de1-104">This topic contains C/C++ Tracelogging examples.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="f8de1-105">Collegare Advapi32. lib durante la compilazione di questi esempi.</span><span class="sxs-lookup"><span data-stu-id="f8de1-105">Link advapi32.lib when compiling these examples.</span></span>

 

-   [<span data-ttu-id="f8de1-106">Registra tipi di dati intrinseci</span><span class="sxs-lookup"><span data-stu-id="f8de1-106">Log intrinsic data types</span></span>](#log-intrinsic-data-types)
-   [<span data-ttu-id="f8de1-107">Campi di evento nome</span><span class="sxs-lookup"><span data-stu-id="f8de1-107">Name event fields</span></span>](#name-event-fields)
-   [<span data-ttu-id="f8de1-108">Livello di dettaglio del log degli eventi</span><span class="sxs-lookup"><span data-stu-id="f8de1-108">Log events verbosity level</span></span>](#log-event-verbosity-level)
-   [<span data-ttu-id="f8de1-109">Parole chiave degli eventi di log</span><span class="sxs-lookup"><span data-stu-id="f8de1-109">Log events keywords</span></span>](#log-event-keywords)
-   [<span data-ttu-id="f8de1-110">Dati evento array di log</span><span class="sxs-lookup"><span data-stu-id="f8de1-110">Log array event data</span></span>](#log-array-event-data)
-   [<span data-ttu-id="f8de1-111">Dati evento struttura log</span><span class="sxs-lookup"><span data-stu-id="f8de1-111">Log structure event data</span></span>](#log-structure-event-data)

### <a name="log-intrinsic-data-types"></a><span data-ttu-id="f8de1-112">Registra tipi di dati intrinseci</span><span class="sxs-lookup"><span data-stu-id="f8de1-112">Log intrinsic data types</span></span>

<span data-ttu-id="f8de1-113">In questo esempio viene illustrato come registrare i tipi di dati intrinseci, ad esempio numeri interi, valori booleani e così via.</span><span class="sxs-lookup"><span data-stu-id="f8de1-113">This example shows how to log intrinsic data types such as integers, booleans, and so on.</span></span>
...
### <a name="name-event-fields"></a><span data-ttu-id="f8de1-114">Campi di evento nome</span><span class="sxs-lookup"><span data-stu-id="f8de1-114">Name event fields</span></span>

<span data-ttu-id="f8de1-115">Nell'esempio viene illustrato come assegnare un nome ai campi evento.</span><span class="sxs-lookup"><span data-stu-id="f8de1-115">The example shows how to name event fields.</span></span>
...
### <a name="log-event-verbosity-level"></a><span data-ttu-id="f8de1-116">Livello di dettaglio degli eventi di log</span><span class="sxs-lookup"><span data-stu-id="f8de1-116">Log event verbosity level</span></span>

<span data-ttu-id="f8de1-117">Questo esempio Mostra come registrare gli eventi in base al livello di dettaglio.</span><span class="sxs-lookup"><span data-stu-id="f8de1-117">This example shows how to log events by verbosity level.</span></span>
...
### <a name="log-event-keywords"></a><span data-ttu-id="f8de1-118">Parole chiave degli eventi di log</span><span class="sxs-lookup"><span data-stu-id="f8de1-118">Log event keywords</span></span>

<span data-ttu-id="f8de1-119">Questo esempio illustra come impostare le parole chiave degli eventi.</span><span class="sxs-lookup"><span data-stu-id="f8de1-119">This example illustrates how to set event keywords.</span></span> <span data-ttu-id="f8de1-120">Il filtraggio degli eventi può essere eseguito tramite il livello e la parola chiave.</span><span class="sxs-lookup"><span data-stu-id="f8de1-120">Event filtering can be done by level and keyword.</span></span> <span data-ttu-id="f8de1-121">Gli eventi di completamento dello scenario o gli eventi di errore possono ad esempio essere raggruppati in parole chiave separate, in modo da poter filtrare facilmente tali eventi.</span><span class="sxs-lookup"><span data-stu-id="f8de1-121">For example, scenario completion events or failure events might be grouped under separate keywords so that you can easily filter those events.</span></span>
...
### <a name="log-array-event-data"></a><span data-ttu-id="f8de1-122">Dati evento array di log</span><span class="sxs-lookup"><span data-stu-id="f8de1-122">Log array event data</span></span>

<span data-ttu-id="f8de1-123">In questo esempio viene illustrato come registrare le matrici come dati dell'evento.</span><span class="sxs-lookup"><span data-stu-id="f8de1-123">This example shows how to log arrays as event data.</span></span>
...
### <a name="log-structure-event-data"></a><span data-ttu-id="f8de1-124">Dati evento struttura log</span><span class="sxs-lookup"><span data-stu-id="f8de1-124">Log structure event data</span></span>

<span data-ttu-id="f8de1-125">In questo esempio viene illustrato come registrare i dati degli eventi della struttura.</span><span class="sxs-lookup"><span data-stu-id="f8de1-125">This example shows how to log structure event data.</span></span>
...
### <a name="how-to-fix-tracelogging-cc-build-errors"></a><span data-ttu-id="f8de1-126">Come correggere gli errori di compilazione in C/C++ TraceLogging</span><span class="sxs-lookup"><span data-stu-id="f8de1-126">How to fix TraceLogging C/C++ build errors</span></span>

<span data-ttu-id="f8de1-127">Errore di compilazione</span><span class="sxs-lookup"><span data-stu-id="f8de1-127">Build Error</span></span>
...
<span data-ttu-id="f8de1-128">Fix</span><span class="sxs-lookup"><span data-stu-id="f8de1-128">Fix</span></span>

<span data-ttu-id="f8de1-129">Per correggere l'errore, collegare Advapi32. lib nel progetto.</span><span class="sxs-lookup"><span data-stu-id="f8de1-129">Fix this error by linking advapi32.lib in your project.</span></span>
