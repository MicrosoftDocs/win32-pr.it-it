...
description: Le costanti di sessione nell'enumerazione WSManSessionFlags specificano l'autenticazione e altre informazioni per le chiamate \_ CreateSession WSMan.CreateSession o IWSMan che si connettono \_ a un computer remoto.
...
## <a name="using-session-constants"></a>Uso delle costanti di sessione

È possibile impostare i flag di sessione per la chiamata a [**WSMan.CreateSession**](wsman-createsession.md) in due modi diversi. Uno è più breve e più semplice. Il modo più lungo, come illustrato nell'esempio seguente, è individuare il valore del flag che si vuole usare e creare una costante nello script con tale valore. La costante viene quindi usata per impostare il valore del *parametro iFlags.*
...
Il modo consigliato, come illustrato nell'esempio seguente, è usare il metodo [**dell'oggetto WSMan**](wsman.md) associato al flag .
...
Specificare il metodo di autenticazione e la modalità di gestione dei server dei certificati.
