---
description: Impostando la proprietà Factoid su Nessuno, il riconoscimento dei caratteri riconosce la grafia in base al carattere.
...
Impostando la [proprietà Factoid](/previous-versions/ms835848(v=msdn.10)) su **Nessuno,** il riconoscimento dei caratteri riconosce la grafia carattere per carattere.

La [proprietà RecoTimeout](/previous-versions/ms835852(v=msdn.10)) [**(RecoTimeout**](/windows/desktop/api/inked/nf-inked-iinkedit-get_recognitiontimeout) in Automazione) specifica il numero di millisecondi di ritardo tra l'ultimo [tratto](/previous-versions/ms552692(v=vs.100)) e la fine dell'input di scrittura manuale. È possibile aumentare questo valore per riconoscere il testo prima che siano scritte intere parole o frasi. È anche possibile forzare immediatamente il riconoscimento dell'input penna usando il [metodo Recognize.](/previous-versions/ms836275(v=msdn.10))
