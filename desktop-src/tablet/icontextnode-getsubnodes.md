...
*ppSubContextNodes* \[ Cambio\]
</dt> <dd>

Raccolta di oggetti [**IContextNode**](icontextnode.md) che sono nodi figlio diretti di **questo oggetto IContextNode.**
...
In questo esempio viene illustrato un metodo `ExploreContextNode` , che esamina un oggetto [**IContextNode**](icontextnode.md). Il metodo esegue le operazioni seguenti:

-   Ottiene il tipo del nodo di contesto.
-   Esamina proprietà specifiche del tipo di nodo chiamando un metodo helper, se il nodo di contesto è un input penna non classificato, un hint di analisi o un nodo di riconoscimento personalizzato.
-   Esamina ogni sottonodo chiamando se stesso, se il nodo ha sottonodi.
