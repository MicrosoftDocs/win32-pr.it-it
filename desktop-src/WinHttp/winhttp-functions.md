...
WinHTTP fornisce le funzioni seguenti:
...
Separa un URL nelle parti del componente, ad esempio il nome host e il percorso.
...
Crea un handle per l'uso da [**WinHttpGetProxyForUrlEx**](/windows/desktop/api/Winhttp/nf-winhttp-winhttpgetproxyforurlex).
...
Trova l'URL per il file di configurazione automatica del proxy (PAC). Questa funzione segnala l'URL del file PAC, ma il file non viene scaricato.
...
Ottiene la configurazione del proxy Internet Explorer (IE) per l'utente corrente.
...
Recupera le informazioni sul proxy per l'URL specificato.
...
Inizializza l'utilizzo delle funzioni WinHTTP da parte di un'applicazione.
...
Restituisce il numero di byte di dati disponibili immediatamente per la lettura con [**WinHttpReadData**](/windows/desktop/api/Winhttp/nf-winhttp-winhttpreaddata).
...
Imposta una funzione di callback che può essere chiamata da WinHTTP come avanzamento durante un'operazione.
...
Imposta i diversi timeout che interessano le transazioni HTTP.
