...
keywords:
- Windows Media Format SDK, oggetti di registrazione del dispositivo
...
L'oggetto di registrazione del dispositivo gestisce il database di registrazione del dispositivo. Questo database archivia informazioni sui dispositivi di rete, ad esempio lettori video set-top, connessi al computer client. Lo scopo principale del database di registrazione dei dispositivi è gestire i dispositivi che usano il protocollo Windows Media DRM 10 per i dispositivi di rete per ricevere flussi di dati protetti.

Se l'applicazione supporta Windows Media DRM 10 per i dispositivi di rete, è necessario usare le interfacce di questo oggetto per registrare i dispositivi di rete e convalidare tali dispositivi. È anche possibile usare il database di registrazione dei dispositivi per archiviare le informazioni sui dispositivi di rete che non usano Windows Media DRM 10 per dispositivi di rete, anche se non tutte le informazioni nel database verranno applicate a tali dispositivi.

L'oggetto di registrazione del dispositivo viene creato dalla [**funzione WMCreateDeviceRegistration,**](/previous-versions/windows/desktop/api/Wmsdkidl/nf-wmsdkidl-wmcreatedeviceregistration) che imposta un puntatore a **un'interfaccia IWMDeviceRegistration.** Gli altri metodi dell'oggetto registrazione del dispositivo possono essere ottenuti chiamando il **metodo QueryInterface.**
