...
description: Il software di installazione per la DLL EAP può creare i valori del Registro di sistema seguenti sotto eaptypeid.
...
ms.locfileid: "119739561"
---
# <a name="authentication-protocol-registry-values"></a>Valori del Registro di sistema del protocollo di autenticazione

Il software di installazione per la DLL EAP può creare i valori del Registro di sistema seguenti **&lt; sotto eaptypeid &gt;**. Questi valori del Registro di sistema sono definiti nel file di intestazione Raseapif.h. I **RAS_EAP_VALUENAME_PATH** e **RAS_EAP_VALUENAME_FRIENDLY_NAME** sono obbligatori. Il software di installazione può creare anche altre chiavi e valori. Possono essere usati dal protocollo di autenticazione stesso. Per altre informazioni e un esempio di configurazione del Registro di sistema, vedere [Esempio di valori del Registro di sistema.](registry-values-example.md)
...
[RAS_EAP_VALUENAME_INTERACTIVEUI](#ras_eap_valuename_interactiveui) Ho
...
|----------------|-----------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_EXPAND_SZ                                                                                                                   |
| Descrizione    | Specifica il percorso della DLL che implementa l'interfaccia utente di configurazione nel client, perché l'interfaccia utente è solo per il client. |
...
|----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                                                                                               |
| Descrizione    | Specifica se l'utente deve fornire i dati di configurazione nell'interfaccia utente dell'applicazione client EAP. Se questo valore è 1, l'utente non può uscire dall'interfaccia utente dell'applicazione client EAP senza fornire dati di configurazione. Il valore predefinito è 0. |
...
|----------------|----------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_EXPAND_SZ                                                                                                                        |
| Descrizione    | Specifica il percorso della DLL che implementa le funzioni per ottenere l'identità utente nel client, perché questa interfaccia utente è solo per il client. |
...
|----------------|---------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_EXPAND_SZ                                                                                                                 |
| Descrizione    | Specifica il percorso della DLL che implementa l'interfaccia utente interattiva nel client, perché l'interfaccia utente è solo per il client. |
...
|----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                                             |
| Descrizione    | Specifica se RAS deve visualizzare la finestra di dialogo del nome utente standard Windows NT/Windows 2000 (valore 1) o richiamare [**RasEapGetIdentity**](/previous-versions/windows/desktop/api/Raseapif/nf-raseapif-raseapgetidentity) (valore 0). Il valore predefinito è 1. |
...
|----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                                  |
| Descrizione    | Specifica se RAS deve visualizzare la finestra di dialogo Windows NT/Windows 2000 password standard. Se questo valore esiste e è 0, RAS non visualizza la finestra di dialogo della password. Il valore predefinito è 1. |
...
|----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                                    |
| Descrizione    | Se questo valore è 1, il protocollo di autenticazione può generare chiavi per lo stile di crittografia MPPE (Microsoft Point-to-Point Encryption). I valori possibili sono 0 o 1. Il valore predefinito è 0. |
...
|----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                     |
| Descrizione    | Specifica se questo protocollo di autenticazione è supportato in un server Windows 2000 Server. Il valore 0 indica che il protocollo EAP non è supportato. Il valore predefinito è 1. |

## <a name="ras_eap_valuename_roles_supported"></a>RAS_EAP_VALUENAME_ROLES_SUPPORTED

| Constant Value | Ruoli Supportati                                                                                                                                                                                                                             |
|----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tipo           | REG_DWORD                                                                                                                                                                                                                                 |
| Descrizione    | Specifica i vari ruoli supportati da EAP. Indica se può essere usato in un server o in un client, se è supportato per le connessioni RAS (VPN) o PEAP. Il comportamento predefinito è visualizzare il metodo EAP in PEAP e in EAP. |
...
Questo valore del Registro di sistema non è in uso.
