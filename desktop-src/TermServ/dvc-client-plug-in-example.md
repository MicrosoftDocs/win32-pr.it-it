---
title: Esempio di plug-in client DVC
description: Esempio di codice C++ in cui viene illustrato come creare un plug-in del canale virtuale dinamico (DVC) client di Connessione Desktop remoto (RDC).
...
ms.locfileid: "103955335"
---
# <a name="dvc-client-plug-in-example"></a><span data-ttu-id="e50e0-103">Esempio di plug-in client DVC</span><span class="sxs-lookup"><span data-stu-id="e50e0-103">DVC Client Plug-in Example</span></span>

<span data-ttu-id="e50e0-104">Nell'esempio di codice C++ riportato di seguito viene fornito un esempio di creazione di un plug-in di un canale virtuale dinamico (DVC) client Connessione Desktop remoto (RDC).</span><span class="sxs-lookup"><span data-stu-id="e50e0-104">The following C++ code sample provides an example of how to create a Remote Desktop Connection (RDC) client dynamic virtual channel (DVC) plug-in.</span></span>
