...
- Il metodo ResumeService Servizi Desktop remoto , Win32_Service classe
...
Il metodo della classe [WMI](/windows/desktop/WmiSdk/retrieving-a-class) **ResumeService** tenta di posizionare il servizio a cui si fa riferimento nello stato ripreso.
...
Anche se potrebbe sembrare che non vi sia alcuna differenza pratica tra un servizio arrestato e un servizio in pausa, i due stati appaiono in modo diverso rispetto a Gestione controllo servizi. Un servizio arrestato è un servizio che non è in esecuzione e deve eseguire l'intera procedura di avvio del servizio. Un servizio sospeso, tuttavia, è ancora in esecuzione, ma il suo funzionamento è sospeso. Per questo problema, un servizio sospeso non deve eseguire l'intera procedura di avvio del servizio, ma richiede una procedura diversa per riprendere il funzionamento.

È necessario utilizzare il metodo appropriato per avviare un servizio che è stato arrestato o per riprendere un servizio che è stato sospeso. I [**metodi \_ startService**](/windows/desktop/CIMWin32Prov/win32-service) e **ResumeService** del servizio Win32 devono essere usati nelle situazioni seguenti: [](win32-terminalservice-startservice.md)
...
-   Se un servizio viene sospeso, è necessario usare **ResumeService**. Se si usa il [**metodo StartService**](win32-terminalservice-startservice.md) in un servizio sospeso, viene visualizzato il messaggio "Il servizio è già in esecuzione". Tuttavia, il servizio rimane in pausa fino a quando non viene inviato il codice di controllo del servizio di ripresa.
