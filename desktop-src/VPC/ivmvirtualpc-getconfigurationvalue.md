...
Valore di preferenza. Questo parametro può essere uno dei tipi **Variant** seguenti: VT **\_ Array** VT \| **\_ Ui1** (byte non elaborati), **VT \_ BSTR** (String) **, VT \_ I4** (Integer) o **VT \_ bool** (Boolean).
...
Questo metodo fornisce accesso di basso livello a qualsiasi valore di preferenza per l'utente corrente. Può essere usato per recuperare i valori di preferenza per le chiavi definite dal cliente.
