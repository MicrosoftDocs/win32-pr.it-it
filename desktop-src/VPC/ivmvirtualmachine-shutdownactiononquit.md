...
Specifica come arrestare questa macchina virtuale se è in esecuzione quando viene chiuso il PC virtuale di Windows. Per un elenco di valori, vedere [**VMShutdownAction**](vmshutdownaction.md).
...
Per impostazione predefinita, le macchine virtuali in esecuzione vengono salvate alla chiusura di Windows Virtual PC. L'azione di arresto **vmShutdownAction \_ Save** (0) salverà lo stato della macchina virtuale. L'azione **vmShutdownAction \_ deviazione** (1) disattiva la macchina virtuale. L'azione di **\_ arresto vmShutdownAction** (2) arresterà il sistema operativo guest se i componenti di integrazione sono disponibili e salverà la macchina virtuale in caso contrario.
