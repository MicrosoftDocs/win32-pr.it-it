...
keywords:
- Abilitazione dell'account del servizio per l'accesso alle proprietà di SCP AD
ms.topic: article
ms.date: 05/31/2018
...
Nell'esempio di codice seguente viene impostata una coppia di voci di controllo di accesso (ACE) in un oggetto punto di connessione del servizio (SCP). Le voci ACE concedono l'accesso in lettura/scrittura all'account utente o computer con cui verrà eseguita l'istanza del servizio. Il programma di installazione del servizio usa codice simile al seguente per garantire che il servizio possa aggiornarne le proprietà in fase di esecuzione. Se le ACE simili a queste non sono impostate, il servizio non avrà accesso alle proprietà del punto di connessione del servizio.

In genere, queste voci ACE vengono impostate da un programma di installazione del servizio dopo la creazione dell'oggetto SCP. Per altre informazioni e un esempio di codice che crea un punto di connessione del servizio e chiama questa funzione, vedere Come i client trovano e usano [un punto di connessione del servizio.](how-clients-find-and-use-a-service-connection-point.md) Se il servizio viene riconfigurato per l'esecuzione con un account diverso, è necessario aggiornare le voci ACE. Per una corretta esecuzione, questo esempio di codice deve essere eseguito nel contesto di sicurezza di un amministratore di dominio.

Il primo parametro della funzione di esempio specifica il nome dell'account utente a cui concedere l'accesso. La funzione presuppone che il nome sia in formato *Dominio* **\\** _UserName._ Se non viene specificato alcun account, la funzione presuppone che il servizio usi l'account LocalSystem. Ciò significa che la funzione deve concedere l'accesso all'account computer del server host in cui è in esecuzione il servizio. A tale scopo, l'esempio di codice chiama la [**funzione GetComputerObjectName**](/windows/desktop/api/secext/nf-secext-getcomputerobjectnamea) per ottenere il dominio e il nome utente del computer locale.

L'esempio di codice seguente può essere modificato per concedere al servizio l'accesso completo all'oggetto SCP, ma la procedura consigliata consiste nel concedere solo i diritti di accesso specifici necessari al servizio in fase di esecuzione. In questo caso, la funzione concede l'accesso a due proprietà.
...
| [**serviceBindingInformation**](/windows/desktop/ADSchema/a-servicebindinginformation) | Informazioni di associazione private aggiornate dal servizio all'avvio. |



 

Ogni proprietà è identificata da **schemaIDGUID** della classe **attributeSchema della** proprietà. Ogni proprietà nello schema ha un proprio **schemaIDGUID univoco.** Nell'esempio di codice seguente vengono utilizzate stringhe per specificare i GUID. Le stringhe GUID hanno il formato seguente, in cui ogni "X" viene sostituita da una cifra esadecimale: {XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX}.

Fare riferimento alle pagine di riferimento dello schema di Active Directory per i **valori schemaIDGUID** assegnati alle proprietà a cui concedere o negare l'accesso.
...
2.  Impostare le voci ACE appropriate nell'elenco DICL (Discretionary Access Control List) del descrittore di sicurezza.
