...
In genere, un programma di installazione del servizio imposterà queste voci ACE dopo la creazione dell'oggetto SCP. Per altre informazioni e un esempio di codice che crea un SCP e chiama questa funzione, vedere Come i client trovano e usano [un punto di connessione del servizio](how-clients-find-and-use-a-service-connection-point.md). Se il servizio viene riconfigurato per l'esecuzione con un account diverso, è necessario aggiornare le voci ACE. Per essere eseguito correttamente, questo esempio di codice deve essere eseguito nel contesto di sicurezza di un amministratore di dominio.

Il primo parametro della funzione di esempio specifica il nome dell'account utente a cui viene concesso l'accesso. La funzione presuppone che il nome sia in formato *Domain* **\\** _UserName._ Se non viene specificato alcun account, la funzione presuppone che il servizio usi l'account LocalSystem. Ciò significa che la funzione deve concedere l'accesso all'account computer del server host in cui è in esecuzione il servizio. A tale scopo, l'esempio di codice chiama la [**funzione GetComputerObjectName**](/windows/desktop/api/secext/nf-secext-getcomputerobjectnamea) per ottenere il dominio e il nome utente del computer locale.

L'esempio di codice seguente può essere modificato per concedere al servizio l'accesso completo all'oggetto SCP, ma la procedura consigliata consiste nel concedere solo i diritti di accesso specifici che il servizio richiede in fase di esecuzione. In questo caso, la funzione concede l'accesso a due proprietà.
...
| [**serviceBindingInformation**](/windows/desktop/ADSchema/a-servicebindinginformation) | Informazioni sull'associazione privata aggiornate dal servizio all'avvio. |
