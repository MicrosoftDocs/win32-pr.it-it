...
---
# <a name="example-code-to-retrieve-changes-using-usnchanged"></a><span data-ttu-id="c75f5-104">Codice di esempio per recuperare le modifiche tramite USNChanged</span><span class="sxs-lookup"><span data-stu-id="c75f5-104">Example Code to Retrieve Changes Using USNChanged</span></span>

<span data-ttu-id="c75f5-105">Nell'esempio di codice seguente viene usato l'attributo **uSNChanged** di un oggetto in Active Directory Domain Services per recuperare le modifiche da una query precedente.</span><span class="sxs-lookup"><span data-stu-id="c75f5-105">The following code example uses the **uSNChanged** attribute of an object in Active Directory Domain Services to retrieve changes since a previous query.</span></span> <span data-ttu-id="c75f5-106">Nell'esempio di codice può essere eseguita una sincronizzazione completa o un aggiornamento incrementale.</span><span class="sxs-lookup"><span data-stu-id="c75f5-106">The code example can perform either a full synchronization or an incremental update.</span></span> <span data-ttu-id="c75f5-107">Per una sincronizzazione completa, l'applicazione di esempio si connette al rootDSE di un controller di dominio e legge i parametri seguenti archiviati per l'utilizzo nella successiva sincronizzazione incrementale:</span><span class="sxs-lookup"><span data-stu-id="c75f5-107">For a full synchronization, the sample application connects to the rootDSE of a domain controller and reads the following parameters that it stores to be used in the next incremental synchronization:</span></span>

-   <span data-ttu-id="c75f5-108">Nome DNS del controller di dominio.</span><span class="sxs-lookup"><span data-stu-id="c75f5-108">The DNS name of the DC.</span></span> <span data-ttu-id="c75f5-109">Le sincronizzazioni incrementali devono essere eseguite nello stesso controller di dominio della sincronizzazione precedente.</span><span class="sxs-lookup"><span data-stu-id="c75f5-109">Incremental synchronizations must be performed on the same DC as the previous synchronization.</span></span>
-   <span data-ttu-id="c75f5-110">GUID **invocationID** del controller di dominio.</span><span class="sxs-lookup"><span data-stu-id="c75f5-110">The **invocationID** GUID of the DC.</span></span> <span data-ttu-id="c75f5-111">Nell'esempio di codice viene utilizzato questo valore per rilevare che il controller di dominio è stato ripristinato da un backup, nel qual caso l'esempio deve eseguire una sincronizzazione completa.</span><span class="sxs-lookup"><span data-stu-id="c75f5-111">The code example uses this value to detect that the DC has been restored from a backup, in which case, the sample must perform a full synchronization.</span></span>
-   <span data-ttu-id="c75f5-112">**HighestCommittedUSN**.</span><span class="sxs-lookup"><span data-stu-id="c75f5-112">The **highestCommittedUSN**.</span></span> <span data-ttu-id="c75f5-113">Questo valore diventa il limite inferiore per il filtro **uSNChanged** alla successiva sincronizzazione incrementale.</span><span class="sxs-lookup"><span data-stu-id="c75f5-113">This value becomes the lower bound for the **uSNChanged** filter on the next incremental synchronization.</span></span>

<span data-ttu-id="c75f5-114">Nell'esempio di codice viene utilizzata l'interfaccia [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) , specificando il nome distinto della base della ricerca, un ambito di ricerca e un filtro.</span><span class="sxs-lookup"><span data-stu-id="c75f5-114">The code example uses the [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) interface, specifying the distinguished name of the base of the search, a search scope, and a filter.</span></span> <span data-ttu-id="c75f5-115">Non esistono restrizioni per la base o l'ambito di ricerca.</span><span class="sxs-lookup"><span data-stu-id="c75f5-115">There are no restrictions on the search base or scope.</span></span> <span data-ttu-id="c75f5-116">Oltre a specificare gli oggetti di interesse, è necessario che il filtro specifichi anche un confronto **uSNChanged** , ad esempio "uSNChanged >= <lower bound USN> ".</span><span class="sxs-lookup"><span data-stu-id="c75f5-116">In addition to specifying the objects of interest, the filter must also specify a **uSNChanged** comparison, such as "uSNChanged >= <lower bound USN>".</span></span> <span data-ttu-id="c75f5-117">Per una sincronizzazione completa, " &lt; USN limite inferiore &gt; " è zero.</span><span class="sxs-lookup"><span data-stu-id="c75f5-117">For a full synchronization, "&lt;lower bound USN&gt;" is zero.</span></span> <span data-ttu-id="c75f5-118">Per una sincronizzazione incrementale, è il valore 1 più il valore **highestCommittedUSN** della ricerca precedente.</span><span class="sxs-lookup"><span data-stu-id="c75f5-118">For an incremental synchronization, it is the 1 plus the **highestCommittedUSN** value from the previous search.</span></span>

<span data-ttu-id="c75f5-119">Tenere presente che questa applicazione di esempio è destinata solo a mostrare come usare **uSNChanged** per recuperare le modifiche dal server Active Directory.</span><span class="sxs-lookup"><span data-stu-id="c75f5-119">Be aware that this sample application is intended only to show how to use **uSNChanged** to retrieve changes from the Active Directory server.</span></span> <span data-ttu-id="c75f5-120">Stampa le modifiche e non sincronizza effettivamente i dati in una risorsa di archiviazione secondaria.</span><span class="sxs-lookup"><span data-stu-id="c75f5-120">It prints the changes and does not actually synchronize the data in a secondary storage.</span></span> <span data-ttu-id="c75f5-121">Di conseguenza, non viene illustrato come gestire problemi come gli oggetti spostati o le condizioni "nessun elemento padre".</span><span class="sxs-lookup"><span data-stu-id="c75f5-121">Consequently, it does not show how to handle issues like moved objects or "no parent" conditions.</span></span> <span data-ttu-id="c75f5-122">Viene illustrato come recuperare gli oggetti eliminati, ma non viene illustrato il modo in cui un'applicazione utilizza **objectGUID** degli oggetti eliminati per determinare l'oggetto corrispondente da eliminare nell'archivio.</span><span class="sxs-lookup"><span data-stu-id="c75f5-122">It does show how to retrieve deleted objects, but it does not show how an application uses the **objectGUID** of the deleted objects to determine the corresponding object to delete in the storage.</span></span>

<span data-ttu-id="c75f5-123">Inoltre, l'esempio memorizza nella cache il nome del controller di dominio, **invocationID** e **highestCommittedUSN** nel registro di sistema.</span><span class="sxs-lookup"><span data-stu-id="c75f5-123">Also, the sample caches the DC name, **invocationID**, and **highestCommittedUSN** in the registry.</span></span> <span data-ttu-id="c75f5-124">In un'applicazione di sincronizzazione reale è necessario archiviare i parametri nello stesso spazio di archiviazione mantenuto coerente con il server Active Directory.</span><span class="sxs-lookup"><span data-stu-id="c75f5-124">In a real synchronization application, you must store the parameters in the same storage that are kept consistent with the Active Directory server.</span></span> <span data-ttu-id="c75f5-125">In questo modo si garantisce che i parametri e i dati oggetto rimangano sincronizzati se il database viene ripristinato da un backup.</span><span class="sxs-lookup"><span data-stu-id="c75f5-125">This ensures that the parameters and object data remain synchronized if your database is ever restored from a backup.</span></span>
