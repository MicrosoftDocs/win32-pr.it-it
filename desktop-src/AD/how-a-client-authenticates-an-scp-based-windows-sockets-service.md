...
---
# <a name="how-a-client-authenticates-an-scp-based-windows-sockets-service"></a>Come un client autentica un servizio Windows Sockets basato su SCP

In questo argomento viene illustrato il codice utilizzato da un'applicazione client per comporre un nome SPN per un servizio. Il client viene associato al punto di connessione del servizio (SCP) del servizio per recuperare i dati necessari per la connessione al servizio. SCP contiene inoltre i dati che il client può usare per comporre il nome SPN del servizio. Per ulteriori informazioni e un esempio di codice associato al punto di connessione del servizio e per recuperare le proprietà necessarie, vedere [modalità di individuazione e utilizzo di un punto di connessione del servizio](how-clients-find-and-use-a-service-connection-point.md)da parte dei client.

In questo argomento viene inoltre illustrato come un client utilizza un pacchetto di sicurezza SSPI e il nome SPN del servizio per stabilire una connessione autenticata reciprocamente al servizio Windows Sockets. Tenere presente che questo codice è quasi identico al codice necessario in Microsoft Windows NT 4,0 e versioni precedenti solo per autenticare il client nel server. L'unica differenza è che il client deve fornire il nome SPN e specificare il \_ \_ flag di autenticazione reciproca req di ISC \_ .
...
Questo esempio di codice è costituito da due routine: **DoAuthentication** e **GenClientContext**. Dopo la chiamata di [**DsMakeSpn**](/windows/desktop/api/Dsparse/nf-dsparse-dsmakespna) per comporre un nome SPN per il servizio, il client passa il nome SPN alla routine **DoAuthentication** , che chiama **GenClientContext** per generare il buffer iniziale da inviare al servizio. **DoAuthentication** utilizza l'handle socket per inviare il buffer e ricevere la risposta del servizio passata al pacchetto SSPI da un'altra chiamata a **GenClientContext**. Questo ciclo viene ripetuto fino a quando l'autenticazione non riesce o **GenClientContext** imposta un flag che indica che l'autenticazione è stata completata.
...
-   Credenziali del client. La funzione [**AcquireCredentialsHandle**](../SecAuthN/acquirecredentialshandle--general.md) del pacchetto di sicurezza estrae queste credenziali dal contesto di sicurezza del client stabilito al momento dell'accesso.
-   Per richiedere l'autenticazione reciproca, il client deve specificare il flag di autenticazione \_ \_ reciproca req di ISC \_ quando chiama la funzione [**InitializeSecurityContext**](../SecAuthN/initializesecuritycontext--general.md) durante la routine **GenClientContext** .
