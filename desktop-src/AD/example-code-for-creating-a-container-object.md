...
---
# <a name="example-code-for-creating-a-container-object"></a>Codice di esempio per la creazione di un oggetto contenitore

Nell'esempio di codice seguente vengono creati due oggetti. Per prima cosa, un oggetto contenitore e, secondo, un oggetto contenitore secondario. Un valore per l'oggetto contenitore secondario viene aggiunto alla proprietà [**otherWellKnownObjects archiviato nel**](/windows/desktop/ADSchema/a-otherwellknownobjects) dell'oggetto contenitore. L'esempio di codice viene associato all'oggetto sottocontenitore usando l'associazione WKGUID e ne Visualizza il ADsPath. Viene quindi rinominato l'oggetto del sottocontenitore e associato nuovamente utilizzando la stessa associazione WKGUID.
