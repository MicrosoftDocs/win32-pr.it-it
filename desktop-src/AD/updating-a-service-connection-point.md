...
---
# <a name="updating-a-service-connection-point"></a>Aggiornamento di un punto di connessione del servizio

Nell'esempio di codice seguente viene illustrato come aggiornare un punto di connessione del servizio. Questo codice viene in genere eseguito da un servizio all'avvio.

Questo esempio recupera la stringa di binding GUID SCP che il programma di installazione del servizio ha memorizzato nella cache nel registro di sistema. Usa questa stringa per eseguire il binding a un puntatore [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) nell'oggetto SCP, quindi chiama il metodo [**IDirectoryObject:: GetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) per ottenere gli attributi **serviceDNSName** e **serviceBindingInformation** del SCP. Tenere presente che il servizio potrebbe richiedere di verificare e aggiornare attributi aggiuntivi.

Il codice confronta il valore **serviceDNSName** con il nome DNS restituito dalla funzione [**presunto GetComputerNameEx**](/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getcomputernameexa) . Viene inoltre confrontato il numero di porta corrente del servizio con il numero di porta archiviato nell'attributo **serviceBindingInformation** . Se uno di questi valori è stato modificato, il codice chiama il metodo [**IDirectoryObject:: SetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-setobjectattributes) per aggiornare gli attributi SCP.
