---
title: Codice di esempio per l'implementazione di un servizio Winsock con una pubblicazione RnR
description: Nell'esempio di codice seguente viene implementato il servizio Winsock di esempio con la pubblicazione RnR.
ms.assetid: 8800ba76-f24c-4aa7-ba31-97eaf884130c
ms.tgt_platform: multiple
keywords:
- Active Directory per la registrazione e la risoluzione di Windows Sockets, codice di esempio, implementazione di un servizio Winsock con una pubblicazione RnR
- Implementazione di un servizio Winsock con un annuncio di pubblicazione RnR, codice di esempio
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103707648"
---
# <a name="example-code-for-implementing-a-winsock-service-with-an-rnr-publication"></a><span data-ttu-id="3b2e1-105">Codice di esempio per l'implementazione di un servizio Winsock con una pubblicazione RnR</span><span class="sxs-lookup"><span data-stu-id="3b2e1-105">Example Code for Implementing a Winsock Service with an RnR Publication</span></span>

<span data-ttu-id="3b2e1-106">Nell'esempio di codice seguente viene implementato il servizio Winsock di esempio con la pubblicazione RnR.</span><span class="sxs-lookup"><span data-stu-id="3b2e1-106">The following code example implements the example Winsock service with RnR publication.</span></span>

<span data-ttu-id="3b2e1-107">Questo esempio usa la funzione **serverRegister** definita nel [codice di esempio per la pubblicazione dell'argomento punto di connessione RNR](example-code-for-publishing-the-rnr-connection-point.md) e la funzione **ServerUnregister** definita nel [codice di esempio per la rimozione dell'argomento punto di connessione RNR](example-code-for-removing-the-rnr-connection-point.md) .</span><span class="sxs-lookup"><span data-stu-id="3b2e1-107">This sample uses the **serverRegister** function defined in the [Example Code for Publishing the RnR Connection Point](example-code-for-publishing-the-rnr-connection-point.md) topic and the **serverUnregister** function defined in the [Example Code for Removing the RnR Connection Point](example-code-for-removing-the-rnr-connection-point.md) topic.</span></span>
