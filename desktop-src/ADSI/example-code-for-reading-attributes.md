---
title: Codice di esempio per la lettura degli attributi
description: Nell'esempio di codice seguente vengono enumerate le proprietà dell'utente specificato nel dominio corrente, cercando l'utente e quindi utilizzando IADsPropertyList per enumerarne le proprietà.
ms.assetid: f5541e9b-e149-4d22-9ff6-a32bd8239f57
ms.tgt_platform: multiple
keywords:
- Codice di esempio per la lettura di attributi ADSI
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104515463"
---
# <a name="example-code-for-reading-attributes"></a><span data-ttu-id="463bf-104">Codice di esempio per la lettura degli attributi</span><span class="sxs-lookup"><span data-stu-id="463bf-104">Example Code for Reading Attributes</span></span>

<span data-ttu-id="463bf-105">Nell'esempio di codice seguente vengono enumerate le proprietà dell'utente specificato nel dominio corrente, cercando l'utente e quindi utilizzando [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) per enumerarne le proprietà.</span><span class="sxs-lookup"><span data-stu-id="463bf-105">The following code example enumerates the properties of the specified user in the current domain, by searching for the user and then using [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) to enumerate its properties.</span></span> <span data-ttu-id="463bf-106">Tenere presente che vengono gestiti i valori di data e ora, come numeri interi di grandi dimensioni, e come vengono gestite le stringhe ottetto per **objectSID** e **objectGUID** .</span><span class="sxs-lookup"><span data-stu-id="463bf-106">Be aware that time and date values, as large integers, are handled and how octet strings for **objectSID** and **objectGUID** are handled.</span></span>
...
<span data-ttu-id="463bf-107">Nell'esempio di codice Visual Basic riportato di seguito viene illustrato come ottenere le proprietà di un oggetto utente.</span><span class="sxs-lookup"><span data-stu-id="463bf-107">The following Visual Basic code example shows how to get the properties of a user object.</span></span> <span data-ttu-id="463bf-108">Per usare questo esempio di codice, creare un riferimento alla libreria dei tipi di servizi di dominio Active Directory e alla libreria di Microsoft ActiveX Data Objects nel progetto Visual Basic.</span><span class="sxs-lookup"><span data-stu-id="463bf-108">To use this code example, create a reference to the Active DS Type Library and the Microsoft ActiveX Data Objects Library in your Visual Basic project.</span></span>
