---
description: Il codificatore audio MPEG-2 Microsoft Media Foundation è una trasformazione Media Foundation che codifica audio mono o stereo in audio MPEG-1 (ISO/IEC 11172-3) o MPEG-2 (ISO/IEC 13818-3).
...
| <a href="mf-mt-subtype-attribute.md">MF_MT_SUBTYPE</a> | Sottotipo audio. | Obbligatorio. Deve essere <strong>MFAudioFormat_MPEG</strong>. Questo sottotipo viene usato per l'audio MPEG-1 e MPEG-2. | 
| <a href="mf-mt-audio-samples-per-second-attribute.md">MF_MT_AUDIO_SAMPLES_PER_SECOND</a> | Esempi al secondo. | Obbligatorio. I valori seguenti sono supportati sia per MPEG-1 che per MPEG-2:<ul><li>32000</li><li>44100</li><li>48000</li></ul>Per MPEG-2 LSF sono supportati anche i valori seguenti: <br /><ul><li>16000</li><li>22050</li><li>24000</li></ul> | 
...
| <a href="mf-mt-audio-avg-bytes-per-second-attribute.md">MF_MT_AUDIO_AVG_BYTES_PER_SECOND</a> | Velocità in bit del flusso MPEG codificato, in byte al secondo. | facoltativo.<br /> Le specifiche ISO/IEC 11172-3 e ISO/IEC 13818-3 (LSF) definiscono diverse velocità in bit, a seconda della frequenza di campionamento, del numero di canali e del livello audio (1 o 2). <br /> Per impostazione predefinita, il codificatore è audio di livello 2. Se <a href="mf-mt-audio-avg-bytes-per-second-attribute.md">l MF_MT_AUDIO_AVG_BYTES_PER_SECOND'attributo</a> non è impostato, il codificatore usa le velocità in bit predefinite seguenti:<br /><ul><li>MPEG-1 stereo: 224.000 bit al secondo (bps) = 28.000 byte al secondo.</li><li>MPEG-1 mono: 192.000 bps = 24.000 byte al secondo.</li><li>MPEG-2 LSF, mono o stereo: 160.000 bps = 20.000 byte al secondo.</li></ul>Questo attributo può essere impostato su altri valori. Se il valore non è valido in base alle specifiche MPEG, MFT rifiuterà il tipo di supporto.<br /> È anche possibile impostare la velocità in bit usando <a href="/windows/desktop/api/strmif/nn-strmif-icodecapi"><strong>l'interfaccia ICodecAPI.</strong></a> Per ulteriori informazioni, vedere la sezione Osservazioni.<br /> | 
...
| <a href="mf-mt-audio-valid-bits-per-sample-attribute.md">MF_MT_AUDIO_VALID_BITS_PER_SAMPLE</a> | Numero di bit validi di dati audio in ogni campione audio. | facoltativo. Se impostato, il valore deve essere identico <a href="mf-mt-audio-bits-per-sample-attribute.md">a</a>MF_MT_AUDIO_BITS_PER_SAMPLE . | 
...
3.  Facoltativo: è possibile eseguire l'override della velocità in bit predefinita impostando un nuovo valore per [MF \_ MT AUDIO \_ \_ AVG BYTES PER \_ \_ \_ SECOND](mf-mt-audio-avg-bytes-per-second-attribute.md) nel tipo di supporto di output. Le velocità in bit valide dipendono dalla frequenza di campionamento, dal numero di canali e dal livello audio.
    > [!Note]  
    > A questo punto del processo di configurazione, il codificatore usa per impostazione predefinita l'audio di livello 2 e accetta solo velocità in bit di livello 2. Sarà possibile passare il codificatore al livello 1 in un passaggio successivo (vedere il passaggio 7). In tal caso, per il momento lasciare la velocità in bit predefinita. è possibile modificarlo nuovamente nel passaggio 8.
