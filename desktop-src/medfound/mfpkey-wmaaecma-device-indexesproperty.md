---
description: Specifica i dispositivi audio utilizzati dal provider di servizi di acquisizione vocale per l'acquisizione e il rendering dell'audio.
...
Specifica i dispositivi audio utilizzati dal provider di servizi di acquisizione vocale per l'acquisizione e il rendering dell'audio.
...
Impostare questa proprietà se si usa DSP in modalità di origine. Il provider di servizi di dominio ignora questa proprietà in modalità filtro.

Il valore della proprietà è due **parole a** 16 bit in formato **DWORD.** I 16 bit superiori specificano il dispositivo di rendering audio (in genere un altoparlante) e i 16 bit inferiori specificano il dispositivo di acquisizione (in genere un microfono). Ogni dispositivo viene specificato come indice nella raccolta di dispositivi audio. Se l'indice è -1, viene usato il dispositivo predefinito.

L'indice del dispositivo corrisponde all'indice della raccolta usato [**nell'interfaccia IMMDeviceCollection.**](/windows/win32/api/mmdeviceapi/nn-mmdeviceapi-immdevicecollection) L'applicazione deve riprodurre la voce far-end tramite il dispositivo di rendering selezionato. La voce di estrema distanza è la voce della persona all'altra estremità della linea telefonica, che viene riprodotta tramite il parlante nel computer dell'utente. Se il dispositivo di rendering selezionato non ha un flusso attivo, il provider di servizi di distribuzione non può elaborare alcun output.
