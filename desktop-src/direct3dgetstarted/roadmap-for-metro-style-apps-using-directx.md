...
| [Informazioni di riferimento sulla programmazione XInput](/windows/desktop/xinput/programming-reference) | Nodo di primo livello per la documentazione di riferimento dell'API controller XInput. |
...
| [Implementazione di buffer shadow per la funzionalità Direct3D livello 9](/previous-versions/windows/apps/jj262110(v=win.10)) | Linee guida per l'implementazione di mappe shadow nel livello di funzionalità DirectX \_ \* 9. |

## <a name="work-with-c"></a>Usare C++

Se si ha una vecchia mano con C++ nelle piattaforme Windows, le cose potrebbero avere un aspetto leggermente diverso. Ecco alcuni puntatori ad argomenti che consentono di ottenere un handle sulla differenza.
...
| [**Riferimento rapido (Windows Runtime e Visual C++)**](/cpp/cppcx/quick-reference-c-cx?view=vs-2019) | Tabella che fornisce informazioni rapide sulle Visual C++ di componenti (C++/CX) e sulle parole chiave. |
| [**Sistema di tipi (C++/CX)**](/cpp/cppcx/type-system-c-cx?view=vs-2019) | Contenuto di riferimento per i tipi supportati da C++/CX. |
| [**Spazi dei nomi (C++/CX)**](/cpp/cppcx/namespaces-reference-c-cx?view=vs-2019) | Fare riferimento al contenuto per gli spazi dei nomi che contengono tipi specifici di C++ che possono essere usati nelle app Windows Store. |
...
## <a name="additional-useful-libraries-for-windows-c-programming"></a>Librerie utili aggiuntive per la Windows C++

| Argomento | Descrizione |
|-|-|
| [Libreria di modelli standard C++](https://msdn.microsoft.com/library/c191tb28(v=VS.71).aspx) | Windows I tipi di runtime sono molto validi con i tipi di libreria di modelli standard. La maggior parte delle app Windows Store C++ usa raccolte e algoritmi della libreria di modelli standard, ad eccezione del limite ABI. |
...
| Argomento | Descrizione |
|-|-|
| [Blog di Games for Windows e DirectX](https://walbourn.github.io/) | Blog aggiornato regolarmente da un collaboratore di sviluppo DirectX chiave e una risorsa ideale per gli sviluppatori DirectX. |
| [Windows Blog per sviluppatori DirectX](https://devblogs.microsoft.com/directx/) | Blog ufficiale Windows DirectX. |
| [Blog DirectX di Shawn Hargreave](https://www.shawnhargreaves.com/blogindex.html)) | Blog per sviluppatori di un altro collaboratore di sviluppo Windows DirectX. |
| [DirectX Tool Kit](https://directxtk.codeplex.com/) | Sito CodePlex per DirectX Tool Kit (DxTK), che contiene una serie di API di supporto utili per il caricamento di mesh, la riproduzione di suoni e altri comportamenti comuni. |
| [Libreria di elaborazione delle trame DirectXTex](https://directxtex.codeplex.com/) | Sito plex di codice per DirectX Texture Processing Library (DxTEX), che contiene le API di supporto per l'elaborazione delle trame e la compressione/decompressione. |