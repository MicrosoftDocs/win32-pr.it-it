...
Mentre i controlli di selezione vengono usati per l'input numerico, l'input non deve essere un numero intero puro. L'input può essere un numero decimale e può avere segni negativi, delimitatori (ad esempio due punti o trattini) e modificatori di unità.
...
-   **Il controllo è associato a una casella di testo?** In caso contrario, non usare . I controlli di selezione non devono essere usati da soli o con altri tipi di controlli oltre a una casella di testo.
    ...
    **Non corretto:**
    ...
    ![screenshot del controllo di selezione, grafico, nessuna casella di testo ](images/ctrl-spin-controls-image2.png)
...
-   **Il controllo spin è utile?** Il controllo viene usato in un contesto in cui è probabile che gli utenti utilizzino il mouse? In caso contrario, considerare un controllo di selezione facoltativo.
-   **Gli elenchi a discesa dei controlli di pari livello** Se sono presenti altri elenchi a discesa, è consigliabile usare un elenco a discesa per la coerenza.
...
-   **Gli utenti con tocco o penna sono una destinazione principale?** In tal caso, è consigliabile usare invece un elenco a discesa. I pulsanti freccia in un controllo di selezione sono troppo piccoli per essere usati in modo efficiente con il tocco o una penna.
...
-   **Usare i controlli di selezione ogni volta che sono pratici e utili.** Vedere [È il controllo giusto?](#is-this-the-right-control)
    ...
    **Corretto:**
    ...
    ![screenshot dei controlli di selezione mese, giorno, anno ](images/ctrl-spin-controls-image5.png)

    In questo esempio viene usato un controllo spin con il controllo year per coerenza, anche se non è sempre pratico.

    **Non corretto:**

    ![Screenshot del controllo di selezione dell'indirizzo IP ](images/ctrl-spin-controls-image6.png)

    In questo esempio, il controllo spin è inutilizzabile.

-   **Fare sempre in modo che un controllo di rotazione sia il "controllo" della casella di testo.** In questo modo, il controllo di selezione viene posto all'interno della casella di testo.
    ...
    **Non corretto:**
    ...
    Nell'esempio corretto, il controllo di selezione viene posizionato all'interno della casella di testo associata.
...
-   **Definire il pulsante superiore per aumentare il valore di un'unità e il pulsante inferiore per diminuire di un'unità.** In genere, l'unità è una, ma deve essere la più piccola modifica comune nel valore. Idealmente, il controllo di selezione deve coprire tutti i valori validi e dovrebbe essere più pratico che digitare il testo.

    ![Screenshot del controllo di selezione "margini" ](images/ctrl-spin-controls-image9.png)

    In questo esempio, facendo clic su un controllo di selezione, i valori vengono 0,1, che è la più piccola modifica comune nel valore. L'uso di un'unità più piccola copre l'intervallo di valori validi, ma rende inutilizzabili i controlli di selezione.
...
-   **Alla fine di un intervallo di valori validi, riavviare l'intervallo.** La metafora del controllo di rotazione è che l'utente ruota una ruota di valori, quindi questo comportamento simile a una ruota.
    -   **Eccezione:** Non riavviare l'intervallo se il valore risultante è certo non corretto.

        ![Screenshot del controllo di selezione "numero di copie" ](images/ctrl-spin-controls-image10.png)

        In questo esempio, facendo clic sul pulsante con la freccia rivolta verso il basso non viene riavviato l'intervallo (fino al valore massimo) perché è certo che il valore non è corretto.

-   **Usare testo anziché valori numerici speciali.** Consentire agli utenti di selezionare questi valori speciali invece di conoscerli e digitarli.

    ![Screenshot del controllo di selezione "Soscise dopo (mai)" ](images/ctrl-spin-controls-image11.png)

    In questo esempio Non è mai un valore speciale, ma gli utenti possono ruotare su di esso.
...
-   **Se il valore contiene delimitatori, la casella di testo associata deve avere più punti di attivazione dell'input.** In questo modo i segmenti numerici possono essere manipolati singolarmente.
    ...
    In questo esempio, il controllo spin influisce sui valori per ore, minuti, secondi e A.M./P.M., a seconda del valore che ha lo stato attivo.

-   **Se il valore ha unità, usare il controllo rotazione anche per modificare tali unità.**
...
-   Fare riferimento ai controlli di selezione e alle caselle di selezione solo nella programmazione e in altre documentazione tecnica.
