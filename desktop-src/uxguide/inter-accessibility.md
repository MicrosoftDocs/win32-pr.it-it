...
Il numero di utenti che le funzionalità di accessibilità possono essere utili può risultare sorprendente; Nel Stati Uniti, ad esempio, i sondaggi hanno dimostrato che più della metà di tutti gli utenti di computer ha difficoltà o problemi correlati all'accessibilità ed è probabile che trarranno vantaggio dall'uso della tecnologia accessibile. Inoltre, l'approccio alla progettazione del software con la flessibilità e l'inclusività che sono le caratteristiche distintive dell'accessibilità spesso comporta un miglioramento generale dell'usabilità e della soddisfazione dei clienti.
...
**Nota:** Le linee guida relative [](vis-color.md) [a tastiera,](inter-keyboard.md) [mouse,](inter-mouse.md)colore [e audio](vis-sound.md) sono presentate in articoli separati.
...
| Destrezza<br/>          | Varia da grave (che interessa il 19% degli utenti) a grave (che interessa il 5% degli utenti). Questo problema spesso comporta difficoltà nell'esecuzione di determinate competenze motorie con la tastiera o il mouse.<br/> | Ridondanza del metodo di input: funzionalità del programma accessibili dagli equivalenti del mouse o della tastiera.<br/>                                                                                       |
...
Per altre linee guida su come aiutare gli utenti con questi problemi, vedere Risolvere particolari [problemi](#addressing-particular-impairments) più avanti in questo articolo.
...
Un'utilità per la lettura dello schermo consente agli utenti con disabilità visive o con problemi di spostarsi all'interno di un'interfaccia utente trasformando gli oggetti visivi in audio. Il testo dell'interfaccia utente, i controlli, i menu, le barre degli strumenti, la grafica e altri elementi dello schermo vengono quindi pronunciati dalla voce computerizzata dell'utilità per la lettura dello schermo. Per creare un programma ottimizzato per l'assistive technology dello schermo, è necessario pianificare il modo in cui l'utilità per la lettura dello schermo identificherà ogni elemento dell'interfaccia utente.
...
Assistente vocale è un'utilità per la lettura dello schermo ridotta in Windows che consente agli utenti di ascoltare il testo sullo schermo e gli elementi dell'interfaccia utente letti a voce alta, inclusi alcuni eventi (inclusi i messaggi di errore) che si verificano in modo sconsarato. L'utente può ascoltare i menu dell'Assistente vocale senza uscire dalla finestra attiva.
...
I tasti di scelta sono spesso confusi con i tasti di scelta rapida, ma i tasti di scelta rapida vengono assegnati in modo diverso dai tasti di scelta e hanno obiettivi diversi. I tasti di scelta rapida, ad esempio, usano le sequenze di tasti CTRL e FUNZIONE e sono destinati principalmente a utenti avanzati anziché per l'accessibilità.
...
I programmi accessibili aiutano tutti gli utenti in qualche modo perché gli obiettivi di accessibilità e usabilità si sovrappongono. Ad esempio, le funzionalità progettate per rendere gli utenti avanzati il più efficienti possibile possono essere utili anche per gli utenti che preferiscono usare la tastiera a causa di problemi di complessità.
...
-   Assicurarsi che gli utenti possono modificare le dimensioni del testo nell'interfaccia utente del programma, ad esempio tramite un controllo dispositivo di scorrimento o una casella di riepilogo a discesa per le dimensioni del carattere. Se possibile, supportare la modalità punti per pollice (dpi) elevata.
...
È anche necessario dimostrare il rispetto in base a:
...
-   Creazione di documentazione accessibile in formati accessibili. Pertanto, la documentazione stessa deve rispettare le stesse regole di accessibilità dell'interfaccia utente primaria, tra cui la possibilità di ingrandire le dimensioni del carattere, l'uso del testo alternativo per la grafica e l'architettura delle informazioni ridondante (ad esempio, usando la codifica a colori solo come supplemento al testo).

Nei prodotti software, il rispetto per gli utenti può manifestarsi nell'usabilità e nella ricerca di mercato, nei servizi di supporto e nella documentazione efficaci e, naturalmente, nelle decisioni di progettazione. Ad esempio, rimettendo in mente la progettazione per gli utenti avanzati: si sta inserendo questa nuova funzionalità all'avanguardia perché lo si vuole o perché si sa che gli utenti avanzati l'hanno richiesta? Quest'ultimo caso indica che il processo decisionale di progettazione è ben informato dal valore del rispetto.
...
-   **Non usare il testo nella grafica.** Gli utenti con problemi visivi possono avere la grafica disattivata (ad esempio, in un Web browser) o semplicemente non visualizzare o cercare testo inserito nella grafica.
-   **Assicurarsi che le finestre e le finestre** di dialogo hanno nomi significativi, in modo che un utente che sta udindo anziché visualizzare lo schermo (ad esempio, usando un'utilità per la lettura dello schermo) osezioni le informazioni contestuali appropriate.
...
-   **Mantenere conciso il testo del fumetto** in modo che sia più facile da leggere e ridurre al minimo l'interruzione delle utilità per la lettura dello schermo.
    ...
    Anche se i fumetto possono usare testo del corpo aggiuntivo se necessario, questo esempio mostra che talvolta il solo testo del titolo raggiunge lo stesso obiettivo in modo più economico e accessibile.
...
-   **Consentire agli utenti di controllare il volume dell'output audio.** A questo scopo, Windows volume Mixer volume. Per altre informazioni, vedere [Sound](vis-sound.md).
...
-   **Per migliorare la consapevolezza delle assegnazioni delle chiavi di accesso nel programma, è possibile visualizzarle in qualsiasi momento.** In Pannello di controllo passare al Centro accessibilità e fare clic su Rendi la **tastiera più semplice da usare.** quindi selezionare la casella **di controllo Sottolinea tasti di scelta rapida** e tasti di scelta.
