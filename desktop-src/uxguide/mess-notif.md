...
In Windows Vista e versioni successive, le notifiche vengono visualizzate per una durata fissa di 9 secondi. Le notifiche non vengono visualizzate immediatamente quando gli utenti sono inattivi o gli screen saver sono in esecuzione. Windows accoda automaticamente le notifiche durante questi orari e visualizza le notifiche in coda quando l'utente riprende l'attività regolare. Di conseguenza, non è necessario eseguire operazioni per gestire queste circostanze speciali.
...
In questo esempio, Outlook la connessione e lo stato di sincronizzazione sulla barra di stato.
...
Sfortunatamente, sono presenti troppe notifiche inappropriati, inappropriati, inutili e irrilevanti. Si considerino le notifiche Windows XP Hall of Rieti:
...
In questi esempi, Windows XP tenta apparentemente di assistere gli utenti nella configurazione iniziale. Tuttavia, queste notifiche vengono visualizzate troppo spesso e ben dopo essere state utili, quindi sono poco più di annunci di funzionalità non richieste.

## <a name="user-flow-must-be-maintained"></a>È necessario mantenere il flusso utente

**Idealmente, gli utenti immersi nel proprio lavoro non visualizzano affatto le notifiche. Ma vedranno le notifiche solo quando il flusso è già interrotto.**

In Flow: The Satisfactiony of Optimal Experience, Mihaly Csikszentmihalyi afferma che gli utenti entrano in uno stato di flusso quando sono pienamente soddisfatti dell'attività durante la quale perdono il senso del tempo e hanno una grande soddisfazione.
...
-   Se si usano correttamente le notifiche e non richiedono un intervento immediato da parte dell'utente, fare in modo che gli utenti sceglino di ignorarle per impostazione predefinita. Non modificare questa impostazione.
...
Ad esempio, il Windows di risparmio energia indica inizialmente una batteria in esaurimento modificando semplicemente l'icona dell'area di notifica.

![Screenshot di sei icone che mostrano lo stato della batteria ](images/mess-notif-image7.png)

In questi esempi, il Windows risparmio energia usa l'icona dell'area di notifica per notificare agli utenti l'alimentazione a batteria progressivamente inferiore.

Quando l'alimentazione della batteria si riduce, Windows avvisa gli utenti dell'interruzione dell'alimentazione della batteria tramite una notifica.

![Screenshot della notifica di alimentazione a batteria in esaurimento](images/mess-notif-image8.png)

In questo esempio, Windows risparmio energia usa una notifica per indicare agli utenti che l'alimentazione a batteria è debole.

Questa notifica viene visualizzata mentre gli utenti hanno ancora diverse opzioni. Gli utenti possono collegarsi, modificare le opzioni di risparmio energia, eseguire il wrapping del lavoro e arrestare il computer oppure ignorare la notifica e continuare a funzionare. Con l'esaurimento della batteria, il testo e l'icona della notifica riflettono l'urgenza aggiuntiva. Tuttavia, quando l'alimentazione a batteria diventa così bassa che gli utenti devono agire immediatamente, Windows risparmio energia invia una notifica agli utenti usando una [finestra](glossary.md) di messaggio modale.

![Screenshot dell'avviso relativo all'alimentazione a batteria in esaurimento](images/mess-notif-image9.png)

In questo esempio, Windows risparmio energia usa una finestra di messaggio modale per notificare agli utenti l'alimentazione a batteria in condizioni critiche.
...
-   **Non usare notifiche durante l'esperienza di Windows iniziale.** Per migliorare la prima esperienza, Windows 7 elimina tutte le notifiche visualizzate durante le prime ore di utilizzo. Progettare il programma presupponendo che gli utenti non vedano tali notifiche.
...
-   **Non notificare operazioni riuscite, ad eccezione delle circostanze seguenti:**
    -   **Sicurezza.** Gli utenti considerano le operazioni di sicurezza di massima importanza, pertanto notificare agli utenti le operazioni di sicurezza riuscite.
...
-   Per il modello FYI, non inviare una notifica se gli utenti possono continuare a funzionare normalmente o se è improbabile che esercitino operazioni diverse in seguito **alla notifica.**

    **Non corretto:**

    ![Screenshot della notifica per prestazioni più veloci ](images/mess-notif-image17.png)

    In questo esempio le informazioni sono utili solo se l'utente ha già installato le porte. In caso contrario, è probabile che l'utente non esererà un'operazione diversa come risultato.

    -   Eccezione: è possibile notificare agli utenti informazioni di dubbia pertinenza se sono facoltative e gli **utenti acconsentino esplicitamente.**

        **Corretto:**

        ![Screenshot della notifica del contatto connesso ](images/mess-notif-image16.png)

        In questo esempio gli utenti ricevono una notifica quando i contatti vengono online e scelgono di ricevere queste informazioni facoltative.

-   Per gli eventi di sistema non critici e i modelli FYI, **usare le notifiche complete per un singolo evento.** Non presentare più di uno parziale.
...
-   **Visualizzare una notifica in base al modello di progettazione:**
...
| Errore dell'azione<br/>            | In caso di errore di un'attività asincrona.<br/>                                                                                                                                                                   |
| Evento di sistema non critico<br/> | Quando si verifica un evento e l'utente è attivo o la condizione continua a esistere. Se questo è il risultato di un problema, rimuovere la notifica attualmente visualizzata immediatamente dopo che il problema è stato risolto.<br/> |
...
In questo esempio, in Windows Vista la notifica di nessuna connettività wireless è prematura perché viene spesso seguita immediatamente da una notifica di buona connettività.

-   Per il successo dell'azione  e i modelli FYI, usare l'opzione in tempo reale in modo che le notifiche non in attesa non siano accodati quando gli utenti eseguono un'applicazione a schermo intero o non usano attivamente il computer.
-   Per il modello di eventi di sistema non critico, non creare la possibilità di tempeste di notifica sfalsando gli eventi associati a eventi noti, ad esempio **l'accesso utente.** Associare invece l'evento a un periodo di tempo successivo all'evento. Ad esempio, è possibile ricordare agli utenti di registrare il prodotto cinque minuti dopo l'accesso utente.
...
| Errore dell'azione<br/>            | Singola occorrenza.<br/>                                                                                                            |
| Evento di sistema non critico<br/> | Una volta quando si verifica l'evento per la prima volta. Se questo è il risultato di un problema che gli utenti devono risolvere, visualizzarlo di nuovo una volta al giorno.<br/> |
...
-   **Per le attività utente facoltative, non tentare di eseguire il pester degli utenti nell'invio visualizzando costantemente le notifiche.** Se l'attività è obbligatoria, visualizzare immediatamente una finestra di dialogo modale anziché usare le notifiche.
...
-   **Se alla fine gli utenti devono intervenire,** usare l'escalation progressiva per visualizzare un'interfaccia utente alternativa che gli utenti non possono ignorare.

### <a name="interaction"></a>Interazione

-   **Rendere selezionabili le notifiche quando:**
    -   **Gli utenti devono eseguire un'azione.** Facendo clic sulla notifica verrà visualizzata una finestra in cui gli utenti possono eseguire l'azione. Questo approccio è preferibile per l'errore di azione e per i modelli di progettazione facoltativi delle attività utente.
...
-   **Visualizzare sempre una finestra quando gli utenti fa clic per eseguire un'azione.** Non è necessario fare clic su Esegui un'azione direttamente.
-   **Se si fa clic per visualizzare altre informazioni, vengono sempre visualizzate altre informazioni.** Non ridefinire semplicemente le informazioni già presenti nella notifica.
...
-   **Per altri modelli, usare le icone** che mostrano gli oggetti correlati o consigliano l'oggetto , ad esempio uno schermo per la sicurezza o una batteria per l'alimentazione.
-   **Usare le icone in base all'applicazione o alla personalizzazione aziendale se gli utenti di destinazione le riconosceranno e non esiste un'alternativa migliore.**
-   Per un'escalation progressiva, è consigliabile usare le icone con un aspetto progressivamente più **enfatico, in** quanto la situazione diventa più urgente.
-   **Non usare l'icona delle informazioni standard.** Che le notifiche siano informazioni vanno senza dirlo.
-   **È consigliabile usare icone grandi (32 x 32 pixel) quando:**
    -   Gli utenti comprenderanno rapidamente l'icona anziché il testo.
    -   Le icone grandi comunicano il loro significato in modo più chiaro ed efficace rispetto alle icone standard da 16x16 pixel.
    -   L'icona usa [lo stile Aero.](vis-icons.md)

![Screenshot della notifica "messaggi importanti" ](images/mess-notif-image22.png)

In questo esempio, gli utenti possono comprendere rapidamente la natura della notifica con uno sguardo all'icona grande.

### <a name="notification-queuing"></a>Accodamento delle notifiche
...
-   Per il successo dell'azione e i modelli **FYI,** usare l'opzione in tempo reale in modo che la notifica non sia accodata a lungo. Queste notifiche hanno valore solo quando possono essere visualizzate immediatamente.
-   **Rimuovere le notifiche in coda quando non sono più rilevanti.**
-   **Sviluppatori:** A tale scopo, impostare il flag NIF \_ INFO in uFlags e impostare szInfo su una stringa vuota. Questa operazione non è dannosa se la notifica non è più presente nella coda.
...
### <a name="body-text"></a>Corpo del testo

-   **Usare il testo del corpo che fornisce una descrizione (senza ripetere le informazioni nel titolo) e, facoltativamente, che fornisce dettagli specifici sulla notifica e consente agli utenti di sapere quale azione è disponibile.**
-   **Usare frasi complete con punteggiatura finale.**
-   **Usare le maiuscole/minuscole come nelle frasi comuni.**
-   **Usare non più di 200 caratteri (in inglese) per supportare la localizzazione.** Il corpo del testo ha una lunghezza massima di 255 caratteri, ma è necessario consentire l'espansione del 30% quando viene tradotto il testo in lingua inglese.
-   **Includere informazioni essenziali nel corpo del testo, ad esempio nomi di oggetti specifici.** (Esempi: nomi utente, nomi di file o URL). Gli utenti non devono aprire un'altra finestra per trovare tali informazioni.
-   **Inserire virgolette doppie per i nomi degli oggetti.**
    -   **Eccezione:** Non usare le virgolette quando:
        -   Il nome dell'oggetto usa sempre [l'uso di](glossary.md)maiuscole e minuscole in stile titolo, ad esempio con i nomi utente.
        -   Il nome dell'oggetto viene compensato con due punti (ad esempio: Nome stampante: Stampante).
        -   Il nome dell'oggetto può essere facilmente determinato dal contesto.
-   **Se è necessario troncare i nomi di oggetto a una dimensione massima fissa per supportare la localizzazione, usare i puntini di sospensione per indicare il troncamento.**
...
-   **Descrivere l'evento in modo pertinente per gli utenti di destinazione.** Una notifica è pertinente se è possibile che gli utenti eseereranno un'attività o cambino il proprio comportamento come risultato della notifica. È spesso possibile eseguire questa operazione descrivendo le notifiche in termini di obiettivi dell'utente anziché di problemi tecnologici.
...
-   Fare riferimento al componente come notifica, non come un fumetto o un avviso.
...
-   Fare riferimento all'area di notifica come area di notifica, non all'area di notifica.
