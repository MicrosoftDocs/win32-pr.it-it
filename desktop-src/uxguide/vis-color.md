...
Il colore viene spesso pensato in termini di spazio colore, dove RGB (rosso, verde, blu), HSL (tonalità, saturazione, luminosità) e HSV (tonalità, saturazione, valore) sono gli spazi colore usati più comunemente.
...
-   **Stato.** Lo stato di un oggetto può essere indicato tramite il colore. Ad esempio, Windows colore per indicare gli stati di selezione e passaggio del mouse. I collegamenti all'interno delle pagine Web usano il blu per gli elementi nonvisi e viola per le pagine visitate.
...
-   **Enfasi.** Il colore può essere usato per attirare l'attenzione degli utenti. Ad esempio, Windows istruzioni [principali blu](text-ui.md) per distinguerle dall'altro testo.
...
**L'interpretazione del colore da parte degli utenti dipende spesso dalle impostazioni cultura.** Ad esempio, nel Stati Uniti, l'abbigliamento per le fedi nuziali per la spose è in gran parte associato al colore bianco, mentre il nero è associato ai fiori funebri. Tuttavia, molto tempo fa in Giappone il simbolismo dei colori era esattamente il contrario: il bianco era il colore predominante ai funebri e il nero era considerato un colore che porta buona fortuna per i fedi nuziali.

Detto questo, **l'interpretazione di rosso, giallo e verde** per lo stato è coerente a livello globale. Ciò è dovuto alla Convenzione di Maiolandese [per](https://www.unece.org/trans/conventn/signalse.pdf)i segnali stradali, che definisce la convenzione mondiale per i semafori (dove il rosso indica stop, il verde significa procedere e il giallo indica procedere con cautela). È possibile usare questi colori di stato senza problemi per le interpretazioni dipendenti dalle impostazioni cultura.

Oltre ai colori di stato, Windows i significati ai colori in base alla convenzione, come illustrato nella sezione Linee guida di questo articolo. Assicurarsi che l'utilizzo dei colori del programma sia compatibile con queste convenzioni di colore.
...
I colori primari, come si vede con la normale visione dei colori.

![figura che mostra gli stessi colori visualizzati con la protanopia ](images/vis-color-image7.png)

I colori primari, come si vede con Protanopia (1% della popolazione di sesso femminile).

![Figura che mostra gli stessi colori visualizzati con la deuteranopia ](images/vis-color-image8.png)

I colori primari, come si vede con Deuteranopia (6% della popolazione di sesso femminile).
...
-   **Posizione.** È anche possibile usare la posizione relativa, ma questo approccio è più debole rispetto alle alternative. Per essere efficace, la posizione deve essere standard e ben nota, come con i semafori.
...
Ironia della sorte, il modo migliore per progettare il colore è iniziare progettando senza colore, usando [wireframe](glossary.md) o monocromatici e quindi aggiungere il colore in un secondo momento. In questo modo si garantisce che le informazioni non vengano comunicate usando solo il colore. Garantisce inoltre che le stampe siano di grande qualità nelle stampanti monocroma.

### <a name="use-theme-or-system-colors"></a>Usare il tema o i colori di sistema

Anche se esistono molti fattori complessi nell'uso efficace del colore, nell'interfaccia utente [](glossary.md) di [](glossary.md) Windows la scelta del colore spesso si riduce alla semplice scelta del colore del tema o del colore di sistema appropriato in base ad alcune semplici regole. Gli utenti possono quindi selezionare e personalizzare queste combinazioni di colori nel modo desiderato.

In questo modo, non solo si adattano le preferenze di colore di tutti gli utenti, ma si elimina il carico di scegliere l'unica combinazione di colori perfetta che funziona per tutti i asserzioni, gli stili e le impostazioni cultura (che, naturalmente, è altrimenti impossibile).

**Se si fa una sola cosa...**

Scegliere i colori selezionando il colore del tema o del sistema appropriato. Non usare mai il colore come metodo di comunicazione principale, ma come metodo secondario per rafforzare visivamente il significato. Progettare usando wireframe o monocromatici per garantire che il colore sia secondario.

### <a name="use-theme-or-system-colors-correctly"></a>Usare correttamente il tema o i colori di sistema

Si supponga che gli utenti scelsino i colori del tema o del sistema in base alle esigenze personali e che il tema o i colori di sistema siano costruiti in modo appropriato. In base a questo presupposto, se si scelgono sempre i colori del tema o del sistema in base allo scopo previsto e si associano i primi piani con gli sfondi associati, i colori sono leggibili e rispettano i desideri degli utenti in tutte le modalità video, inclusa la modalità a [contrasto elevato](glossary.md). Ad esempio, il colore del sistema di testo della finestra è garantito per essere leggibile rispetto al colore di sistema di sfondo della finestra.
...
-   **Abbina i colori di primo piano ai colori di sfondo associati.** I colori di primo piano sono garantiti per essere leggibili solo rispetto ai colori di sfondo associati. Non combinare e abbinare i colori di primo piano con altri colori di sfondo o, ancora, con altri colori di primo piano.
-   **Non combinare tipi di colore.** In altre parole, associare sempre i colori del tema ai colori del tema associati, i colori di sistema con i colori di sistema associati e i colori hardwired con altri colori hardwired. Ad esempio, non è garantito che un colore del testo del tema sia leggibile su uno sfondo hardwired.
-   **Se è necessario eseguire il hardwire dei colori, gestire la modalità a contrasto elevato come caso speciale.**

**Se si fa una sola cosa...**

Scegliere sempre i colori del tema o del sistema in base allo scopo previsto e associare i primo piano agli sfondi associati.

### <a name="using-other-colors"></a>Uso di altri colori

Anche se Windows tema definisce un set completo di parti del tema, è possibile che il programma necessita di colori non definiti nel file del tema. Sebbene sia possibile eseguire il hardwire di questi colori, un approccio migliore consiste nel derivare i colori dal tema o dai colori di sistema. L'uso strategico di questo approccio offre tutti i vantaggi dell'uso del tema e dei colori di sistema, ma con una maggiore flessibilità.

Si supponga, ad esempio, di avere bisogno di uno sfondo della finestra più scuro del colore di sfondo della finestra del tema. Nello spazio colore HSL, un colore più scuro indica un colore con una luminosità inferiore. È quindi possibile derivare un colore di sfondo della finestra più scuro seguendo questa procedura:
...
-   Riconvertire in valori RGB.

Con questo approccio, è garantito che il colore derivato venga percepito come una sfumatura più scura del colore originale (a meno che il colore originale non fosse molto scuro per iniziare).
...
-   Dipendenza complessiva dal colore tramite il filtro Scala di grigi.
...
-   Temi abilitati usando il tema Windows predefinito.
-   Tema abilitato con un tema non predefinito.
-   Tema disabilitato ("Windows stile classico" nell'elemento Theme Impostazioni nell'elemento Personalization Pannello di controllo).
...
-   **Non usare mai il colore come metodo di comunicazione principale,** ma come metodo secondario per rafforzare visivamente il significato.

### <a name="using-theme-and-system-colors"></a>Uso del tema e dei colori di sistema

-   Quando possibile, scegliere i colori selezionando il colore del tema o del **sistema appropriato.** In questo modo, è sempre possibile rispettare le preferenze di colore degli utenti.
-   **Scegliere il tema e i colori di sistema in base allo scopo.** Non scegliere i colori in base all'aspetto corrente, perché tale aspetto può essere modificato dall'utente o dalle versioni future di Windows.
-   **Abbina i colori di primo piano ai colori di sfondo associati.** I colori di primo piano sono garantiti per essere leggibili solo rispetto ai colori di sfondo associati. Non combinare e abbinare i colori di primo piano con altri colori di sfondo o, ancora, con altri colori di primo piano.
-   **Non combinare tipi di colore.** In altre parole, associare sempre i colori del tema ai colori del tema associati, i colori di sistema con i colori di sistema associati e i colori hardwired con altri colori hardwired. Ad esempio, non è garantito che un colore del testo del tema sia leggibile su uno sfondo hardwired.
-   **Se è necessario usare un colore diverso da un tema o un colore di sistema:**
    -   **Preferisce derivare il colore da un tema o da un colore di sistema invece di impostarne il valore come hardwiring.** Usare il processo descritto in precedenza in questo articolo, in Uso di altri colori.
    -   **Gestire la modalità a contrasto elevato come caso speciale.**
-   **Gestire le modifiche al tema.** Le modifiche al tema vengono gestite automaticamente dalle finestre con frame di finestra standard e controlli comuni. Windows con frame di finestra personalizzati, controlli personalizzati o di disegno del proprietario e altri usi del colore devono gestire in modo esplicito le modifiche al tema.
    -   **Sviluppatori:** È possibile rispondere agli eventi di modifica del tema gestendo il messaggio \_ WM THEMECHANGED.

### <a name="color-meaning"></a>Significato del colore

-   Anche se è consigliabile usare i colori del tema e di sistema (o i colori derivati) ogni volta che è possibile, assicurarsi che qualsiasi altro uso del colore sia compatibile con l'uso del colore seguente all'interno di Windows.



| Tonalità | Significato | Usare in Windows  |
|--------------------------------------|-------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| blu/verde<br/>                | Windows marchio<br/>                                                      | Background: Windows personalizzazione.<br/>                                                                                                                        |
...
| rosso<br/>                       | error, stop, vulnerable, critical, immediate attention, restricted<br/> | Background: stato, stato arrestato (barre di stato).<br/> Icone: errore, arresto, chiusura della finestra, eliminazione, input richiesto, mancante, non disponibile.<br/>     |
| yellow<br/>                    | avviso, attenzione, dubbia<br/>                                     | Sfondo: stato, stato sospeso (barre di stato).<br/> Icone: avviso<br/>                                                                       |
...
| purple<br/>                    | Visitato<br/>                                                            | Primo piano: collegamenti visitati (per i collegamenti Windows Internet Explorer e documenti).<br/>                                                                |
...
-   **Assegnare per impostazione predefinita colori facili da distinguere.** In genere, i colori sono facili da distinguere se sono distanti l'uno dall'altro negli spazi colori HSL/HSV, mantenendo un contrasto elevato con lo sfondo:
    ...
    -   I colori hanno un contrasto elevato se vi è una grande differenza nella tonalità, nella saturazione o nella luminosità.
    ...
    -   L'uso di uno sfondo bianco o molto chiaro semplifica la distinzione dei colori di primo piano.
...
-   **Fare riferimento agli elementi dell'interfaccia utente in base al nome e non ai colori.** Tali riferimenti non sono accessibili e i colori di sistema possono cambiare. Se il nome di un elemento dell'interfaccia utente non è noto o non è sufficientemente descrittivo, visualizzare uno screenshot per chiarire.
...
![screenshot del messaggio contenente la "barra color oro" ](images/vis-color-image18.png)

Nell'esempio non corretto, il messaggio fa riferimento Windows Internet Explorer barra informazioni in base al colore anziché al nome.
