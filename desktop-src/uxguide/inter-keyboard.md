...
La Tecnologia Windows per Tablet PC tastiera su schermo.

![Screenshot del blocco di scrittura del tablet Windows ](images/inter-keyboard-image2.png)

Il Tecnologia Windows per Tablet PC scrittura.
...
-   Un tasto di modifica combinato con un altro tasto modifica il significato del tasto associato, ad esempio CTRL, ALT, MAIUSC e il tasto Windows logo.
...
-   **Pulsanti predefiniti.** Windows con pulsanti di comando e collegamenti di comando hanno un singolo pulsante predefinito indicato da un bordo evidenziato, ovvero il pulsante su cui si fa clic quando si preme INVIO. Per impostazione predefinita, è assegnato un solo pulsante di comando o collegamento di comando predefinito. Tuttavia, il pulsante predefinito viene spostato quando l'utente passa a un altro pulsante di comando o collegamento di comando. Di conseguenza, anche qualsiasi pulsante di comando o collegamento di comando con lo stato attivo per l'input è sempre il pulsante predefinito.
...
Poiché le chiavi di accesso non sono destinate a essere memorizzate, vengono assegnate a un carattere che si trova **all'inizio dell'etichetta** per renderle più facili da trovare, anche se è presente una parola chiave che viene visualizzata più avanti nell'etichetta.

**Corretto:**

![Screenshot del primo carattere nell'etichetta sottolineata ](images/inter-keyboard-image9.png)

**Non corretto:**

![Screenshot di 21 caratteri sottolineati ](images/inter-keyboard-image10.png)
...
Poiché i tasti di scelta rapida sono destinati a essere memorizzati, i tasti di scelta rapida usati più di frequente usano idealmente le lettere dei primi caratteri o dei caratteri più facili da ricordare **all'interno** delle parole chiave del comando, ad esempio CTRL+C per Copia e CTRL+Q per La richiesta.
...
-   Assegnare prima di tutto i tasti di scelta ai [pulsanti di commit e](glossary.md) ai collegamenti di comando. Usare la tabella delle assegnazioni di tasti di scelta standard quando si applica. In caso contrario, usare la prima lettera della prima parola.
-   Ignorare i controlli a cui non sono assegnate chiavi di accesso.
-   Assegnare chiavi di accesso univoche ai controlli rimanenti (a partire dai controlli usati più di frequente):
    -   Se possibile, assegnare la chiave di accesso in base alla tabella delle assegnazioni delle chiavi di accesso standard.
    -   In caso contrario:
        -   Preferisce i caratteri visualizzati all'inizio dell'etichetta, idealmente il primo carattere della prima o della seconda parola.
        ...
        -   Evitare di usare caratteri che rendono difficile visualizzare la sottolineatura, ad esempio lettere larghe un pixel, lettere con discendenti e lettere accanto a una lettera con un discendente.
...
-   Se ancora non tutti i controlli possono avere chiavi di accesso univoche:
    ...
    -   In caso contrario, è possibile accedere ai controlli rimanenti usando tab e il tasto di direzione.
...
Se una finestra visualizzata fuori contesto (non avviata dall'utente) ruba lo stato attivo per l'input, è probabile che questa finestra riceva l'input destinato a un'altra finestra. Inoltre, i tasti di scelta hanno effetto quando vengono premuti senza premere ALT se nella finestra di dialogo non sono presenti controlli che accettano input di testo, ad esempio caselle di testo ed elenchi. Nell'esempio seguente, quindi, premendo "r" viene attivato il pulsante Riavvia ora.

Chiaramente, tale input può avere conseguenze impreviste significative.

**Non corretto:**

![Screenshot del pulsante Riavvia ora, sottolineata 'r' ](images/inter-keyboard-image14.png)

In questo esempio, digitando testo con spazio, "r" o INVIO si riavvia accidentalmente Windows.

Naturalmente, la soluzione migliore a questo problema è non rubare lo stato attivo per l'input. Al contrario, fare lampeggiare il pulsante della barra delle [applicazioni](winenv-taskbar.md) del programma o visualizzare una notifica per ottenere l'attenzione dell'utente.

Tuttavia, se è necessario visualizzare una finestra di questo tipo, l'approccio migliore consiste nel non assegnare un pulsante predefinito o i tasti di scelta e assegnare lo stato attivo per l'input iniziale a un controllo diverso da un pulsante di commit.

**Corretto:**

![Screenshot del pulsante di riavvio, 'r' non sottolineato ](images/inter-keyboard-image15.png)

In questo esempio il riavvio accidentale Windows è molto più difficile da eseguire.

**Se si eservino solo sei operazioni...**

1.  Progettare una navigazione da tastiera appropriata, con un ordine di tabulazione appropriato e i gruppi di controllo appropriati, lo stato attivo iniziale per l'input e i pulsanti predefiniti.
2.  Assegnare i tasti di scelta a tutti i menu e alla maggior parte dei controlli.
3.  Assegnare i tasti di scelta a un carattere visualizzato all'inizio dell'etichetta, per semplificare l'individuazione.
4.  Assegnare i tasti di scelta rapida ai comandi usati più di frequente.
5.  Provare ad assegnare i tasti di scelta rapida al primo o più facile da ricordare all'interno delle parole chiave.
...
-   **Non usare il tasto MAIUSC per modificare i comandi nei menu o nelle finestre di dialogo.** Questa operazione non è rilevabile e imprevista.

    **Non corretto:**

    ![Screenshot della finestra di dialogo conferma sostituzione cartella ](images/inter-keyboard-image16.png)

    In questo esempio Windows XP, tenendo premuto MAIUSC si sostituisce Sì a Tutti con No a Tutti.

-   **Non disabilitare un controllo con lo stato attivo per l'input. Questa operazione potrebbe impedire alla finestra di ricevere l'input da tastiera.** Prima di disabilitare un controllo con lo stato attivo per l'input, spostare lo stato attivo per l'input su un altro controllo.
-   **Se una finestra viene visualizzata fuori contesto, potenzialmente sorprendendo gli utenti, potrebbe essere necessario evitare conseguenze impreviste significative:**
    -   Non assegnare un pulsante predefinito.
    -   Non assegnare chiavi di accesso.
    -   Assegnare lo stato attivo all'input iniziale a un controllo diverso da un pulsante di commit.

### <a name="keyboard-navigation"></a>Spostamento tramite tastiera

-   **Mostra sempre l'indicatore dello stato attivo per l'input. Eccezione: è** possibile eliminare temporaneamente l'indicatore dello stato attivo per l'input se:
    -   L'indicatore dello stato attivo per l'input è visivamente distraendo (come nel caso di una visualizzazione elenco di grandi dimensioni non nella visualizzazione Dettagli).
...
-   **L'ordine di tabulazione deve seguire l'ordine di lettura, che in genere scorre da sinistra a destra, dall'alto verso il basso.** È consigliabile creare eccezioni per i controlli di uso comune inserendoli in precedenza nell'ordine di tabulazione. Tab deve scorrere tutte le tabulazioni in entrambe le direzioni senza arrestarsi.
...
-   **Selezionare il pulsante di comando o il collegamento di comando più sicuro (per evitare la perdita di dati o l'accesso al sistema) come impostazione predefinita.** Se sicurezza e sicurezza non sono fattori, selezionare la risposta più probabile o comoda.
-   **Lo spostamento tramite tastiera non deve modificare i valori dei controlli o causare un messaggio di errore.** Non richiedere mai agli utenti di modificare il valore iniziale di un controllo durante la navigazione. Inizializzare invece i controlli che convalidano all'uscita con valori validi e convalidare il valore di un controllo solo quando è stato modificato.

### <a name="access-keys"></a>Chiavi di accesso

-   **Quando possibile, assegnare le chiavi di accesso per i comandi di uso comune in base alla tabella seguente.** Anche se le assegnazioni coerenti delle chiavi di accesso non sono sempre possibili, sono sicuramente preferibili soprattutto per i comandi usati di frequente.

    |  Chiave di accesso         | Comando                             |
    |---------------------------|-------------------------------------------|
    | A<br/>              | Informazioni<br/>                          |
    | A<br/>              | Always on top (Sempre in alto)<br/>                  |
    | A<br/>              | Applica<br/>                          |
    | B<br/>              | Indietro<br/>                           |
    | B<br/>              | Bold<br/>                           |
    | B o r<br/>         | Esplora<br/>                         |
...
-   **Semplificare l'individuazione delle chiavi di accesso:**
    ...
    -   Per le voci di menu usate meno di frequente, scegliere le lettere che sono una consonante distintiva o una vocale nell'etichetta.
...
-   **Quando possibile, assegnare chiavi di accesso univoche a tutti i controlli interattivi o alle relative etichette.** [Le caselle di testo di sola](ctrl-text-boxes.md) lettura sono controlli interattivi (perché gli utenti possono scorrerle e copiare testo), quindi traggono vantaggio dai tasti di scelta. **Non assegnare chiavi di accesso a:**
    ...
    -   **Pulsanti della Guida generica,** accessibili con F1.
...
-   **Non distinguere tra le posizioni delle chiavi.** Ad esempio, Windows possibile distinguere tra MAIUSC sinistro e destro, ALT, CTRL, [logo Windows](glossary.md)e tasti dell'applicazione, nonché i tasti del tastierino numerico. [](glossary.md) L'assegnazione di un comportamento a una sola posizione chiave è confusa e imprevista.
-   **Non usare il tasto di modifica Windows logo per i tasti di scelta rapida del programma.** Windows chiave del logo è riservata per l Windows appreso. Anche se Windows combinazione di tasti logo non viene attualmente usata da Windows, potrebbe essere in futuro.
-   **Non usare il tasto applicazione come modificatore del tasto di scelta rapida.** Usare invece CTRL, ALT e MAIUSC.
-   **Non usare i tasti di scelta rapida usati da Windows per i tasti di scelta rapida del programma.** Questa operazione sarà in conflitto con i tasti Windows di sistema quando il programma ha lo stato attivo per l'input.
...
-   **Non usare combinazioni CTRL+ALT,** perché Windows interpreta questa combinazione in alcune versioni del linguaggio come un tasto ALTGR, che genera caratteri alfanumerici.
...
-   Assegnare le combinazioni di tasti a partire dal tasto di modifica. Presentare i tasti di modifica nell'ordine seguente: Windows logo, Applicazione, CTRL, ALT, MAIUSC. Se viene usato il modificatore Numpad, inseriscilo subito prima del tasto modificato.
-   Non usare tutte le lettere maiuscole per i tasti della tastiera. Seguire invece le maiuscole usate dalle tastiere standard o minuscole se il tasto non è etichettato sulla tastiera.
    ...
    -   Usare Windows logo e Chiave applicazione per fare riferimento alle chiavi etichettate con icone. Non usare etichette grafiche per queste chiavi.

**Corretto:**

barra spaziatrice, TAB, INVIO, PRS, CTRL+ALT+CANC, ALT+W, CTRL+segno più
...
-   Per visualizzare una combinazione di tasti che include la punteggiatura che richiede l'uso del tasto MAIUSC, ad esempio il punto interrogativo, aggiungere MAIUSC alla combinazione e assegnare il nome o il simbolo del tasto spostato. L'uso del nome della chiave non impostata, ad esempio 4 anziché $, potrebbe generare confusione per gli utenti o addirittura sbagliato. ad esempio? i caratteri e /non sono sempre i tasti spostati su ogni tastiera.
...
-   Prima di tutto, usare il tasto e con il nome della chiave, se necessario, per maggiore chiarezza, ad esempio il tasto F1. Per tutti i riferimenti successivi, fare riferimento alla chiave solo in base al nome, ad esempio premere F1.
-   Fare riferimento in particolare ai tasti di scelta rapida e ai tasti di scelta rapida nella programmazione e in altre documentazione tecnica. Non usare tasti di scelta rapida, mnemoici o tasti di scelta rapida. Ovunque si usino i tasti di scelta rapida, in particolare nella documentazione dell'utente.

Quando si fa riferimento all'interazione:

-   Quando si preme e si rilascia immediatamente un tasto, si avvia un'azione all'interno del programma o ci si sposta all'interno di un documento o di un'interfaccia utente.
...
-   Non usare press come sinonimo di clic.
