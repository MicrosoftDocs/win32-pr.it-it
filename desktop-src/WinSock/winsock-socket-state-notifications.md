...
|API|Descrizione|
|-|-|
|[**Funzione ProcessSocketNotifications**](/windows/win32/api/winsock2/nf-winsock2-processsocketnotifications)|Associa un set di socket a una porta di completamento e recupera le notifiche già in sospeso su tale porta. Una volta associata, la porta di completamento riceve le notifiche sullo stato del socket specificate.|
|[**SOCK_NOTIFY_REGISTRATION**](/windows/win32/api/winsock2/ns-winsock2-sock_notify_registration) struttura|Rappresenta le informazioni fornite alla **funzione ProcessSocketNotifications.**|
|[**Funzione SocketNotificationRetrieveEvents**](/windows/win32/api/winsock2/nf-winsock2-socketnotificationretrieveevents)|Questa funzione helper inline viene fornita per comodità per recuperare la maschera degli eventi da un [**OVERLAPPED_ENTRY**](/windows/win32/api/minwinbase/ns-minwinbase-overlapped_entry).|

Il flusso di lavoro inizia con l'associazione di socket a una porta di completamento I/O ([**ProcessSocketNotifications**](/windows/win32/api/winsock2/nf-winsock2-processsocketnotifications) [**e SOCK_NOTIFY_REGISTRATION**](/windows/win32/api/winsock2/ns-winsock2-sock_notify_registration)). Successivamente, la porta fornisce informazioni sulle modifiche dello stato del socket usando i normali metodi di query della porta di completamento I/O.

Queste API consentono di creare facilmente astrazioni indipendenti dalla piattaforma. Di conseguenza, sono supportati i flag persistenti e one-shot e i flag attivati dal livello e dall'edge. Ad esempio, le registrazioni attivate a livello one-shot sono il modello consigliato per i server multi-thread.
...
Di seguito sono riportati esempi di codice che illustrano alcuni scenari di notifica dello stato del socket. Parte del codice contiene *per eseguire elementi* per le proprie applicazioni.

## <a name="common-code"></a>Codice comune

Prima di tutto, ecco un listato di codice che contiene alcune definizioni e funzioni comuni usate dagli scenari seguenti.
...
## <a name="simple-replacement-for-polling"></a>Sostituzione semplice per il polling

Questo scenario illustra una sostituzione dell'accesso per le applicazioni che usano il polling ([**WSAPoll**](/windows/win32/api/winsock2/nf-winsock2-wsapoll)) o API simili. È a thread singolo e usa registrazioni persistenti (non singole). Poiché non è necessario registrare nuovamente la registrazione, usa [**GetQueuedCompletionStatusEx**](/windows/win32/fileio/getqueuedcompletionstatusex-func) per la disattivazione della coda delle notifiche.
...
## <a name="edge-triggered-udp-server"></a>Server UDP attivato da Edge

Questa è una semplice illustrazione dell'uso delle API con l'attivazione dei bordi.
...
## <a name="multi-threaded-server"></a>Server multi-thread
