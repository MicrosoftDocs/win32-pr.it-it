...
---
# <a name="registering-for-device-notification"></a>Registrazione per la notifica del dispositivo

Il sistema operativo può inviare messaggi di notifica che rappresentano gli eventi generati da un dispositivo. Il sistema invia tali notifiche a una finestra o a un servizio Windows. È possibile eseguire la registrazione per ricevere le notifiche chiamando la funzione [**RegisterDeviceNotification**](/windows/win32/api/winuser/nf-winuser-registerdevicenotificationw) .

Questo argomento fornisce un esempio di codice che illustra come eseguire la registrazione per le notifiche del dispositivo. È possibile identificare il dispositivo per ricevere notifiche specificando una classe dispositivo quando si chiama **RegisterDeviceNotification**. E la classe Device viene identificata in base al relativo GUID.

## <a name="source-code-listing"></a>Listato di codice sorgente

Se si vuole compilare ed eseguire questo esempio di codice sorgente come app, iniziare creando un nuovo progetto in Microsoft Visual Studio. Creare un nuovo progetto basato sul modello di progetto **applicazione desktop di Windows** . Denominare il progetto *RegisterDeviceNotification*.

Apri `RegisterDeviceNotification.cpp`, elimina l'intero contenuto e incolla il listato riportato di seguito.
...
## <a name="about-the-code-example"></a>Informazioni sull'esempio di codice

L'esempio di codice usa un handle di finestra per le notifiche di modifica del dispositivo e contiene una grande quantità di codice solo per la gestione dell'infrastruttura di windowing. Un'applicazione destinata a essere installata come servizio Windows conterrebbe un codice di infrastruttura diverso.

L'applicazione riceverà il messaggio di [**WM_DEVICECHANGE**](./wm-devicechange.md) ogni volta che viene inviata una notifica degli eventi di interfaccia del dispositivo USB.

Il sistema trasmette un set di eventi di modifica del dispositivo predefiniti a tutte le applicazioni e i servizi. Non è necessario eseguire la registrazione per ricevere questi eventi predefiniti. Per informazioni dettagliate, vedere la sezione **osservazioni** in [**RegisterDeviceNotification**](/windows/win32/api/winuser/nf-winuser-registerdevicenotificationw).

## <a name="related-topics"></a>Argomenti correlati

* [Notifiche del dispositivo](./device-notifications.md)