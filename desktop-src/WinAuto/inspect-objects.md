...
**Inspect** viene installato come parte del set complessivo di strumenti nel Windows Software Development Kit (Windows SDK) (SDK), non viene distribuito come download separato. Il Windows SDK include tutti gli strumenti correlati all'accessibilità documentati in questa sezione.

[Scaricare il Windows SDK](https://developer.microsoft.com/en-us/windows/downloads/windows-10-sdk/).

> [!NOTE]
> Per le versioni precedenti del Windows SDK, vedere l'archivio Windows SDK [e dell'emulatore](https://developer.microsoft.com/en-us/windows/downloads/sdk-archive/).
...
1. Scegliere **Impostazioni** dal menu Opzioni **oppure** Selezionare **Mostra finestra di dialogo impostazioni** sulla barra degli strumenti.
...
| Visualizzazione solo finestre visibili    | Disponibile solo in modalità MSAA.                                                                                                                                                                                                       |
...
| Visualizzazione controlli                 | Presenta la [visualizzazione controlli](uiauto-treeoverview.md) dell'Automazione interfaccia utente albero nella **visualizzazione** Albero. Disponibile solo Automazione interfaccia utente modalità predefinita.                                                                            |
| Visualizzazione contenuto                 | Presenta la [visualizzazione contenuto](uiauto-treeoverview.md) dell'Automazione interfaccia utente albero nella **visualizzazione** Albero. Disponibile solo Automazione interfaccia utente modalità predefinita.                                                                            |
...
| Segnale acustico in base all'errore                | Effemi quando viene rilevato un errore durante un'Automazione interfaccia utente o msaa.                                                                                                                                                          |
...
| Stato attivo dell'orologio                  | Segue lo stato attivo della tastiera. Quando questa opzione è selezionata, viene installato un hook di evento dello stato attivo asincrono e il cursore viene spostato in alto a sinistra dell'elemento con lo stato attivo. In questo **modo Inspect** aggiorna le proprietà in circa un secondo. |
| Guarda il caret                  | Segue il caret. Disponibile solo in modalità MSAA.                                                                                                                                                                                    |
| Cursore espressioni di controllo                 | Segue il cursore.                                                                                                                                                                                                                |
| Descrizioni comando di Espressioni di controllo               | Segue le descrizioni comando.                                                                                                                                                                                                              |
| Mostra albero                    | Visualizza la **visualizzazione** Albero.                                                                                                                                                                                                        |

## <a name="verifying-accessibility-navigation"></a>Verifica dell'accessibilità di spostamento

Dopo aver selezionato un elemento dell'interfaccia utente usando **Inspect**, è possibile verificare che l'elemento esponga la navigazione corretta di Automazione di Windows per assistive technology prodotti.

### <a name="verify-accessibility-navigation"></a>Verificare la navigazione dell'accessibilità

1. Aprire **Inspect** e l'applicazione da testare.
...
4. Usare la **visualizzazione Albero,** il menu **Di** spostamento o i pulsanti di spostamento sulla barra degli strumenti per esplorare l'interfaccia utente e verificare che ogni elemento esponga le proprietà corrette correlate alla navigazione.
    > [!Note]  
...
Automazione di Windows espone metodi che consentono assistive technology prodotti di interagire con un elemento dell'interfaccia utente come se si usa il mouse o la tastiera (ad esempio, per fare clic su un pulsante). Il menu **Ispeziona** azione consente ai tester di richiamare i metodi di Automazione di Windows su un elemento, ad esempio **Invoke.Invoke** chiama il metodo [**IUIAutomationInvokePattern::Invoke.**](/windows/desktop/api/UIAutomationClient/nf-uiautomationclient-iuiautomationinvokepattern-invoke)
...
3. Dal menu **Azione o** dalla barra degli strumenti selezionare l'azione corrispondente al metodo di Automazione di Windows da richiamare.

Il menu **Azione** contiene  gli **elementi Aggiorna** e Stato attivo, insieme ad altri elementi che variano a seconda Automazione interfaccia utente modalità msaa o in modalità MSAA selezionata. In Automazione interfaccia utente, gli altri elementi riflettono i pattern di controllo supportati dall'elemento dell'interfaccia utente attualmente selezionato. In modalità MSAA, gli altri elementi sono sempre costituiti dagli elementi seguenti:

| Azione                | Descrizione                                                                                                           |
|-----------------------|-----------------------------------------------------------------------------------------------------------------------|
| Aggiorna               | Aggiorna l'interfaccia utente. Disponibile in msaa e Automazione interfaccia utente modalità predefinita.                                               |
| Azione predefinita        | Esegue l'azione predefinita per l'elemento .                                                                          |
| Focus                 | Imposta lo stato attivo sull'elemento. Disponibile in msaa e Automazione interfaccia utente modalità predefinita.                                                  |
| Seleziona                | Seleziona l'elemento.                                                                                                  |
| Estendere la selezione      | Estende la selezione degli elementi per includere tutti gli elementi tra il primo elemento selezionato e l'elemento corrente. |
...
| Figlio con stato attivo         | Passa all'elemento figlio dell'elemento che attualmente ha lo stato attivo.                                                       |
...
|--------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
| Richiamare l'azione predefinita dell'oggetto sotto il cursore (Do Default Action). Disponibile solo in modalità MSAA.                                       | CTRL+MAIUSC+F2              |
...
| Passare all'oggetto sotto il cursore del mouse (HitTest at Cursor). Disponibile solo in modalità MSAA.                                                      | CTRL+MAIUSC+1               |
...
| Osservare l'oggetto con lo stato attivo (Watch Focus).                                                                                                   | CTRL+MAIUSC+6               |
