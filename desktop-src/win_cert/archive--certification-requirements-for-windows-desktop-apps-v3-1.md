...
---
# <a name="archive-certification-requirements-for-windows-desktop-apps-v31"></a>Archivio: Requisiti di certificazione per App desktop di Windows v3.1

**Versione del documento:** 3.1

**Data del documento:** 18 giugno 2013

Questo documento contiene i requisiti tecnici e le qualifica di idoneità che un'app desktop deve soddisfare per partecipare al programma di certificazione app desktop Windows 8.1 desktop. Per Windows 7, questo programma era noto come Windows Software Logo Program.
...
Perché un'app sia idonea Windows 8.1 la certificazione dell'app desktop, deve soddisfare i criteri seguenti e tutti i requisiti tecnici elencati in questo documento.
...
-   Non deve comunicare con le app di Windows Store tramite meccanismi locali, inclusi file e chiavi del Registro di sistema
...
1.5 L'app non deve caricare DLL arbitrarie per intercettare le chiamate API Win32 usando HKLM \\ Software \\ Microsoft Windows NT \\ \\ CurrentVersion Windows \\ AppInit \_ dlls.  
</dl>
...
</dl> </dd> 5.2 Your app must never force the user to restart the computer immediately <dl> Il riavvio del computer non dovrebbe mai essere l'unica opzione al termine di un'installazione, una disinstallazione o un aggiornamento. Gli utenti dovrebbero avere la possibilità di riavviare in un secondo momento.  
...
Gli utenti di Windows devono essere in grado di eseguire sessioni simultanee senza conflitti o interruzioni.<dl> 11.1 L'app deve garantire che, in caso di esecuzione in più sessioni in locale o in remoto, la normale funzionalità dell'app non sia influenzata negativamente  
...
</dl><strong>Nota: se un'app non supporta più sessioni utente o l'accesso remoto, deve indicarlo chiaramente all'avvio </strong> da questo tipo di sessione.

## <a name="12-apps-must-support-x64-versions-of-windows"></a>12. Le app devono supportare le versioni x64 di Windows

Poiché l'hardware a 64 bit diventa più comune, gli utenti si aspettano che gli sviluppatori di app possano sfruttare i vantaggi dell'architettura a 64 bit eseguendo la migrazione delle app a 64 bit o che le versioni a 32 bit dell'app vengono eseguite bene nelle versioni a 64 bit di Windows.<dl> 12.1 L'app deve supportare in modo nativo le app a 64 bit o, come minimo, le app basate su Windows a 32 bit devono essere eseguite senza problemi nei sistemi a 64 bit per mantenere la compatibilità con le versioni a 64 bit di Windows  
