...
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a>Programma C di esempio: cofirma e decodifica di un messaggio

È possibile usare la funzione [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) per la firma di un messaggio. Questa operazione viene eseguita chiamando [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) una volta per firmare il messaggio originale, quindi chiamare di nuovo **CryptSignMessage** per firmare il messaggio firmato.

Quando si verifica la firma di un messaggio firmato, utilizzare la funzione [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) per ottenere il numero di firmatari del messaggio e quindi chiamare il [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) per ogni firma. Se tutte le firme sono state verificate, si sa che il messaggio con segno di cofirmato è valido.

Nell'esempio seguente viene illustrato come firmare un messaggio da più di una persona (cofirmare il messaggio), verificare tutte le firme e decodificare il messaggio.
