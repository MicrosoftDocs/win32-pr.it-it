---
description: È possibile usare la funzione CryptSignMessage per la firma di un messaggio.
ms.assetid: b400436f-a71f-426a-ac8a-7fdcfa6d7575
title: 'Programma C di esempio: cofirma e decodifica di un messaggio'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104343591"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="13334-103">Programma C di esempio: cofirma e decodifica di un messaggio</span><span class="sxs-lookup"><span data-stu-id="13334-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="13334-104">È possibile usare la funzione [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) per la firma di un messaggio.</span><span class="sxs-lookup"><span data-stu-id="13334-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="13334-105">Questa operazione viene eseguita chiamando [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) una volta per firmare il messaggio originale, quindi chiamare di nuovo **CryptSignMessage** per firmare il messaggio firmato.</span><span class="sxs-lookup"><span data-stu-id="13334-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="13334-106">Quando si verifica la firma di un messaggio firmato, utilizzare la funzione [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) per ottenere il numero di firmatari del messaggio e quindi chiamare il [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) per ogni firma.</span><span class="sxs-lookup"><span data-stu-id="13334-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="13334-107">Se tutte le firme sono state verificate, si sa che il messaggio con segno di cofirmato è valido.</span><span class="sxs-lookup"><span data-stu-id="13334-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="13334-108">Nell'esempio seguente viene illustrato come firmare un messaggio da più di una persona (cofirmare il messaggio), verificare tutte le firme e decodificare il messaggio.</span><span class="sxs-lookup"><span data-stu-id="13334-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
