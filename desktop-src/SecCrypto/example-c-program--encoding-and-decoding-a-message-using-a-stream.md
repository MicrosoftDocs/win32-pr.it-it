...
---
# <a name="example-c-program-encoding-and-decoding-a-message-using-a-stream"></a><span data-ttu-id="3d96b-103">Esempio di programma C: codifica e decodifica di un messaggio tramite un flusso</span><span class="sxs-lookup"><span data-stu-id="3d96b-103">Example C Program: Encoding and Decoding a Message Using a Stream</span></span>

<span data-ttu-id="3d96b-104">Nell'esempio seguente viene illustrato come usare le funzioni [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)e [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate) con la struttura [**delle \_ \_ informazioni di flusso di CMSG**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) per codificare e decodificare un messaggio usando le funzionalità di streaming di queste funzioni.</span><span class="sxs-lookup"><span data-stu-id="3d96b-104">The following example demonstrates how to use the [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode), and [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate) functions with the [**CMSG\_STREAM\_INFO**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) structure to encode and decode a message using the streaming features of these functions.</span></span>

<span data-ttu-id="3d96b-105">La firma e la codifica di un messaggio non garantiscono la privacy del messaggio.</span><span class="sxs-lookup"><span data-stu-id="3d96b-105">Signing and encoding a message does not ensure privacy for that message.</span></span> <span data-ttu-id="3d96b-106">Ma garantisce l'autenticità del messaggio.</span><span class="sxs-lookup"><span data-stu-id="3d96b-106">Rather it ensures the authenticity of the message.</span></span> <span data-ttu-id="3d96b-107">Poiché il messaggio viene firmato con la chiave privata del mittente, quando il ricevitore del messaggio decrittografa la firma con la [*chiave pubblica*](../secgloss/p-gly.md) del mittente (disponibile dal certificato inviato insieme al messaggio), il destinatario può essere sicuro che il messaggio sia stato inviato dalla persona o dall'entità associata al certificato e che il messaggio non sia stato modificato dopo la firma.</span><span class="sxs-lookup"><span data-stu-id="3d96b-107">Because the message is signed with the sender's private key, when the receiver of the message decrypts the signature with the sender's [*public key*](../secgloss/p-gly.md) (available from the certificate that is sent along with the message), the receiver can be sure that the message was sent by the person or entity associated with the certificate and that the message was not changed after it was signed.</span></span>

<span data-ttu-id="3d96b-108">Questa parte relativa alla firma della codifica di questo esempio illustra le attività e le funzioni CryptoAPI seguenti:</span><span class="sxs-lookup"><span data-stu-id="3d96b-108">This encoding signing portion of this example illustrates the following tasks and CryptoAPI functions:</span></span>

-   <span data-ttu-id="3d96b-109">Apertura di un archivio certificati utilizzando [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).</span><span class="sxs-lookup"><span data-stu-id="3d96b-109">Opening a certificate store by using [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).</span></span>
-   <span data-ttu-id="3d96b-110">Recupero di un certificato con un nome soggetto specifico utilizzando [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).</span><span class="sxs-lookup"><span data-stu-id="3d96b-110">Retrieving a certificate with a specific subject name by using [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).</span></span>
-   <span data-ttu-id="3d96b-111">Recupero e stampa del nome del soggetto di un certificato tramite [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).</span><span class="sxs-lookup"><span data-stu-id="3d96b-111">Getting and printing a certificate's subject name by using [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).</span></span>
-   <span data-ttu-id="3d96b-112">Recupero dell'handle a un provider di crittografia in grado di fornire una chiave privata con la funzione [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey) .</span><span class="sxs-lookup"><span data-stu-id="3d96b-112">Getting the handle to a cryptographic provider that can provide a private key with the [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey) function.</span></span>
-   <span data-ttu-id="3d96b-113">Inizializzazione [**delle \_ \_ \_ informazioni di codifica CMSG firmate**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_signed_encode_info) e delle strutture di [**\_ \_ informazioni del flusso CMSG**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) da usare in una chiamata a [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode).</span><span class="sxs-lookup"><span data-stu-id="3d96b-113">Initializing the [**CMSG\_SIGNED\_ENCODE\_INFO**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_signed_encode_info) and [**CMSG\_STREAM\_INFO**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) structures to be used in a call to [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode).</span></span>
-   <span data-ttu-id="3d96b-114">Firma e codifica di un messaggio tramite [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode) e [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).</span><span class="sxs-lookup"><span data-stu-id="3d96b-114">Signing and encoding a message by using [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode) and [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).</span></span>
-   <span data-ttu-id="3d96b-115">Implementazione di una funzione di callback del flusso in grado di salvare un messaggio codificato e firmato in qualsiasi formato persistente, ad esempio scrivendolo in un file.</span><span class="sxs-lookup"><span data-stu-id="3d96b-115">Implementing a stream callback function that can save an encoded and signed message in any persistent format, such as writing it to a file.</span></span>

<span data-ttu-id="3d96b-116">Nella parte relativa alla decodifica di questo esempio vengono illustrate le attività e le funzioni CryptoAPI seguenti:</span><span class="sxs-lookup"><span data-stu-id="3d96b-116">The decoding portion of this example illustrates the following tasks and CryptoAPI functions:</span></span>

-   <span data-ttu-id="3d96b-117">Inizializzazione di una struttura di [**\_ \_ informazioni del flusso CMSG**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) da utilizzare in una chiamata a [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode).</span><span class="sxs-lookup"><span data-stu-id="3d96b-117">Initializing a [**CMSG\_STREAM\_INFO**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) structure to be used in a call to [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode).</span></span>
-   <span data-ttu-id="3d96b-118">Implementazione di una funzione di callback del flusso che può salvare un messaggio decodificato in qualsiasi formato persistente, ad esempio stamparlo sullo schermo.</span><span class="sxs-lookup"><span data-stu-id="3d96b-118">Implementing a stream callback function that can save a decoded message in any persistent format, such as printing it to the screen.</span></span>
-   <span data-ttu-id="3d96b-119">Lettura di un messaggio codificato da un file e decodifica del messaggio tramite [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).</span><span class="sxs-lookup"><span data-stu-id="3d96b-119">Reading an encoded message from a file and decoding the message by using [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).</span></span>

<span data-ttu-id="3d96b-120">Per un esempio di come eseguire queste stesse operazioni senza usare un callback di flusso, vedere [esempio di programma C: firma, codifica, decodifica e verifica di un messaggio](example-c-program-signing-encoding-decoding-and-verifying-a-message.md).</span><span class="sxs-lookup"><span data-stu-id="3d96b-120">For an example of how to perform these same operations without using a stream callback, see [Example C Program: Signing, Encoding, Decoding, and Verifying a Message](example-c-program-signing-encoding-decoding-and-verifying-a-message.md).</span></span>

<span data-ttu-id="3d96b-121">In questo esempio viene usata la funzione [**MyHandleError**](myhandleerror.md).</span><span class="sxs-lookup"><span data-stu-id="3d96b-121">This example uses the function [**MyHandleError**](myhandleerror.md).</span></span> <span data-ttu-id="3d96b-122">Il codice per questa funzione è incluso nell'esempio.</span><span class="sxs-lookup"><span data-stu-id="3d96b-122">Code for this function is included with the sample.</span></span> <span data-ttu-id="3d96b-123">Il codice per questa e altre funzioni ausiliarie è elencato anche in [ \_ \_ funzioni di utilizzo generico](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="3d96b-123">Code for this and other auxiliary functions is also listed under [General\_Purpose\_Functions](general-purpose-functions.md).</span></span>
