...
title: 'Esempio di programma C: uso di CryptEncryptMessage e CryptDecryptMessage'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104569227"
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="766b4-103">Esempio di programma C: uso di CryptEncryptMessage e CryptDecryptMessage</span><span class="sxs-lookup"><span data-stu-id="766b4-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="766b4-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) è l'unica chiamata di funzione necessaria per eseguire tutte le attività elencate in [crittografia di un messaggio](../secauthn/encrypting-a-message.md).</span><span class="sxs-lookup"><span data-stu-id="766b4-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="766b4-105">L'inizializzazione delle strutture di dati è necessaria.</span><span class="sxs-lookup"><span data-stu-id="766b4-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="766b4-106">Nella figura seguente viene illustrata la relazione tra i parametri di funzione che puntano a strutture o matrici e i relativi dati inizializzati.</span><span class="sxs-lookup"><span data-stu-id="766b4-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="766b4-107">In questo esempio viene inoltre decrittografato il messaggio utilizzando [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span><span class="sxs-lookup"><span data-stu-id="766b4-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![Mappa di inizializzazione per una chiamata a cryptencryptmessage](images/crypenc.png)

<span data-ttu-id="766b4-109">**Per crittografare i dati**</span><span class="sxs-lookup"><span data-stu-id="766b4-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="766b4-110">Ottenere un puntatore al contenuto da crittografare.</span><span class="sxs-lookup"><span data-stu-id="766b4-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="766b4-111">Determinare le dimensioni del contenuto da crittografare.</span><span class="sxs-lookup"><span data-stu-id="766b4-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="766b4-112">Acquisire un handle per un provider di crittografia.</span><span class="sxs-lookup"><span data-stu-id="766b4-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="766b4-113">Aprire un [*archivio certificati*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="766b4-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="766b4-114">Ottenere il certificato del destinatario.</span><span class="sxs-lookup"><span data-stu-id="766b4-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="766b4-115">Creare la matrice di certificati del destinatario.</span><span class="sxs-lookup"><span data-stu-id="766b4-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="766b4-116">Inizializza la struttura dell' [**\_ \_ identificatore dell'algoritmo di crittografia**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) .</span><span class="sxs-lookup"><span data-stu-id="766b4-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="766b4-117">Inizializzare la struttura del [**\_ messaggio di crittografia \_ \_ crypt**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) .</span><span class="sxs-lookup"><span data-stu-id="766b4-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="766b4-118">Chiamare [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) per crittografare il contenuto e creare un messaggio in busta digitale.</span><span class="sxs-lookup"><span data-stu-id="766b4-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="766b4-119">Nell'esempio seguente viene implementata questa procedura.</span><span class="sxs-lookup"><span data-stu-id="766b4-119">The following example implements this procedure.</span></span>

<span data-ttu-id="766b4-120">I commenti correlano i frammenti di codice a ogni passaggio della procedura.</span><span class="sxs-lookup"><span data-stu-id="766b4-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="766b4-121">Per ulteriori informazioni sulla funzione, vedere [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="766b4-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="766b4-122">Per ulteriori informazioni sulle strutture dei dati, vedere [**crypt \_ algorithm \_ Identifier**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**crypt \_ Encrypt \_ message \_ para**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="766b4-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="766b4-123">Questo esempio usa le funzioni [**MyHandleError**](myhandleerror.md) e [**ByteToStr**](bytetostr.md).</span><span class="sxs-lookup"><span data-stu-id="766b4-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="766b4-124">Il codice per questa funzione è incluso nell'esempio.</span><span class="sxs-lookup"><span data-stu-id="766b4-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="766b4-125">Il codice per questa e altre funzioni ausiliarie è elencato anche in [funzioni per utilizzo generico](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="766b4-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
