...
---
# <a name="example-c-program-using-cryptacquirecontext"></a>Esempio di programma C: uso di CryptAcquireContext

Nell'esempio seguente vengono illustrati diversi modi per utilizzare [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) e le funzioni CryptoAPI correlate per utilizzare un [*provider del servizio di crittografia*](../secgloss/c-gly.md) (CSP) e un [*contenitore di chiavi*](../secgloss/k-gly.md).

In questo esempio vengono illustrate le attività e le funzioni CryptoAPI seguenti:

-   Utilizzare la funzione [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) per acquisire un handle per il CSP predefinito e il contenitore di chiavi predefinito. Se non esiste un contenitore di chiavi predefinito, usare la funzione **CryptAcquireContext** per creare il contenitore di chiavi predefinito.
-   Utilizzare la funzione [**CryptGetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetprovparam) per recuperare informazioni su un provider CSP e un contenitore di chiavi.
-   Aumentare il [*numero di riferimenti*](../secgloss/r-gly.md) nel provider utilizzando la funzione [**CryptContextAddRef**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptcontextaddref) .
-   Rilasciare un CSP utilizzando la funzione [**CryptReleaseContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptreleasecontext) .
-   Creare un contenitore di chiavi denominato usando la funzione [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) .
-   Acquisire un handle per un CSP usando il contenitore di chiavi appena creato.
-   Eliminare un contenitore di chiavi tramite la funzione [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) .
