...
---
# <a name="example-c-program-using-certoidtoalgid-and-certcompareintegerblob"></a><span data-ttu-id="1ba8f-103">Esempio di programma C: uso di CertOIDToAlgId e CertCompareIntegerBlob</span><span class="sxs-lookup"><span data-stu-id="1ba8f-103">Example C Program: Using CertOIDToAlgId and CertCompareIntegerBlob</span></span>

<span data-ttu-id="1ba8f-104">Nell'esempio seguente viene illustrato l'utilizzo delle funzioni [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) e [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) .</span><span class="sxs-lookup"><span data-stu-id="1ba8f-104">The following example demonstrates using the [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) and [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) functions.</span></span>

<span data-ttu-id="1ba8f-105">Per prima cosa, tutti i OID disponibili vengono enumerati usando [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo).</span><span class="sxs-lookup"><span data-stu-id="1ba8f-105">First, all available OIDs are enumerated using [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo).</span></span> <span data-ttu-id="1ba8f-106">Il codice usato con questa funzione illustra anche l'uso di una funzione di callback.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-106">Code used with this function also demonstrates the use of a callback function.</span></span> <span data-ttu-id="1ba8f-107">La funzione di callback Mostra la logica di interruzione per sospendere tra ogni gruppo OID e dopo aver presentato le informazioni su un numero set di OID.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-107">The callback function demonstrates break logic to pause between each OID group and after presenting information on a set number of OIDs.</span></span>

<span data-ttu-id="1ba8f-108">In secondo luogo, tre stringhe dell' [*identificatore di oggetto*](../secgloss/o-gly.md) (OID) vengono convertite in integer con identificatore di algoritmo **DWORD** usando [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid).</span><span class="sxs-lookup"><span data-stu-id="1ba8f-108">Second, three [*object identifier*](../secgloss/o-gly.md) (OID) strings are converted into **DWORD** algorithm identifier integers using [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid).</span></span> <span data-ttu-id="1ba8f-109">Il codice dimostra anche che tutte le stringhe OID non hanno identificatori di algoritmo correlati.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-109">The code also demonstrates that all OID strings do not have related algorithm identifiers.</span></span>

<span data-ttu-id="1ba8f-110">Infine, nell'esempio viene illustrato il confronto dei BLOB di interi.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-110">Finally, the example demonstrates comparing integer BLOBs.</span></span> <span data-ttu-id="1ba8f-111">In questo esempio viene illustrato il troncamento di 0x00's iniziali da numeri positivi e 0xFF's iniziali da numeri negativi.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-111">This example demonstrates the truncation of leading 0x00's from positive numbers and leading 0xFF's from negative numbers.</span></span>

<span data-ttu-id="1ba8f-112">Mostra anche che i numeri interi vengono confrontati come se fossero archiviati in formato [*Little-Endian*](../secgloss/l-gly.md) con le cifre più significative a destra.</span><span class="sxs-lookup"><span data-stu-id="1ba8f-112">It also shows that integers are compared as though they are stored in [*little-endian*](../secgloss/l-gly.md) form with the most significant digits on the right.</span></span>
