...
---
# <a name="example-c-program-sending-and-receiving-a-signed-and-encrypted-message"></a><span data-ttu-id="2c08f-103">Esempio di programma C: invio e ricezione di un messaggio firmato e crittografato</span><span class="sxs-lookup"><span data-stu-id="2c08f-103">Example C Program: Sending and Receiving a Signed and Encrypted Message</span></span>

<span data-ttu-id="2c08f-104">Nell'esempio seguente viene firmato un messaggio utilizzando la [*chiave privata*](../secgloss/p-gly.md) di un mittente e viene crittografato il messaggio firmato utilizzando la [*chiave pubblica*](../secgloss/p-gly.md)di un destinatario.</span><span class="sxs-lookup"><span data-stu-id="2c08f-104">The following example signs a message using a sender's [*private key*](../secgloss/p-gly.md) and encrypts the signed message using a receiver's [*public key*](../secgloss/p-gly.md).</span></span> <span data-ttu-id="2c08f-105">Nell'esempio viene quindi decrittografato il messaggio utilizzando la chiave privata del destinatario e viene verificata la firma utilizzando la chiave pubblica del mittente.</span><span class="sxs-lookup"><span data-stu-id="2c08f-105">The example then decrypts the message using the receiver's private key and verifies the signature using the sender's public key.</span></span> <span data-ttu-id="2c08f-106">Il certificato del mittente contenente la chiave pubblica necessaria è incluso nel messaggio crittografato.</span><span class="sxs-lookup"><span data-stu-id="2c08f-106">The sender's certificate containing the needed public key is included in the encrypted message.</span></span> <span data-ttu-id="2c08f-107">Questo esempio scrive anche il messaggio firmato e crittografato in un file.</span><span class="sxs-lookup"><span data-stu-id="2c08f-107">This example also writes the signed and encrypted message to a file.</span></span> <span data-ttu-id="2c08f-108">Per ulteriori informazioni, vedere [esempio C Program: ricezione di un messaggio firmato e crittografato](example-c-program-receiving-a-signed-and-encrypted-message.md).</span><span class="sxs-lookup"><span data-stu-id="2c08f-108">For more information, see [Example C Program: Receiving a Signed and Encrypted Message](example-c-program-receiving-a-signed-and-encrypted-message.md).</span></span>

<span data-ttu-id="2c08f-109">Per firmare il messaggio, è necessario che la chiave privata del firmatario e il certificato del firmatario siano disponibili.</span><span class="sxs-lookup"><span data-stu-id="2c08f-109">To sign the message, the signer's private key and the signer's certificate must be available.</span></span> <span data-ttu-id="2c08f-110">Per crittografare il messaggio firmato, è necessario che sia disponibile un certificato del ricevitore che includa la chiave pubblica del destinatario.</span><span class="sxs-lookup"><span data-stu-id="2c08f-110">To encrypt the signed message, a receiver's certificate including the receiver's public key must be available.</span></span>

<span data-ttu-id="2c08f-111">Per decrittografare il messaggio, è necessario che sia disponibile la chiave privata del destinatario.</span><span class="sxs-lookup"><span data-stu-id="2c08f-111">To decrypt the message, the receiver's private key must be available.</span></span> <span data-ttu-id="2c08f-112">Dopo la decrittografia del messaggio, la firma viene verificata utilizzando la chiave pubblica del certificato incluso nel messaggio crittografato.</span><span class="sxs-lookup"><span data-stu-id="2c08f-112">After the message is decrypted, the signature is verified using the public key from the certificate included in the encrypted message.</span></span>

> [!Note]  
> <span data-ttu-id="2c08f-113">Non tutti i certificati in un [*archivio certificati*](../secgloss/c-gly.md) forniscono l'accesso alla [*chiave privata*](../secgloss/p-gly.md) associata al certificato.</span><span class="sxs-lookup"><span data-stu-id="2c08f-113">Not all of the certificates in a [*certificate store*](../secgloss/c-gly.md) provide access to the [*private key*](../secgloss/p-gly.md) associated with that certificate.</span></span> <span data-ttu-id="2c08f-114">Quando il messaggio viene firmato e crittografato, è necessario usare un certificato che appartiene al firmatario con accesso alla chiave privata del firmatario.</span><span class="sxs-lookup"><span data-stu-id="2c08f-114">When the message is signed and encrypted, a certificate belonging to the signer with access to the private key of that signer must be used.</span></span> <span data-ttu-id="2c08f-115">Inoltre, il ricevitore del messaggio deve avere accesso alla chiave privata associata alla chiave pubblica usata per crittografare il messaggio.</span><span class="sxs-lookup"><span data-stu-id="2c08f-115">In addition, the receiver of the message must have access to the private key associated with the public key used to encrypt the message.</span></span>

 

<span data-ttu-id="2c08f-116">In questo esempio vengono illustrate le attività seguenti:</span><span class="sxs-lookup"><span data-stu-id="2c08f-116">This example illustrates the following tasks:</span></span>

-   <span data-ttu-id="2c08f-117">Apertura e chiusura di archivi certificati di sistema.</span><span class="sxs-lookup"><span data-stu-id="2c08f-117">Opening and closing system certificate stores.</span></span>
-   <span data-ttu-id="2c08f-118">Ricerca di certificati per un mittente e un ricevitore di messaggi negli archivi certificati aperti.</span><span class="sxs-lookup"><span data-stu-id="2c08f-118">Finding certificates for a message sender and message receiver in the open certificate stores.</span></span>
-   <span data-ttu-id="2c08f-119">Ricerca e stampa del nome del soggetto dai certificati.</span><span class="sxs-lookup"><span data-stu-id="2c08f-119">Finding and printing the subject name from certificates.</span></span>
-   <span data-ttu-id="2c08f-120">Inizializzazione di strutture di dati necessarie per firmare, crittografare, decrittografare e verificare un messaggio.</span><span class="sxs-lookup"><span data-stu-id="2c08f-120">Initializing data structures needed to sign, encrypt, decrypt, and verify a message.</span></span>
-   <span data-ttu-id="2c08f-121">Chiamata di una funzione CryptoAPI per individuare le dimensioni richieste di un buffer, allocando il buffer della dimensione richiesta e richiamando la funzione CryptoAPI per riempire il buffer.</span><span class="sxs-lookup"><span data-stu-id="2c08f-121">Calling a CryptoAPI function to find the required size of a buffer, allocating the buffer of the required size, and calling the CryptoAPI function again to fill the buffer.</span></span> <span data-ttu-id="2c08f-122">Per altre informazioni, vedere [recupero di dati di lunghezza sconosciuta](retrieving-data-of-unknown-length.md).</span><span class="sxs-lookup"><span data-stu-id="2c08f-122">For more information, see [Retrieving Data of Unknown Length](retrieving-data-of-unknown-length.md).</span></span>
-   <span data-ttu-id="2c08f-123">Visualizzazione di parte del contenuto crittografato di un buffer.</span><span class="sxs-lookup"><span data-stu-id="2c08f-123">Displaying some of the encrypted contents of a buffer.</span></span> <span data-ttu-id="2c08f-124">La funzione locale inclusa, **ShowBytes**, Visualizza i caratteri nel buffer con valori compresi tra' 0' è z '.</span><span class="sxs-lookup"><span data-stu-id="2c08f-124">The included local function, **ShowBytes**, displays characters in the buffer with values between '0' and 'z'.</span></span> <span data-ttu-id="2c08f-125">Tutti gli altri caratteri vengono visualizzati come carattere "-".</span><span class="sxs-lookup"><span data-stu-id="2c08f-125">All other characters are displayed as the '-' character.</span></span>

<span data-ttu-id="2c08f-126">In questo esempio vengono utilizzate le funzioni CryptoAPI seguenti:</span><span class="sxs-lookup"><span data-stu-id="2c08f-126">This example uses the following CryptoAPI functions:</span></span>

-   [<span data-ttu-id="2c08f-127">**CertOpenStore**</span><span class="sxs-lookup"><span data-stu-id="2c08f-127">**CertOpenStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)
-   [<span data-ttu-id="2c08f-128">**CertFindCertificateInStore**</span><span class="sxs-lookup"><span data-stu-id="2c08f-128">**CertFindCertificateInStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)
-   [<span data-ttu-id="2c08f-129">**CertGetNameString**</span><span class="sxs-lookup"><span data-stu-id="2c08f-129">**CertGetNameString**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)
-   [<span data-ttu-id="2c08f-130">**CryptAcquireCertificatePrivateKey**</span><span class="sxs-lookup"><span data-stu-id="2c08f-130">**CryptAcquireCertificatePrivateKey**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey)
-   [<span data-ttu-id="2c08f-131">**CryptSignAndEncryptMessage**</span><span class="sxs-lookup"><span data-stu-id="2c08f-131">**CryptSignAndEncryptMessage**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignandencryptmessage)
-   [<span data-ttu-id="2c08f-132">**CryptDecryptAndVerifyMessageSignature**</span><span class="sxs-lookup"><span data-stu-id="2c08f-132">**CryptDecryptAndVerifyMessageSignature**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptandverifymessagesignature)
-   [<span data-ttu-id="2c08f-133">**CertFreeCertificateContext**</span><span class="sxs-lookup"><span data-stu-id="2c08f-133">**CertFreeCertificateContext**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatecontext)
-   [<span data-ttu-id="2c08f-134">**CertCloseStore**</span><span class="sxs-lookup"><span data-stu-id="2c08f-134">**CertCloseStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore)

<span data-ttu-id="2c08f-135">Questo esempio usa funzioni separate per visualizzare il processo di firma/crittografia e il processo di decrittografia/verifica della firma.</span><span class="sxs-lookup"><span data-stu-id="2c08f-135">This example uses separate functions to show the signing/encryption process and the decryption/signature-verification process.</span></span> <span data-ttu-id="2c08f-136">USA anche [**MyHandleError**](myhandleerror.md) per uscire normalmente dal programma in caso di errore.</span><span class="sxs-lookup"><span data-stu-id="2c08f-136">It also uses [**MyHandleError**](myhandleerror.md) to exit the program gracefully in case of any failure.</span></span> <span data-ttu-id="2c08f-137">Il codice **MyHandleError** è incluso nell'esempio e può anche essere trovato insieme ad altre funzioni ausiliarie in [funzioni per utilizzo generico](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="2c08f-137">The code **MyHandleError** is included with the example and can also be found along with other auxiliary functions under [General Purpose Functions](general-purpose-functions.md).</span></span>
