...
---
# <a name="requesting-a-key-archival-certificate"></a>Richiesta di un certificato di archiviazione delle chiavi

Nell'esempio seguente viene illustrata la creazione e l'invio di una [*richiesta di certificato*](../secgloss/c-gly.md) e la ricezione del certificato di archiviazione della chiave risultante. Affinché questo esempio abbia esito positivo, l' [*autorità di certificazione*](../secgloss/c-gly.md) (CA) che riceve la richiesta di certificato deve essere configurata per l'archiviazione delle chiavi.

Nei passaggi seguenti viene descritto come creare e inviare una richiesta di certificato.

**Per creare e inviare una richiesta di certificato**

1.  Recuperare il certificato di Exchange della CA usando il metodo [**ICertRequest2:: GetCACertificate**](/windows/desktop/api/Certcli/nf-certcli-icertrequest-getcacertificate) .
2.  Specificare che il certificato di scambio CA recuperato è il certificato di archiviazione delle chiavi utilizzando la proprietà [**ICEnroll4::P rivatekeyarchivecertificate**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-get_privatekeyarchivecertificate) .
3.  Creare una richiesta di certificato CMC usando il metodo [**ICEnroll4:: CreateRequest**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-createrequest) .
4.  Inviare la richiesta di certificato a una CA usando il metodo [**ICertRequest2:: Submit**](/windows/desktop/api/Certcli/nf-certcli-icertrequest-submit) . La CA deve essere configurata in modo da supportare l'archiviazione delle chiavi.

Nei passaggi seguenti viene descritto come recuperare il certificato emesso per scopi di archiviazione delle chiavi.

**Per recuperare il certificato emesso per scopi di archiviazione chiavi**

1.  Recuperare la risposta completa, incluso il certificato emesso, usando il metodo [**ICertRequest2:: GetFullResponseProperty**](/windows/desktop/api/Certcli/nf-certcli-icertrequest2-getfullresponseproperty) .
2.  Installare il certificato emesso usando il metodo [**ICEnroll4:: acceptResponse**](/windows/desktop/api/Xenroll/nf-xenroll-icenroll4-acceptresponse) .

Nell'esempio seguente viene illustrata la creazione e l'invio di una richiesta di certificato e la ricezione del certificato di archiviazione della chiave risultante.
