...
2.  Per informazioni dettagliate sulle voci della sottochiave **Percorsi** app, vedere la tabella seguente. 
    ...
    <tr class="even">
    <td>Percorso</td>
    <td>Fornisce una stringa (sotto forma di elenco di directory separate da punti e virgola) da aggiungere alla variabile di ambiente PATH quando un'applicazione viene avviata chiamando <a href="/windows/desktop/api/Shellapi/nf-shellapi-shellexecuteexa"><strong>ShellExecuteEx</strong></a>. Si tratta del percorso completo dell'.exe. È di <strong>REG_SZ</strong>. In <strong>Windows 7 e versioni successive,</strong>il tipo può essere <strong>REG_EXPAND_SZ</strong>, ed è in <strong>genere</strong> REG_EXPAND_SZ %ProgramFiles%.
    <blockquote>
    [!Note]<br />
