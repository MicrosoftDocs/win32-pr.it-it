---
description: Prima di chiamare una delle funzioni di database seguenti da un programma, ad esempio un'azione personalizzata o un processo di automazione, il programma di installazione deve prima eseguire l'azione CostInitialize, l'azione FileCost e l'azione CostFinalize.
...
ms.locfileid: "119145704"
---
# <a name="calling-database-functions-from-programs"></a>Chiamata di funzioni di database da programmi

Prima di chiamare una delle funzioni di [database](database-functions.md) seguenti da un programma, ad esempio un'azione personalizzata o un processo di automazione, il programma di installazione deve prima eseguire l'azione [CostInitialize](costinitialize-action.md), [l'azione FileCost](filecost-action.md)e l'azione [CostFinalize](costfinalize-action.md).
...
Prima di [**chiamare MsiSetFeatureAttributes**](/windows/desktop/api/Msiquery/nf-msiquery-msisetfeatureattributesa) da un programma, il programma di installazione deve eseguire l'azione CostInitialize. Il programma di installazione esegue quindi l'azione CostFinalize dopo **MsiSetFeatureAttributes**.

L'esempio seguente illustra l'ordine in cui devono essere chiamate le azioni della funzione quando si [**usa MsiGetTargetPath**](/windows/desktop/api/Msiquery/nf-msiquery-msigettargetpatha) in un programma.
