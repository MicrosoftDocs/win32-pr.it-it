...
-   La possibilità di usare l'annuncio è ridotta se la registrazione della classe o del server di estensione viene eseguita all'interno di routine di auto-registrazione.
...
-   A [**DllRegisterServer**](/windows/win32/api/olectl/nf-olectl-dllregisterserver) può essere negato l'accesso alle risorse di rete, ad esempio le librerie dei tipi, se un componente è specificato come run-from-source ed è elencato nella tabella SelfReg. Ciò può causare l'esito negativo dell'installazione del componente durante un'installazione amministrativa.
-   Le DLL autoregistrazione sono più soggette agli errori di codifica perché il nuovo codice necessario per [**DllRegisterServer**](/windows/win32/api/olectl/nf-olectl-dllregisterserver) è in genere diverso per ogni DLL. Usare invece le tabelle del Registro di sistema nel database per sfruttare il codice esistente fornito dal programma di installazione.
-   Le DLL autoregistrazione possono talvolta essere collegate a DLL ausiliarie che non sono presenti o sono nella versione errata. Al contrario, il programma di installazione può registrare le DLL usando le tabelle del Registro di sistema senza alcuna dipendenza dallo stato corrente del sistema.
