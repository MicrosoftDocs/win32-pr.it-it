...
title: Creazione di un OpenSearch file di descrizione Windows ricerca federata
ms.topic: article
ms.date: 05/31/2018
...
---
# <a name="creating-an-opensearch-description-file-in-windows-federated-search"></a>Creazione di un OpenSearch file di descrizione Windows ricerca federata
...
-   [Elementi standard nella Windows federata](#standard-elements-in-windows-federated-search)
    ...
    -   [Paging tramite l'indice degli elementi](#paging-using-the-item-index)
...
Windows il valore **dell'elemento ShortName** per assegnare un nome al file con estensione searchconnector-ms (connettore di ricerca) creato quando l'utente apre il file con estensione osdx. Windows che il nome file generato usi solo i caratteri consentiti nei nomi Windows file. Se non viene specificato alcun valore **ShortName,** il file con estensione searchconnector-ms tenta invece di usare il nome file del file con estensione osdx.
...
|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| {searchTerms}    | Sostituito con i termini di ricerca che l'utente digitare nella casella Windows di input di ricerca di Esplora risorse.<br/>                                         |
...
### <a name="paging-using-the-item-index"></a>Paging tramite l'indice degli elementi
...
Il [provider OpenSearch](https://github.com/dewitt/opensearch) quindi sostituisce il token nell'URL con un valore di indice iniziale. La prima richiesta inizia con il primo elemento, come illustrato nell'esempio seguente:
...
> [!Note]  
> Se una pagina di risultati restituita dal servizio Web contiene meno elementi rispetto alle dimensioni di pagina previste, il provider OpenSearch presuppone che abbia ricevuto l'ultima pagina di risultati e arresti l'esecuzione di richieste.

 

### <a name="paging-using-the-page-index"></a>Paging con l'indice di pagina

Un indice di pagina identifica la pagina di risultati specificata. Se si vuole che i client inviino richieste usando un numero di pagina, è possibile usare il token nell'attributo del modello dell'elemento di formato URL per indicare questo, come illustrato `{startPage}` nell'esempio  seguente: 
...
Il OpenSearch provider sostituisce quindi il token nell'URL con un parametro del numero di pagina. La prima richiesta inizia con la prima pagina, come illustrato nell'esempio seguente:
...
È possibile configurare il servizio Web per consentire a una richiesta di specificare le dimensioni delle pagine usando un parametro nell'URL. Una richiesta deve essere specificata nel file con estensione osdx usando il `{count}` token , come indicato di seguito:
...
Per impostazione predefinita, OpenSearch provider effettua richieste con dimensioni di pagina di 50. Se si vogliono dimensioni di pagina diverse, non fornire un token e inserire invece il numero desiderato `{count}` direttamente nell'elemento del **modello URL.**

Il OpenSearch determina le dimensioni della pagina in base al numero di risultati restituiti alla prima richiesta. Se la prima pagina di risultati ricevuti contiene meno elementi rispetto al conteggio richiesto, il provider reimposta le dimensioni della pagina per le richieste di pagina successive. Se le richieste di pagina successive restituiscono un numero di elementi inferiore a quello richiesto, il provider OpenSearch presuppone che abbia raggiunto la fine dei risultati.
...
Poiché questi elementi figlio estesi non sono supportati nella specifica [OpenSearch](https://github.com/dewitt/opensearch) v1.1, devono essere aggiunti usando lo spazio dei nomi seguente:
...
![Screenshot che mostra i mapping predefiniti delle proprietà rss-to-windows-shell](images/built-inrsstowindowsshellpropertymappings.png)

### <a name="default-mappings"></a>Mapping predefiniti

I mapping predefiniti degli elementi RSS XML Windows proprietà di sistema della shell sono elencati nella tabella seguente. I percorsi XML sono relativi all'elemento item. Il prefisso è definito dallo spazio dei nomi `"media:"` [Yahoo Search.](https://www.rssboard.org/media-rss)
...
Il file OSDX identifica una proprietà Windows Shell per ogni nome di elemento in uno spazio dei nomi. I mapping delle proprietà definiti nel file con estensione osdx eseguono l'override dei mapping predefiniti, se presenti, per le proprietà specificate.
...
Nel file OSDX di esempio seguente l'elemento di posta elettronica **XML** viene mappato alla proprietà Windows Shell [System.Contact.EmailAddress](../properties/props-system-contact-emailaddress.md).
...
Alcune proprietà non possono essere mappate perché i relativi valori vengono sostituiti in un secondo momento o non sono modificabili. Ad esempio, non è possibile eseguire il mapping di [System.ItemFolderPathDisplay](../properties/props-system-itempathdisplay.md) o [System.ItemPathDisplayNarrow](../properties/props-system-itempathdisplaynarrow.md) perché vengono calcolati in base al valore URL fornito negli elementi link o enclosure.

### <a name="thumbnails"></a>Anteprime

Gli URL dell'immagine di anteprima possono essere forniti per qualsiasi elemento usando **l'elemento media:thumbnail url="".** La risoluzione ideale è 150 x 150 pixel. Le anteprime più grandi supportate sono 256 x 256 pixel. Fornire immagini di dimensioni maggiori richiede più larghezza di banda per nessun vantaggio aggiuntivo per l'utente.

### <a name="open-file-location-context-menu"></a>Menu di scelta rapida Apri percorso file

Windows un menu di scelta rapida denominato **Apri percorso file per** gli elementi dei risultati. Se l'utente seleziona un elemento da tale menu, viene aperto l'URL "padre" per l'elemento selezionato. Se l'URL è un URL Web, ad esempio , il Web browser viene aperto `https://...` e si passa a tale URL. Il feed deve fornire un URL personalizzato per ogni elemento per assicurarsi che Windows un URL valido. Questa operazione può essere eseguita includendo l'URL all'interno di un elemento all'interno del codice XML dell'elemento, come illustrato nell'esempio seguente:
...
Se questa proprietà non è impostata in modo esplicito nel codice XML dell'elemento, il provider OpenSearch impostarla sulla cartella padre dell'URL dell'elemento. Nell'esempio precedente, il provider OpenSearch usa il valore del collegamento e imposta il valore della proprietà [System.ItemFolderPathDisplay](../properties/props-system-itemfolderpathdisplay.md) Windows Shell su `"https://example.com/"` .

### <a name="customize-windows-explorer-views-with-property-description-lists"></a>Personalizzare le Windows Explorer con gli elenchi di descrizione delle proprietà

Alcuni Windows layout di visualizzazione di Esplora risorse sono definiti da elenchi di descrizione delle proprietà o proplist. Un proplist è un elenco di proprietà delimitato da punti e virgola, ad esempio , usato per controllare la visualizzazione dei risultati in Windows `"prop:System.ItemName; System.Author"` Explorer.

Le aree dell'interfaccia Windows Explorer che possono essere personalizzate usando proplist sono illustrate nello screenshot seguente:
...
| Area dell'interfaccia utente da personalizzare               | Windows Proprietà shell che implementa la personalizzazione |
|------------------------------------|----------------------------------------------------------|
| Modalità di visualizzazione contenuto (durante la ricerca) | System.PropList.ContentViewModeForSearch                 |
| Modalità di visualizzazione contenuto (durante l'esplorazione)  | System.PropList.ContentViewModeForBrowse                 |
| Modalità di visualizzazione affiancata                     | System.PropList.TileInfo                                 |
| Riquadro dei dettagli                       | System.PropList.PreviewDetails                           |
| Suggerimento (descrizione comando al passaggio del mouse dell'elemento)     | System.PropList.Infotip                                  |
...
1.  Nell'output RSS aggiungere un elemento personalizzato che rappresenta l'elenco proplist da personalizzare. Ad esempio, l'esempio seguente imposta l'elenco per il riquadro dei dettagli:
    ```
...
2.  Per applicare una proprietà a ogni elemento nei risultati della ricerca senza modificare l'output RSS, specificare un proplist all'interno di un elemento **ms-ose:PropertyDefaultValues** nel file OSDX, come illustrato nell'esempio seguente:
...
### <a name="property-list-flags"></a>Flag dell'elenco di proprietà

Solo uno dei flag definiti nella documentazione proplists si applica alla visualizzazione degli elementi nei layout della modalità visualizzazione contenuto: ` "~"` . Negli esempi precedenti, la Windows explorer etichetta alcune delle proprietà, ad esempio `Tags: animals; zoo; lion` . Questo è il comportamento predefinito quando si specifica una proprietà nell'elenco. Ad esempio, l'elenco proplist `"System.Author"` è visualizzato come `"Authors: value"` . Quando si vuole nascondere l'etichetta della proprietà, posizionare un `"~"` davanti al nome della proprietà. Ad esempio, se l'elenco proplist ha , la proprietà viene visualizzata come solo `"~System.Size"` un valore, senza l'etichetta .
...
1.  Se la proprietà Windows Shell di **System.WebPreviewUrl** è impostata per l'elemento, usare tale URL.
    > [!Note]  
    > La proprietà deve essere fornita in RSS usando lo spazio dei nomi Windows Shell o mappata in modo esplicito nel file OSDX.
...
![Diagramma di flusso che mostra come Esplora risorse seleziona l'URL da usare per le anteprime](images/howwindowsexploreridentifieswhichurltouseforpreviews.png)

È possibile usare un URL diverso per l'anteprima rispetto all'elemento stesso. Ciò significa che se si specificano URL diversi per l'URL del collegamento e l'enclosure o , Windows Explorer usa l'URL di collegamento per le anteprime dell'elemento, ma usa l'altro URL per il rilevamento del tipo di file, l'apertura, il download e `media:content URL` così via.
...
2.  Se non si specifica un mapping, Windows Explorer identifica se gli URL del collegamento e dell'enclosure sono diversi. In tal caso, Windows Explorer usa l'URL del collegamento.
...
Quando si fa clic con il pulsante destro del mouse su un elemento, viene **visualizzato** il comando di menu Apri percorso file. Questo comando consente all'utente di accedere al contenitore per o alla posizione dell'elemento. Ad esempio, in una ricerca SharePoint, selezionando questa opzione per un file in una raccolta documenti si aprirà la radice della raccolta documenti nel Web browser.
...
![Diagramma di flusso che mostra come Esplora risorse identifica un contenitore padre](images/howwindowsexploreridentifiesaparentcontainer.png)
