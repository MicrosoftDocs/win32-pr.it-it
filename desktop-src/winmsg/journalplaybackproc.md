...
description: Riproduce una serie di messaggi del mouse e della tastiera registrati in precedenza.
old-location: ''
ms.assetid: na
ms.date: 04/05/2019
ms.keywords: ''
...
In genere, un'applicazione usa questa funzione per riprodurre una serie di messaggi di mouse e tastiera registrati in precedenza dalla routine hook **JournalRecordProc.**
Se è installata una procedura hook **JournalPlaybackProc,** l'input normale di mouse e tastiera è disabilitato.

Il **tipo HOOKPROC** definisce un puntatore a questa funzione di callback.
**JournalPlaybackProc** è un segnaposto per il nome di funzione definito dall'applicazione o definito dalla libreria.
...
| **HC_NOREMOVE** 2 | La routine hook deve prepararsi a copiare il messaggio successivo del mouse o della tastiera nella **struttura EVENTMSG** a cui punta *lParam*. Alla ricezione del **HC_GETNEXT** codice, la routine hook deve copiare il messaggio nella struttura . |
| **HC_SYSMODALOFF** 5 | Una finestra di dialogo modale di sistema è stata distrutta. La procedura hook deve riprendere la riproduzione dei messaggi. |
| **HC_SYSMODALON** 4 | Viene visualizzata una finestra di dialogo modale di sistema. Finché la finestra di dialogo non viene interrotta, la procedura hook deve interrompere la riproduzione dei messaggi. |
...
Questo parametro è valido solo quando il *parametro code* è **HC_GETNEXT**.
...
Questo valore può essere calcolato calcolando la differenza tra i membri time nei messaggi di input correnti e precedenti.
...
Il messaggio deve essere stato registrato in precedenza usando una routine hook **JournalRecordProc,** che non deve modificare il messaggio.

Per recuperare lo stesso messaggio più volte, la routine hook può essere chiamata più volte con  il parametro *code* impostato su **HC_GETNEXT** senza una chiamata intervenendo con il codice impostato su **HC_SKIP**.

Se *il* codice **HC_GETNEXT** e il valore restituito è maggiore di zero, il sistema viene in sospensione per il numero di millisecondi specificato dal valore restituito. Quando il sistema continua, chiama di nuovo la routine hook con *codice* impostato su **HC_GETNEXT** recuperare lo stesso messaggio.
...
Installare la routine hook **JournalPlaybackProc** specificando il tipo WH_JOURNALPLAYBACK e un puntatore alla routine hook in una chiamata alla **funzione SetWindowsHookEx.** [](about-hooks.md)

Se l'utente preme CTRL+ESC O CTRL+ALT+CANC durante la riproduzione del journal, il sistema arresta la riproduzione, annulla la riproduzione del journal e invia un messaggio [WM_CANCELJOURNAL](wm-canceljournal.md) all'applicazione di inserimento nel journal.

Se la routine hook restituisce un messaggio compreso nell'intervallo WM_KEYFIRST **a** **WM_KEYLAST**, si applicano le condizioni seguenti:
