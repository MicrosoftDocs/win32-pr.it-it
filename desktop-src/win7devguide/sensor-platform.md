...
Basato sulla piattaforma Sensor, le API Windows *Location* sono una nuova funzionalità di Windows 7 che consente agli sviluppatori di applicazioni di accedere alle informazioni sulla posizione fisica dell'utente. Le *API Windows Location* possono astrarre l'hardware, supportare simultaneamente più applicazioni e passare facilmente da una tecnologia all'altra, alleviando lo sviluppatore dell'applicazione dal carico di gestione di questi vincoli. Le API Location possono essere usate dai programmatori tramite il linguaggio di programmazione C++ (dai programmatori che hanno familiarità con Component Object Model (COM) o usando oggetti COM nei linguaggi di scripting, ad esempio Microsoft JScript. Il supporto per gli script consente di accedere facilmente ai dati di posizione per progetti come i dispositivi di protezione.
...
[L'estensione della classe Sensor](/windows-hardware/drivers/sensors/about-the-sensor-class-extension) è una parte fondamentale del modello di sviluppo del driver per la piattaforma Sensor. Fornisce i meccanismi seguenti, usati durante la scrittura di un driver del sensore [UMDF (User-Mode Driver Framework):](https://developer.microsoft.com/windows/hardware)
