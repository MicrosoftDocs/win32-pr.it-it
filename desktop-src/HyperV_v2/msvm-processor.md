...
---
# <a name="msvm_processor-class"></a>\_Classe processore MSVM

Rappresenta il processore virtuale in una macchina virtuale.
...
La classe del **\_ processore MSVM** dispone di questi tipi di membri:
...
La classe del **\_ processore MSVM** dispone di questi metodi.
...
| [**RequestStateChange**](msvm-processor-requeststatechange.md) | Richiede una modifica dello stato.<br/>      |
| [**Reset**](msvm-processor-reset.md)                           | Reimposta il dispositivo virtuale.<br/>    |
...
La classe del **\_ processore MSVM** dispone di queste proprietà.
...
Larghezza dell'indirizzo del processore, in bit. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e il valore è 32 o 64, a seconda delle caratteristiche della macchina virtuale.
...
Stato corrente del processore. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Velocità massima del processore fisico, prendendo in considerazione la capacità del processore virtuale. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Larghezza dei dati del processore, in bit. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e il valore è 32 o 64, a seconda delle caratteristiche della macchina virtuale.
...
Un indirizzo o altre informazioni di identificazione per assegnare un nome univoco al dispositivo logico. Questa proprietà viene ereditata da [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)ed è sempre impostata su "Microsoft:*GUID* \\ *Device Specific Data*".
...
Velocità di clock del bus esterno del processore fisico sottostante. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
La famiglia di processori del processore fisico sottostante. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Matrice di stringhe in formato libero che fornisce spiegazioni e dettagli dietro le voci nella matrice [**OtherIdentifyingInfo**](msvm-keyboard.md) . Questa proprietà viene ereditata da [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)ed è sempre impostata su **null**.
...
Percentuale corrente della potenza di elaborazione totale utilizzata dalla macchina virtuale. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Cronologia registrata della percentuale della potenza di elaborazione totale utilizzata dalla macchina virtuale. Si tratta di una matrice contenente esempi.
...
Velocità di clock massima, in megahertz, del processore virtuale. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Descrizione del tipo di famiglia del processore. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor)ed è sempre impostata su **null**.
...
Indica se il risparmio energia è supportato nel dispositivo. Questa proprietà viene ereditata da [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice), ma non viene utilizzata.
...
Stringa che descrive il ruolo del processore. Ad esempio, "Central Processor" o "Math Processor". Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Livello di revisione del processore all'interno della famiglia di processori del processore fisico sottostante. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Identificatore univoco globale per il processore. Questo identificatore può essere univoco solo all'interno di una famiglia di processori. Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor)ed è sempre impostata su **null**.
...
Informazioni sul socket della CPU, che includono i dati su come aggiornare il processore (se sono supportati gli aggiornamenti). Questa proprietà viene ereditata [**dal \_ processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor).

</dd> </dl>

## <a name="remarks"></a>Commenti

L'accesso alla classe del **\_ processore MSVM** potrebbe essere limitato dal filtraggio del controllo dell'account utente. Per ulteriori informazioni, vedere [controllo dell'account utente e WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
...
[**\_Processore CIM**](/windows/desktop/CIMWin32Prov/cim-processor)
</dt> <dt>
