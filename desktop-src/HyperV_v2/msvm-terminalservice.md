---
description: Gestisce tutte le connessioni terminali remote a un host specifico.
ms.assetid: 7eacb8a6-cb8d-4a2a-951e-f5286cf484e7
title: Msvm_TerminalService classe
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "118146710"
---
# <a name="msvm_terminalservice-class"></a>Classe \_ TerminalService msvm

Gestisce tutte le connessioni terminali remote a un host specifico. Il servizio usa una porta configurabile per avviare tutte le connessioni del terminale.
...
La **classe Msvm \_ TerminalService** ha questi tipi di membri:
...
La **classe Msvm \_ TerminalService** dispone di questi metodi.



| Metodo                                                                                        | Descrizione                                                                                                      |
|:----------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------|
| [**GetInteractiveSessionACL**](getinteractivesessionacl-msvm-terminalservice.md)             | Recupera l'elenco DACL corrente che controlla l'accesso alla sessione interattiva di una macchina virtuale.<br/>      |
| [**GrantInteractiveSessionAccess**](grantinteractivesessionaccess-msvm-terminalservice.md)   | Concede l'accesso alla sessione interattiva della macchina virtuale all'elenco specificato di fiduciari.<br/>    |
| [**ModifyServiceSettings**](modifyservicesettings-msvm-terminalservice.md)                   | Modifica i dati di impostazione per il servizio.<br/>                                                            |
| [**RequestStateChange**](msvm-terminalservice-requeststatechange.md)                         | Richiede una modifica dello stato.<br/>                                                                              |
| [**RevokeInteractiveSessionAccess**](revokeinteractivesessionaccess-msvm-terminalservice.md) | Revoca l'accesso alla sessione interattiva della macchina virtuale dall'elenco specificato di fiduciari.<br/> |
| [**Startservice**](msvm-terminalservice-startservice.md)                                     | avvia il servizio.<br/>                                                                                   |
| [**StopService**](msvm-terminalservice-stopservice.md)                                       | arresta il servizio.<br/>                                                                                    |
...
La **classe Msvm \_ TerminalService** ha queste proprietà.
...
Etichetta con cui l'oggetto è noto. Questa proprietà viene ereditata dal [**servizio CIM. \_**](/windows/desktop/CIMWin32Prov/cim-service)
...
Valore che fornisce informazioni su come è possibile raggiungere il proprietario primario del servizio, ad esempio il numero di telefono, l'indirizzo di posta elettronica e così via. Questa proprietà viene ereditata dal [**servizio CIM. \_**](/windows/desktop/CIMWin32Prov/cim-service)
...
Nome del proprietario primario per il servizio, se definito. Il proprietario primario è il contatto di supporto iniziale per il servizio. Questa proprietà viene ereditata dal [**servizio CIM. \_**](/windows/desktop/CIMWin32Prov/cim-service)
...
Ultimo stato richiesto o desiderato per l'elemento. Lo stato effettivo dell'elemento è rappresentato da **EnabledState.** Questa proprietà viene fornita per confrontare gli ultimi stati richiesti e correnti abilitati o disabilitati. Una particolare istanza di [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) potrebbe non supportare il **metodo RequestStateChange.** In questo caso, viene usato il valore 12 (Non applicabile). Questa proprietà viene ereditata da **CIM \_ EnabledLogicalElement.**
...
Valore stringa che indica se il servizio viene avviato automaticamente da un sistema, da un sistema operativo o solo su richiesta. Questa proprietà viene ereditata dal [**servizio CIM. \_**](/windows/desktop/CIMWin32Prov/cim-service)
...
Stato dell'elemento. Questa proprietà viene ereditata da [**CIM \_ ManagedSystemElement.**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)
...
Indica lo stato di destinazione a cui è in fase di transizione l'istanza. Questa proprietà viene ereditata da [**CIM \_ EnabledLogicalElement.**](/previous-versions//cc136818(v=vs.85))

</dd> </dl>

## <a name="remarks"></a>Commenti

L'accesso alla **classe Msvm \_ TerminalService** potrebbe essere limitato dal filtro di Controllo dell'account utente. Per altre informazioni, vedere [Controllo dell'account utente e WMI.](/windows/desktop/WmiSdk/user-account-control-and-wmi)
