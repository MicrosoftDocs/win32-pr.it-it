---
description: Rappresenta una porta Fibre Channel esterna.
...
ms.locfileid: "119789941"
---
# <a name="msvm_externalfcport-class"></a>Classe Msvm \_ ExternalFcPort
...
La **classe Msvm \_ ExternalFcPort** ha questi tipi di membri:
...
La **classe Msvm \_ ExternalFcPort** include questi metodi.
...
| [**RequestStateChange**](msvm-externalfcport-requeststatechange.md) | Richiede una modifica dello stato<br/>       |
| [**Reset**](msvm-externalfcport-reset.md)                           | Reimposta il dispositivo virtuale.<br/>    |
...
La **classe Msvm \_ ExternalFcPort** ha queste proprietà.
...
Matrice di interi che indica le classi di servizio attive. L'elemento COS supportato viene specificato dalla **proprietà SupportedCOS.** Questa proprietà viene ereditata da **CIM \_ FCPort.**
...
Matrice di interi che indica l'Fibre Channel i protocolli FC-4 attualmente in esecuzione. Un elenco di tutti i protocolli supportati viene specificato dalla **proprietà SupportedFC4Types.** Questa proprietà viene ereditata da **CIM \_ FCPort.**
...
<span id="IP_over_FC"></span><span id="ip_over_fc"></span><span id="IP_OVER_FC"></span>**IP su FC** (5)
</dt> <dt>
...
<span id="CP_IPI_-_3_Peer"></span><span id="cp_ipi_-_3_peer"></span><span id="CP_IPI_-_3_PEER"></span>**IPI CP - 3 peer** (23)
</dt> <dt>

<span id="SBCCS_Channel"></span><span id="sbccs_channel"></span><span id="SBCCS_CHANNEL"></span>**Canale SBCCS** (25)
</dt> <dt>

<span id="SBCCS_Control_Unit"></span><span id="sbccs_control_unit"></span><span id="SBCCS_CONTROL_UNIT"></span>**Unità di controllo SBCCS** (26)
</dt> <dt>

<span id="FC-SB-2_Channel"></span><span id="fc-sb-2_channel"></span><span id="FC-SB-2_CHANNEL"></span>**Canale FC-SB-2** (27)
</dt> <dt>

<span id="FC-SB-2_Control_Unit"></span><span id="fc-sb-2_control_unit"></span><span id="FC-SB-2_CONTROL_UNIT"></span>**Unità di controllo FC-SB-2** (28)
</dt> <dt>
...
<span id="BBL_Control"></span><span id="bbl_control"></span><span id="BBL_CONTROL"></span>**Controllo BBL** (80)
</dt> <dt>
...
<span id="BBL_802.3_Encapsulated_LAN_PDU"></span><span id="bbl_802.3_encapsulated_lan_pdu"></span><span id="BBL_802.3_ENCAPSULATED_LAN_PDU"></span>**PDU LAN incapsulata BBL 802.3** (82)
</dt> <dt>
...
<span id="Vendor_unique"></span><span id="vendor_unique"></span><span id="VENDOR_UNIQUE"></span>**Univoco del** fornitore (255)
</dt> </dl>
...
L'unità di trasmissione massima attiva o negoziata (MTU) che può essere supportata, in byte. Questa proprietà viene ereditata da [**CIM \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport).
...
Qualsiasi disponibilità e stato aggiuntivi del dispositivo. Questa proprietà viene ereditata da [**CIM \_ LogicalDevice,**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)ma non viene usata.
...
Se questa proprietà è **True,** questa Fibre Channel porta può essere connessa ai commutatori e quindi può fornire la connettività a una macchina virtuale. Se questa proprietà è **False,** questa porta non può essere usata dalla macchina virtuale Fibre Channel architettura.
...
Specifica il tipo di tecnologia di collegamento per la porta. Se impostata su 1 (Altro), la **proprietà OtherLinkTechnology** contiene una descrizione stringa del tipo di collegamento. Questa proprietà viene ereditata da [**CIM \_ NetworkPort.**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)
...
<span id="ATM"></span><span id="atm"></span>**Bancomat** (6)
</dt> <dt>
...
<span id="Infrared"></span><span id="infrared"></span><span id="INFRARED"></span>**A lor** (9)
</dt> <dt>
...
L'uso di questa proprietà è deprecato al posto della **proprietà PortType.** Questa proprietà viene ereditata da [**CIM \_ NetworkPort.**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)
...
Descrive il tipo di modulo, quando **PortType** è impostato su 1 (Altro). Questa proprietà viene ereditata da [**CIM \_ LogicalPort.**](/previous-versions//cc136869(v=vs.85))
...
Indirizzo di rete hardcoded in una porta. Questo indirizzo hardcoded può essere modificato usando un aggiornamento del firmware o una configurazione software. Quando viene apportata questa modifica, il campo deve essere aggiornato contemporaneamente. Questa proprietà deve essere **Null se** non esiste alcun indirizzo hardcoded per la scheda di rete. Questa proprietà viene ereditata da [**CIM \_ NetworkPort.**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)
...
Modalità specifica attualmente abilitata per la porta. Se impostata su 1 (Altro), la proprietà **OtherPortType correlata** contiene una descrizione stringa del tipo di porta. Questa proprietà viene ereditata da [**CIM \_ LogicalPort.**](/previous-versions//cc136869(v=vs.85))
...
Ultimo stato richiesto o desiderato per l'elemento. Questa proprietà viene ereditata da [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))e viene sempre impostata su 12 (non applicabile).
...
Matrice di numeri interi che indica Fibre Channel classi di servizio (COS) supportate. L'elemento COS attivo viene specificato dalla **proprietà ActiveCOS.** Questa proprietà viene ereditata da **CIM \_ FCPort.**
...
Matrice di interi che indica l'Fibre Channel supportati i protocolli FC-4. I protocolli attivi ed in esecuzione vengono specificati dalla **proprietà ActiveFC4Types.** Questa proprietà viene ereditata da **CIM \_ FCPort.**
...
Nome del nodo a livello di mondo per questa Fibre Channel porta.
...
Nome della porta a livello mondiale per questa Fibre Channel porta.
