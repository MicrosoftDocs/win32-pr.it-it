---
title: endif - ps
description: Contrassegna la fine di un oggetto if... Blocco else.
ms.assetid: 3e20e86b-d050-45f5-8180-8c2ea81c60ba
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- apiref
api_name: ''
api_type: ''
api_location: ''
ms.openlocfilehash: 86612cf089dcaeaa26b136a3dd8c52d1e20b12f7f3850fc3a515838bf834e54c
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 08/11/2021
ms.locfileid: "118512151"
---
# <a name="endif---ps"></a>endif - ps

Contrassegna la fine di un [oggetto if](if-bool---ps.md)... [Blocco else.](else---ps.md)

## <a name="syntax"></a>Sintassi



| endif |
|-------|



 

## <a name="remarks"></a>Commenti



| Versioni dei pixel shader | 1\_1 | 1\_2 | 1 \_ 3 | 1\_4 | 2 \_ 0 | 2 \_ x | 2 \_ sw | 3 \_ 0 | 3 \_ sw |
|-----------------------|------|------|------|------|------|------|-------|------|-------|
| endif                 |      |      |      |      |      | x    | x     | x    | x     |



 

## <a name="related-topics"></a>Argomenti correlati

<dl> <dt>

[Istruzioni per pixel shader](dx9-graphics-reference-asm-ps-instructions.md)
</dt> <dt>

[if bool - ps](if-bool---ps.md)
</dt> <dt>

[else - ps](else---ps.md)
</dt> </dl>

 

 




