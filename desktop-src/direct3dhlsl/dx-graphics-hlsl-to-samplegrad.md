---
title: SampleGrad (oggetto trama DirectX HLSL)
description: Campionare una trama usando una sfumatura per influenzare il modo in cui viene calcolata la posizione del campione. | SampleGrad (oggetto trama DirectX HLSL)
...
ms.locfileid: "119562871"
---
# <a name="samplegrad-directx-hlsl-texture-object"></a>SampleGrad (oggetto trama DirectX HLSL)

Campionare una trama usando una sfumatura per influenzare il modo in cui viene calcolata la posizione del campione.

&lt;Tipo di &gt; modello Object.SampleGrad( sampler \_ state S, float Location, float DDX, float DDY, \[ int Offset \] );
...
<td><p>[in] Frequenza di modifica della geometria della superficie nella direzione x. Il tipo di argomento dipende dal tipo texture-object.</p>
...
<td><p>[in] Velocità di modifica della geometria della superficie nella direzione y. Il tipo di argomento dipende dal tipo texture-object.</p>
...
<td><p>[in] Offset facoltativo delle coordinate della trama, che può essere usato per qualsiasi tipo di oggetto trama. L'offset viene applicato alla posizione prima del campionamento. Usare un offset solo in corrispondenza di un valore integer miplevel. In caso contrario, è possibile ottenere risultati che non si traducono bene in hardware. Il tipo di argomento dipende dal tipo texture-object. Per altre informazioni, vedere<a href="dx-graphics-hlsl-to-sample.md">Applicazione di offset di interi.</a></p>
...
Questo esempio di codice parziale deriva dal file MotionBlur.fx [nell'esempio MotionBlur10](https://msdn.microsoft.com/library/Ee416417(v=VS.85).aspx).
