...
---
# <a name="samplegradsamplegradsfloatfloatfloatintfloatuint-function-for-texture2d"></a>Funzione SampleGrad:: SampleGrad (S, float, float, float, int, float, uint) per Texture2D

Esegue il campionamento di un [**Texture2D**](sm5-object-texture2d.md), utilizzando una sfumatura per influenzare il modo in cui viene calcolata la posizione di esempio, con un valore facoltativo per bloccare i valori del livello di dettaglio (LOD) di esempio in. Restituisce lo stato dell'operazione.
