...
description: Min senza segno atomico immediato in memoria. Restituisce il valore in memoria prima dell'operazione max.
...
ms.locfileid: "119119275"
---
# <a name="imm_atomic_umin-sm5---asm"></a>imm \_ atomic \_ umin (sm5 - asm)

Min senza segno atomico immediato in memoria. Restituisce il valore in memoria prima dell'operazione max.



| imm \_ atomic \_ umin dst0 \[ .single component mask , \_ \_ \] dst1, dstAddress \[ .swizzle \] , src0 \[ .select \_ component\] |
|--------------------------------------------------------------------------------------------------------------|
...
Questa istruzione esegue un singolo componente senza segno minimo a 32 bit dell'operando *src0* con *dst1* a 32 bit per indirizzo componente *dstAddress*.
...
Il valore nella *memoria dst1* prima di min viene restituito *a dst0*.
