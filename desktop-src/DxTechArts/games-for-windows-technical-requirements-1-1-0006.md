...
ms.locfileid: "122482117"
---
# <a name="games-for-windows-technical-requirements-best-practices-for-games-on-windows-xp-windows-vista-windows-7-and-windows-8"></a>Giochi per Windows requisiti tecnici: procedure consigliate per i giochi in Windows XP, Windows Vista, Windows 7 e Windows 8
...
XINPUT 1.4 è integrato in Windows 8. Sia Windows app di Store che app Win32 desktop possono usare XINPUT 1.4. Tutte le versioni di Windows possono usare XINPUT 9.1.0 per i controller comuni semplificati, ma non esiste alcun pacchetto di ridistribuzione con XINPUT 9.1.0. Tutte le versioni di Windows possono anche usare la versione XINPUT 1.3 di DirectX SDK esistente, che richiede la distribuzione di DirectSetup.

Vedere [1.4 Support the Xbox 360 Common Controller for Windows](#14-support-the-xbox-360-common-controller-for-windows-conditional-requirement).
...
Questi giochi attivano un comportamento di compatibilità dell'Windows 8 per abilitare automaticamente il runtime .NET 3.5 (che richiede l'accesso alla rete). Tuttavia, è consigliabile che gli sviluppatori .NET si spostino nel runtime .NET 4.0.
...
Il gioco deve essere visibile all'interno Games Explorer (cartella **Games)** in Windows Vista e Windows 7. Quando questa opzione è selezionata, il gioco deve anche visualizzare metadati corretti, inclusi editore, sviluppatore, data di rilascio, versione, punteggi dell'indice dell'esperienza Windows, classificazione (se applicabile) e collegamenti ipertestuali associati.

Se il gioco è distribuito digitalmente tramite un servizio di gioco online, anche il provider di servizi dovrebbe essere visualizzato in Games Explorer. Per garantire la corretta gestione del provider e abilitare l'uso di feed RSS, è necessario usare la versione 2 dello schema per i file GDF (Game Definition File). Per altre informazioni sui file GDF, vedere Informazioni aggiuntive.
...
-   Gli utenti devono essere in grado di rimuovere il gioco usando Programmi e funzionalità in Pannello di controllo in Windows Vista e Windows 7 oppure Installazione applicazioni in Pannello di controllo in Windows XP.
...
Per le applicazioni associate installate con il gioco, ma non con i giochi, è possibile creare gruppi di programmi, collegamenti e icone del desktop menu Start in tutte le versioni di Windows, tra cui Windows Vista e Windows 7. Tali applicazioni associate devono superare i giochi applicabili per Windows requisiti; Per informazioni dettagliate, vedere [Linee guida per i prodotti middleware dei giochi.](#guidelines-for-game-middleware-products) I servizi di gioco sono invitati a registrarsi Games Explorer come provider di giochi per Windows 7. 1
...
I giochi devono supportare Windows Family Safety regole rispettando le regole seguenti:
...
-   I giochi classificati Windows classificazioni supportate da più schede, ad esempio ESRB e PEGI, devono includere le informazioni sulle classificazioni assegnate nel file di definizione del gioco (GDF). Tutti i dati di classificazione disponibili devono essere inclusi in ogni versione localizzata della funzione GDF, nonché nella versione indipendente dalla lingua.
-   I giochi devono elencare i propri file eseguibili nella funzione GDF per offrire un'esperienza utente ottimale per le restrizioni generali delle applicazioni, a meno che il gioco non usi una tecnologia anti-piracy che crea file eseguibili con nome casuale in fase di esecuzione.
...
Tutti i giochi devono essere eseguiti nel contesto di un account utente standard per consentire agli account controllati da Windows controllo genitori di eseguire il gioco. I genitori vogliono avere la possibilità di monitorare e controllare l'accesso dei propri figli ai giochi. Inoltre, numerosi gruppi di settore, enti pubblici e di consulenza desiderano modi migliori per consentire ai genitori di monitorare e controllare i giochi a cui sono esposti i propri figli. In combinazione con l'architettura offerta da Games Explorer, Microsoft offre ai genitori questa possibilità Windows controllo genitori.

Anche per i giochi che non partecipano a un programma ratings board, la richiesta di privilegi elevati crea un'esperienza di gioco scadente per la maggior parte degli account utente. Questo è in particolare il caso in cui i controlli genitori sono abilitati, il che richiederebbe all'elemento padre di immettere la password dell'amministratore ogni volta che viene avviato il gioco.

Il Windows controllo genitori consente ai genitori di selezionare le classificazioni che ritiene appropriate per i figli. Il controllo genitori supporta molti dei sistemi di classificazione a livello mondiale. Il controllo genitori consente inoltre ai genitori di limitare l'accesso ai giochi in base ai descrittori di contenuto (se il sistema di classificazione applicabile li supporta) e di consentire o impedire l'accesso ai singoli giochi.

La scelta predefinita del sistema di classificazione per Windows Controllo genitori è basata sulle impostazioni locali del sistema, ma può essere modificata dall'utente **in** Opzioni internazionali e della lingua in **Pannello di controllo**. Pertanto, ogni lingua supportata deve fornire tutti i dati delle classificazioni disponibili in modo che l'utente abbia la libertà di selezionare qualsiasi scheda di classificazione desiderata.
...
|------------------|--------------------------|
| Windows Vista | <ul><li>CERO (Giappone)</li><li>ESRB (USA)</li><li>OFLC (Australia)</li><li>PEGI (Europa)</li><li>PEGI Obsolete (deprecato)</li><li>PEGI Portogallo</li><li>PEGI/BBFC (Regno Unito)</li><li>USK (Germania)</li></ul> | 
| Windows Vista con un Service Pack | I Service Pack per Windows Vista aggiungono il supporto per gli elementi seguenti:<br /><ul><li>GRB (Corea del Sud)</li><li>Descrittori di contenuto varianti ESRB "ESRB"</li></ul> | 
...
> [!Note]  
> Tutti i titoli che includono i nuovi descrittori di contenuto di ESRB Windows Vista Service Pack 1 (SP1) verranno visualizzati come Non Windows Vista senza service pack.
...
Per altre informazioni sulla compatibilità di un gioco con privilegi utente standard, vedere l'articolo di DirectX [Controllo dell'account utente per sviluppatori di giochi](/windows/desktop/DxTechArts/user-account-control-for-game-developers).
...
### <a name="14-support-the-xbox-360-common-controller-for-windows-conditional-requirement"></a>1.4 Supportare il Xbox 360 Common Controller per Windows \[ requisito condizionale\]
...
I giochi che supportano i controller gamepad devono supportare il controller Xbox 360 per Windows tramite l'API [XInput.](/windows/desktop/xinput/xinput-game-controller-apis-portal) Se sono supportate anche le periferiche DirectInput, è possibile usare anche DirectInput. Tuttavia, XInput deve essere l'API predefinita se viene Xbox 360 un dispositivo compatibile.

Tutti i riferimenti ai trigger e ai pulsanti comuni del controller devono usare Xbox 360 nomi. Per informazioni [dettagliate, vedere Xbox 360 Common Controller per Windows terminologia.](#xbox-360-common-controller-for-windows-terminology)
...
È consigliabile non usare DirectInput per implementare i controlli della tastiera o del mouse. I controlli di tastiera e mouse devono essere implementati solo Windows messaggi e API Win32. Per informazioni dettagliate su come ottenere informazioni sullo spostamento del mouse ad alta risoluzione senza usare DirectInput, vedere Sfruttare High-Definition [spostamento del mouse.](/windows/desktop/DxTechArts/taking-advantage-of-high-dpi-mouse-movement)
...
Con il Windows desktop 3D, non è possibile presumere una particolare proporzione o risoluzione, a causa dei fattori seguenti:
...
Per altre informazioni, vedere le sezioni Proporzioni e Widescreen dell'articolo di DirectX Introduzione all'esperienza [da 10](/windows/desktop/DxTechArts/introduction-to-the-10-foot-experience-for-windows-game-developers)piedi per sviluppatori di Windows game.
...
La Windows vista e Windows 7 core è progettata in base a Direct3D. Direct3D 8 e versioni precedenti sono supportate dalla modifica del mapping delle interfacce legacy.

È fortemente consigliato l'uso di versioni di Direct3D più recenti rispetto a Direct3D 9. Vedere i giochi per Windows Showcase S.1. La richiesta di Direct3D 10 o Direct3D 11 è completamente conforme al requisito 1.7.
...
I monitor LCD di alta qualità sono comuni come gli schermi dei computer e hanno un aspetto ottimale quando sono guidati dalle risoluzioni native (in genere 1280 1024, 1600 1200 e così via). I clienti che hanno difficoltà a leggere il testo e a visualizzare le immagini a questa risoluzione spesso impostano i desktop del computer su una risoluzione inferiore e subiscono artefatti visivi dal ridimensionamento LCD. Al contrario, i clienti possono lasciare la risoluzione alle dimensioni native e modificare il valore DPI della visualizzazione Windows, in modo da rendere più grande l'aspetto di elementi e testo senza compromettere la qualità dell'immagine.

Anche se questa funzionalità è disponibile in qualche modo Windows XP, raramente è abilitata dai clienti o dagli OEM. Più della metà di tutti gli schermi del computer attualmente è impostata su una risoluzione inferiore rispetto alla risoluzione nativa del monitor, in base ai commenti e suggerimenti dei clienti. Windows 7 rende questa funzionalità molto più visibile ai clienti durante la configurazione iniziale e quando si modificano le impostazioni dello schermo, incoraggiandoli a usare il ridimensionamento DPI anziché modificare la risoluzione del desktop.
...
È invece possibile usare la funzione [**SetProcessDPIAware,**](/windows/desktop/api/winuser/nf-winuser-setprocessdpiaware) se chiamata nelle prime fasi del codice di avvio del processo. È preferibile aggiungere al manifesto per assicurarsi che non siano presenti race conditions con elementi software (ad esempio DLL) che potrebbero essere inizializzati prima della chiamata al punto di ingresso principale. Si noti **che SetProcessDPIAware** è presente solo in Windows Vista e Windows 7.
...
I requisiti seguenti migliorano la sicurezza complessiva dei giochi e consentono di garantire che funzionino con Windows su architetture diverse, in configurazioni diverse e in modalità diverse.

### <a name="21-follow-user-account-control-guidelines"></a>2.1 Seguire le linee guida per il controllo degli account utente
...
In base al requisito 1.2, il gioco principale e l'eseguibile di esecuzione automatica devono avere il livello di esecuzione asInvoker per supportare i contesti utente standard.

I file di dati utente **con** associazioni di file registrate con Esplora file devono essere inseriti in una sottocartella della cartella specificata da CSIDL PERSONAL (denominata anche \_ **Documenti** **o Documenti**). Tutti gli altri file di dati utente devono essere archiviati in una sottocartella delle cartelle specificate da CSIDL \_ LOCAL \_ APPDATA o CSIDL \_ COMMON \_ APPDATA. Queste directory sono nascoste per impostazione predefinita per i singoli utenti e per tutti gli utenti.
...
L'esperienza utente Windows è più sicura se le applicazioni vengono eseguite solo con le autorizzazioni necessarie.
...
Le librerie di collegamento dinamico (DLL) non richiedono tali manifesti.
...
### <a name="22-support-windows-x64-versions"></a>2.2 Supporto per Windows x64
...
-   I programmi di installazione dei titoli e dei titoli non devono contenere codice a 16 bit o basarsi su componenti a 16 bit.
-   Se il gioco dipende dai driver in modalità kernel per il funzionamento, è necessario che siano disponibili versioni x64 di questi driver. La configurazione del gioco deve rilevare e installare i driver e i componenti appropriati per le edizioni a 64 bit di Windows.
...
Molti utenti di Windows Vista e Windows 7 eseguiranno edizioni a 64 bit del sistema operativo per tutta la durata del prodotto, quindi è fondamentale che le applicazioni siano compatibili con questo sistema operativo.
...
La compatibilità con Windows XP Professional x64 Edition non è necessaria, ma è vivamente consigliabile.
...
Se il gioco usa il Windows, i file del pacchetto del programma di installazione (.msi file) devono essere firmati.
...
La firma di un file consente agli utenti di decidere se considerare attendibile un'applicazione e garantisce agli utenti che i file non sono stati manomissionati. Consente inoltre di eseguire correttamente le applicazioni in ambienti bloccati.
...
Se il gioco usa Windows programma di installazione, è consigliabile abilitare l'applicazione di patch UAC/LUA, includendo una tabella MsiPatchCertificate. Per altre informazioni, vedere Applicazione di patch a Controllo [dell'account utente.](/windows/desktop/Msi/user-account-control--uac--patching)
...
I driver di malware o scritti in modo non corretti possono influire in modo grave sulla stabilità e sulla sicurezza di un sistema. Nelle edizioni a 64 bit di Windows Vista e Windows 7, i driver non firmati non vengono caricati. Questo criterio può essere abilitato anche per le edizioni a 32 bit di Windows Vista e Windows 7.
...
Entrambe le versioni native a 32 bit e a 64 bit di tutti i driver in modalità kernel sono necessarie in base al requisito 2.2.
...
### <a name="25-perform-proper-version-checking"></a>2.5 Eseguire un controllo corretto della versione
...
Quando Windows gli utenti aggiornano i sistemi operativi, non devono essere bloccati dalla riproduzione di giochi correnti semplicemente perché il numero Windows versione è aumentato. I controlli di versione scritti in modo non sufficiente continuano a creare problemi per il software che, in caso contrario, funziona correttamente nelle versioni più recenti di Windows o semplicemente con l'aggiunta di un Service Pack Windows.

La logica di confronto del controllo delle versioni non riuscito per il runtime DirectX ha creato numerose installazioni non riuscite quando viene eseguito in versioni diverse di Windows. Il numero di versione di DirectX si applica solo ai componenti principali del sistema operativo. Non si applica ai componenti di DirectX SDK side-by-side usati da molti giochi.
...
È piuttosto comune vedere i programmi di installazione verificare la presenza di una versione minima del sistema operativo. Questo controllo, tuttavia, deve essere eseguito con cautela per assicurarsi che il test sia maggiore o uguale a, anziché alla semplice uguaglianza, bloccando in tal modo una versione specifica del sistema operativo. L Application Verifier test **HighVersionLie** di Application Verifier è un modo semplice e rapido per determinare in che modo il programma di installazione reagisce a una modifica significativa del numero di versione del sistema operativo.

L'uso corretto del pacchetto di ridistribuzione del runtime DirectX (programma di installazione di DirectX) comporta sempre l'avvio durante l'installazione, preferibilmente in modalità invisibile all'utente. Ciò consente al codice fornito da Microsoft di eseguire gli aggiornamenti di versione necessari. Consente inoltre l'installazione di tutti i componenti facoltativi di DirectX SDK side-by-side, ad esempio D3DX, XACT, MDX o XInput.
...
È consigliabile che i giochi che supportano Windows XP controllino la presenza di un Service Pack di livello 2 o superiore, perché Service Pack 2 (SP2) e Service Pack 3 (SP3) offrono miglioramenti significativi per la sicurezza, un requisito di ridistribuzione del runtime DirectX semplificato e una distribuzione estremamente ampia. La maggior parte delle tecnologie Microsoft moderne che supportano Windows XP richiede SP2 o SP3 (XAudio2, Games for Windows - LIVE e così via).
...
I giochi che si basano sulla grafica 3D non devono funzionare su una connessione desktop remoto, ma l'utente deve essere avvisato quando il gioco non riesce.
...
-   L'audio del gioco in una sessione utente non deve essere udito quando un altro utente è attivo in una sessione diversa.
-   I giochi devono supportare il cambio utente rapido.
-   I giochi non devono tentare di disabilitare il cambio di attività standard. I giochi non devono disabilitare i tasti di scelta rapida ALT+TAB. I giochi possono disabilitare i tasti di scelta rapida per l'accessibilità, come descritto in [Disabilitazione dei tasti di scelta rapida in Games.](/windows/desktop/DxTechArts/disabling-shortcut-keys-in-games)
...
Windows gli utenti devono essere in grado di eseguire sessioni simultanee senza conflitti o interruzioni. Si tratta di uno scenario comune per Windows computer condiviso da una famiglia, da colleghi di stanza o da altri utenti.
...
Per altre informazioni, vedere [Cambio rapido utente.](/windows-hardware/drivers/display/fast-user-switching) Si noti che cambio utente rapido si verifica se sono abilitate le restrizioni di tempo di Controllo genitori quando l'ora dell'utente è attiva. Per altri dettagli, vedere il requisito 1.2.
...
Se un gioco supporta il salvataggio di file, deve essere in grado di salvare i file con nomi e percorsi lunghi. Il gioco deve gestire correttamente i caratteri file system speciali, ad esempio \\ / : \* ? " < >, in tutti i campi di input utente usati per creare nomi di file o percorsi.
...
### <a name="31-support-easy-installation"></a>3.1 Supporto semplice dell'installazione
...
I giochi devono fornire un percorso semplificato nell'interfaccia utente di installazione implementando quanto segue:

-   Visualizzare un massimo di una richiesta di licenza con l'utente finale.
...
-   Installare tutti i componenti del sistema operativo necessari, ad esempio i runtime DirectX e Visual C, usando i pacchetti di ridistribuzione Microsoft corretti. L'installazione deve essere eseguita in modo invisibile all'utente, senza chiedere conferma e senza essere sorvegliata dai controlli della versione dei componenti.
-   Fornire la rimozione **tramite Programmi e funzionalità** in **Pannello di controllo** sia per l'applicazione di gioco che per i file di lavoro generati. È consigliabile scegliere un'opzione per eliminare tutti i file di dati creati dall'utente. Il processo di rimozione deve garantire che tutti i file installati siano rimossi e che tutte le impostazioni (ad esempio, le voci dell'elenco di eccezioni del firewall e le chiavi del Registro di sistema) siano cancellate. I componenti del sistema operativo ridistribuiti non devono essere rimossi.
...
L'installazione semplice è una filosofia di sviluppo di giochi incentrata su Windows progettata per semplificare e semplificare il processo talvolta noioso e confuso di installazione di giochi nei computer che eseguono sistemi operativi Windows. L'installazione semplice viene abilitata usando un set di tecnologie e procedure consigliate che riducono la complessità non necessaria e il rischio percepito di installare giochi in Windows computer.

Gli obiettivi principali sono:

-   Ridurre la quantità di tempo per entrare nel gioco e iniziare il gioco.
-   Ridurre il numero di finestre di dialogo e scelte a un numero molto limitato o nessuno per semplificare l'esperienza di installazione del gioco.

Alcune installazioni tradizionali hanno richieste che consentono installazioni non funzionali, anche se l'applicazione sembra essere stata installata correttamente. Ad esempio, un gioco può richiedere a un utente di accettare un contratto di licenza. Il gioco viene quindi installato e quindi viene visualizzato il codice di licenza di DirectX. Questa licenza consente agli utenti di rifiutare e quindi ignorare l'installazione del runtime DirectX. Questo scenario può causare un gioco che non viene eseguito a causa di componenti mancanti.
...
Per altre informazioni sull'installazione di giochi, sulle tecniche di installazione di giochi non tradizionali, sulle soluzioni di applicazione di patch compatibili con controllo dell'account utente e sulla gestione di patch frequenti, vedere gli articoli di DirectX seguenti:

-   [Semplificazione dell'installazione dei giochi](/windows/desktop/DxTechArts/simplifying-game-installation)
-   [Installazione su richiesta per giochi](/windows/desktop/DxTechArts/install-on-demand-for-games)
-   [Applicazione di patch al software di gioco Windows XP, Windows Vista e Windows 7](/windows/desktop/DxTechArts/patching-methods-in-windows-xp-and-vista)
-   [Procedure consigliate per l'installazione di giochi online multiplayer massicci](/windows/desktop/DxTechArts/mmo-installation-best-practices)

> [!Note]  
> La rimozione dei file di dati generati specifici dell'utente deve essere eseguita solo per l'utente corrente e per i percorsi utente condivisi comuni. Non esiste un modo affidabile per analizzare il sistema per rimuovere i file specifici dell'utente per altri utenti, anche se la rimozione richiede credenziali amministrative.
...
Una delle principali modifiche al sistema operativo Windows in Windows Vista è l'aggiunta di Controllo dell'account utente, che esegue applicazioni con privilegi ridotti per impostazione predefinita. Di conseguenza, i programmi di installazione devono gestire i livelli di privilegi di conseguenza. Windows 7 fa anche ampio uso di Controllo dell'account utente. Anche se Windows 7 migliora l'esperienza utente di Controllo dell'account utente, i programmi di installazione devono comunque soddisfare gli stessi requisiti di Windows Vista per funzionare correttamente, senza basarsi su comportamenti di virtualizzazione potenzialmente confusi.

Controllo dell'account utente è attivo per impostazione predefinita in Windows Vista e Windows 7 e la maggior parte dei clienti (88% o più, in base al feedback) lascia abilitata questa funzionalità.
...
Per altre informazioni sulla configurazione del firewall Windows, vedere l'articolo di DirectX Windows [firewall](/windows/desktop/DxTechArts/games-and-firewalls) per sviluppatori di giochi e l'esempio FirewallInstallHelper.

L'avvio standard del gioco alla fine del processo di installazione non riesce a soddisfare l'ultimo aspetto di questo requisito se l'installazione viene avviata da un utente standard e se il processo di installazione richiede privilegi amministrativi, ovvero richiede le credenziali di amministratore. Eredita anche i privilegi amministrativi, che rappresenta un potenziale rischio per la sicurezza. Al contrario, un caricatore bootstrap di installazione deve avviare il gioco dopo la restituzione da una chiamata riuscita del programma di installazione. Per altre informazioni, vedere l'articolo di MSDN Magazine Teach Your Apps to Play Nicely with Windows Vista User Account Control (Insegnare alle app a Windows [controllo dell'account utente di Vista).](/archive/msdn-magazine/2007/january/teach-your-apps-to-work-with-windows-vista-user-account-control)
...
Gli utenti devono avere la flessibilità necessaria per installare le applicazioni laddove necessario. Devono inoltre avere un'esperienza coerente e sicura con il percorso predefinito dei file.
...
I giochi possono usare i vari percorsi di cartelle noti, ad esempio quelli specificati da CSIDL LOCAL APPDATA e \_ \_ CSIDL \_ COMMON APPDATA, per archiviare quantità significative di dati di gioco e file eseguibili di supporto per implementare scenari avanzati di installazione su richiesta e applicazione di \_ patch.

Poiché l'installazione può richiedere l'elevazione a un account utente diverso durante il processo di installazione di tutti gli utenti, non esiste un percorso utente corretto in cui archiviare i dati in fase di installazione. Inoltre, se la crittografia dei file è abilitata, i file crittografati sono accessibili solo dall'account utente che li ha creati.
...
Le applicazioni non devono tentare di installare file o chiavi del Registro di sistema protette da Windows Resource Protection (WRP). Se l'applicazione richiede versioni più recenti dei componenti di sistema, è necessario aggiornare questi componenti utilizzando un Microsoft Service Pack o un pacchetto di installazione approvato da Microsoft che contiene il componente di sistema. I componenti di sistema non devono mai essere riasfesi.
...
Windows La protezione delle risorse (WRP) è progettata per garantire che le risorse di sistema protette siano aggiornate solo tramite meccanismi di installazione o aggiornamento approvati da Microsoft. WRP migliora l'affidabilità del sistema assicurando che i risultati di un'installazione siano prevedibili.
...
WRP è il successore Windows protezione file, che protegge la maggior parte dei componenti di sistema installati nella cartella Windows. WRP protegge la maggior parte delle chiavi del Registro di sistema che archiviano le impostazioni per la creazione di oggetti COM. Riserva inoltre alcune cartelle per l'uso esclusivo da parte del sistema operativo. I tentativi di accesso alle risorse protette in genere comportano un errore di negazione dell'accesso.

Per informazioni dettagliate sulle procedure consigliate per la distribuzione del runtime DirectX con un gioco, vedere l'articolo installazione di DirectX per [sviluppatori di giochi.](/windows/desktop/DxTechArts/directx-setup-for-game-developers)
...
Se il programma di installazione di un gioco usa la Windows Installer (MSI) senza azioni personalizzate, questa funzionalità viene fornita automaticamente. I pacchetti di ridistribuzione Microsoft supportano anche Gestione riavvio.
...
AppVerifier verifica molti problemi noti che causano arresti anomali Windows applicazioni, nonché vulnerabilità di sicurezza note.
...
Per altre informazioni sui Application Verifier, vedere [Application Verifier](/previous-versions/ms220948(v=vs.80)) [e Uso Application Verifier all'interno](/previous-versions/aa480483(v=msdn.10)) del ciclo di vita dello sviluppo software su MSDN. È possibile scaricare Application Verifier [dettagli del download: Application Verifier](https://www.microsoft.com/downloads/details.aspx?familyid=c4a25ab9-649d-4a1b-b4a7-c9d8b095df18) nell'Area download Microsoft.
...
Le SEGNALAZIONE ERRORI WINDOWS forniscono un feedback essenziale a Microsoft per rilevare arresti anomali e blocchi diffusi nelle applicazioni. Ciò consente a Microsoft e ai suoi partner di rilevare e risolvere rapidamente i problemi di sistema e driver che causano errori dell'applicazione.
...
| X                                        | Pulsante X.                                                                                     |
| S                                        | Pulsante Y.                                                                                     |
| controller Xbox 360 per Windows          | Il Xbox 360 gamepad venduto come SKU hardware del PC, incluso un disco Windows driver di dispositivo.          |
...
-   I provider di servizi di giochi devono registrarsi con Games Explorer usando la versione 2 dello schema GDF e usare la funzionalità RSS per fornire notizie correlate al servizio.

### <a name="games-for-windows-showcases"></a>Giochi per Windows showcase

### <a name="introduction"></a>Introduzione

Le vetrine Windows Giochi per Windows offrono un'esperienza di gioco solida Windows PC. Implementando queste funzionalità, i giochi possono aggiungere maggiore entusiasmo all'esperienza utente nelle Windows più recenti.
...
Windows giochi che supportano il tocco sono riproducibili tramite tocco e/o movimenti nei computer che eseguono Windows 7 con un display tocco. È possibile usare anche l'input da tastiera, ma l'interfaccia di riproduzione principale deve essere basata sul tocco.
...
Gli schermi con supporto per il multitocchetto per computer sono disponibili per portatili e desktop e rappresentano una funzionalità hardware chiave che viene promossa con il rilascio di Windows 7. Windows 7 supporta il tocco Windows tutte le interfacce dei controlli comuni e del desktop.
...
Usando una destinazione di rendering a colori elevati, è possibile eseguire il rendering del contenuto hdr (High-Dynamic Range) con una gamma estesa o ampia quando si esegue il mapping del tono a un intervallo a 10 o 16 bit.
...
| <span id="User_Account_Control_Guidelines__"></span><span id="user_account_control_guidelines__"></span><span id="USER_ACCOUNT_CONTROL_GUIDELINES__"></span>Linee guida per il controllo dell'account utente <br/>                      | [Windows Requisiti di sviluppo dell'applicazione Vista per la compatibilità del controllo dell'account utente](/previous-versions/dotnet/articles/bb530410(v=msdn.10))<br/> |
...
| <span id="Games_for_Windows_and_DirectX_SDK_Blog"></span><span id="games_for_windows_and_directx_sdk_blog"></span><span id="GAMES_FOR_WINDOWS_AND_DIRECTX_SDK_BLOG"></span>Blog sui giochi Windows e DirectX SDK<br/> | [Giochi per Windows e DirectX SDK](https://walbourn.github.io/)<br/>                                                                           |
